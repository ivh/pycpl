...
main(void)
{
    const char *names[] = { "char",   "bool",          "int",
                            "long",   "float",         "double",
                            "string", "float complex", "double complex" };

    const char *strings[] = { "Mary",    "Mary had a little lamb", "Mary had a",
                              STRING100, STRING100 STRING100,      "Mary had" };
    ...
    const cpl_type types[] = { CPL_TYPE_CHAR,          CPL_TYPE_BOOL,
    ...
    /* cpl_property_compare_sortkey() relies on sortkey being unsigned */
    cpl_test_assert(CPL_PROPERTY_SORT_UNDEF == (cpl_property_sorttype)0);
    cpl_test_assert(CPL_PROPERTY_SORT_UNDEF < CPL_PROPERTY_SORT_MAX);

    cpl_test_assert(CPL_DICB_PRIMARY < CPL_DICB_HIERARCH_XYZ);
    ...
    for (size_t i = 0; i < nprops; i++) {
        for (size_t j = 1; j <= 100; j += 10) {
            ...
            code = cpl_property_set_comment(property, "");
            cpl_test_eq_error(code, CPL_ERROR_NONE);
            emptystr = cpl_property_get_comment(property);
            cpl_test_error(CPL_ERROR_NONE);
            if (emptystr != NULL) { /* NULL or zero-length is equivalent */
                cpl_test_zero(strlen(emptystr));
            }

            code = cpl_property_set_comment(property, NULL);
            cpl_test_eq_error(code, CPL_ERROR_NONE);
            emptystr = cpl_property_get_comment(property);
            cpl_test_error(CPL_ERROR_NONE);
            if (emptystr != NULL) { /* NULL or zero-length is equivalent */
    ...
    for (size_t i = 0; i < nprops; i++) {
        for (size_t j = 0; j < nstrings; j++) {
            ...
            for (size_t k = 0; k < nstrings; k++) {
                ...
                cpl_test_eq_string(cpl_property_get_name(plist[i]), strings[k]);
