...
main(void)
{
    ...
    cpl_test_eq(cpl_test_get_bytes_image(img1), cpl_test_get_bytes_image(img2));
    ...
    cpl_test_null(cpl_func);
    nxfail++;
    cpl_test_nonnull(NULL);
    nxfail++;

    cpl_test_eq_ptr(NULL, cpl_func);
    nxfail++;
    cpl_test_eq_ptr(cpl_func, NULL);
    nxfail++;

    cpl_test_noneq_ptr(NULL, NULL);
    nxfail++;
    cpl_test_noneq_ptr(cpl_func, cpl_func);
    nxfail++;
    ...
    cpl_test_eq_error(CPL_ERROR_NONE, CPL_ERROR_EOL);
    nxfail++;
    ...
    /* Set a CPL error */
    (void)cpl_msg_set_log_name(NULL);
    cpl_test_eq_error(CPL_ERROR_EOL, CPL_ERROR_EOL);
    nxfail++;
    ...
    cpl_test_eq_string("A", "B");
    nxfail++;
    cpl_test_eq_string(NULL, "");
    nxfail++;
    cpl_test_eq_string(NULL, NULL);
    nxfail++;

    cpl_test_noneq_string(cpl_func, cpl_func);
    nxfail++;
    cpl_test_noneq_string(cpl_func, NULL);
    nxfail++;
    cpl_test_noneq_string(NULL, cpl_func);
    nxfail++;
    cpl_test_noneq_string(NULL, NULL);
    nxfail++;

    cpl_test_abs(0.0, 0.0, -1.0);
    nxfail++;

    cpl_test_abs(0.0, 2.0, 1.0);
    nxfail++;
    cpl_test_abs(2.0, 0.0, 1.0);
    nxfail++;


    cpl_test_abs_complex(0.0, 0.0, -1.0);
    nxfail++;

    cpl_test_abs_complex(0.0, 2.0 * _Complex_I, 1.0);
    nxfail++;
    cpl_test_abs_complex(2.0 * _Complex_I, 0.0, 1.0);
    nxfail++;


    cpl_test_rel(0.0, 0.0, -1.0);
    nxfail++;
    cpl_test_rel(0.0, 1.0, 1.0);
    nxfail++;
    cpl_test_rel(1.0, 0.0, 1.0);
    nxfail++;

    cpl_test_rel(1.0, 3.0, 1.0);
    nxfail++;

    cpl_test_leq(1.0, 0.0);
    nxfail++;
    cpl_test_lt(0.0, 0.0);
    nxfail++;

    cpl_test_vector_abs(NULL, NULL, 1.0);
    nxfail++;

    cpl_test_vector_abs(vec1, vec2, 0.5);
    nxfail++;

    cpl_test_matrix_abs(mat1, mat2, 0.5);
    nxfail++;

    cpl_test_array_abs(NULL, NULL, 1.0);
    nxfail++;

    cpl_test_array_abs(arr1i, arr2i, 0.5);
    nxfail++;

    cpl_test_array_abs(arr1f, arr2f, 0.5);
    nxfail++;

    cpl_test_array_abs(arr1d, arr2d, 0.5);
    nxfail++;

    cpl_test_array_abs(arr1s, arr2s, 0.0);
    nxfail++;

    cpl_test_image_abs(img1, NULL, 1.0);
    nxfail++;
    cpl_test_image_abs(NULL, img2, 1.0);
    nxfail++;
    cpl_test_image_rel(img1, NULL, 1.0);
    nxfail++;
    cpl_test_image_rel(NULL, img2, 1.0);
    nxfail++;

    cpl_test_image_abs(img1, img2, 0.5);
    nxfail++;

    cpl_test_image_rel(img1, img1, -0.5);
    nxfail++;
    ...
    error = cpl_image_multiply_scalar(img1, 2.0);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    cpl_test_image_rel(img1, img3, 1.0 - FLT_EPSILON);
    nxfail++;
    error = cpl_image_multiply_scalar(img1, 0.5);
    cpl_test_eq_error(error, CPL_ERROR_NONE);

    cpl_test_imagelist_abs(NULL, NULL, 1.0);
    nxfail++;
    cpl_test_imagelist_abs(imglist1, NULL, 1.0);
    nxfail++;
    cpl_test_imagelist_abs(NULL, imglist2, 1.0);
    nxfail++;
    cpl_test_imagelist_abs(imglist1, imglist2, 0.5);
    nxfail++;
    (void)cpl_imagelist_unset(imglist1, 0);
    cpl_test_imagelist_abs(imglist1, imglist2, 1.0 + FLT_EPSILON);
    nxfail++;

    cpl_test_polynomial_abs(poly1, poly2, -1.0);
    nxfail++;
    ...
    cpl_test_polynomial_abs(poly1, poly2, 0.0);
    nxfail++;
    cpl_test_polynomial_abs(NULL, poly2, 0.0);
    nxfail++;
    cpl_test_polynomial_abs(poly1, NULL, 0.0);
    nxfail++;
    cpl_test_polynomial_abs(NULL, NULL, 0.0);
    nxfail++;

    cpl_test_eq_mask(mask1, NULL);
    nxfail++;
    cpl_test_eq_mask(NULL, mask1);
    nxfail++;
    ...
    if (fp != NULL) {
        ...
        cpl_test_fits(CPL_TEST_FITS_NAME);
        nxfail++;
