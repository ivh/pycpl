...
static cpl_error_code
    CPL_CONCAT2X(cpl_image_hypot,
                 CPL_CONCAT2X(CPL_TYPE1, CPL_CONCAT2X(CPL_TYPE2, CPL_TYPE3)))(
        cpl_image *,
        const cpl_image *,
        const cpl_image *)
    ...
    const cpl_image *second)
{
    ...
    if (first == NULL) {
        ...
        if (first->bpm != NULL && second->bpm != NULL) {
            if (self->bpm == NULL)
                self->bpm = cpl_mask_wrap(self->nx, self->ny,
                                          (cpl_binary *)cpl_malloc(nxy));
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
CPL_TYPE_ADD(cpl_image_logarithm)(cpl_image *self, double base)
{
    double scale;
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
CPL_TYPE_ADD(cpl_image_exponential)(cpl_image *self, double base)
{
    const int myerrno = errno;
    CPL_TYPE *data = cpl_image_get_data(self);
    const size_t n = cpl_image_get_size_x(self) * cpl_image_get_size_y(self);
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
CPL_TYPE_ADD(cpl_image_power)(cpl_image *self, double exponent)
{
    const int myerrno = errno;
    CPL_TYPE *data = cpl_image_get_data(self);
    const size_t n = cpl_image_get_size_x(self) * cpl_image_get_size_y(self);
        ...
        if (exponent != ceil(exponent)) {
            ...
            for (i = 0; i < n; i++) {
                if (!cbpm || !cbpm[i]) { /* Pixel is not bad (yet) */
                    if ((double)data[i] != 0.0) {
                        data[i] =
                            (CPL_TYPE)cpl_tools_ipow((double)data[i], exponent);
                    }
                    else {
                        errno = EDOM; /* Use errno to reuse code */
                    }
...
if (image1 == NULL) {
    (void)cpl_error_set_message_(CPL_ERROR_NULL_INPUT, "First image is NULL");
    return NULL;
}
else if (image2 == NULL) {
    (void)cpl_error_set_message_(CPL_ERROR_NULL_INPUT, "Second image is NULL");
    return NULL;
}
else if (image1->nx != image2->nx) {
    (void)cpl_error_set_message_(CPL_ERROR_INCOMPATIBLE_INPUT,
                                 "Images are %" CPL_SIZE_FORMAT
                                 " X %" CPL_SIZE_FORMAT
                                 " (%s) <=> %" CPL_SIZE_FORMAT
                                 " X %" CPL_SIZE_FORMAT " (%s)",
                                 image1->nx, image1->ny,
                                 cpl_type_get_name(image1->type), image2->nx,
                                 image2->ny, cpl_type_get_name(image2->type));
    return NULL;
}
...
#define CPL_MATH_ABS1 abs
    case CPL_TYPE_INT: {
        ...
        int *pout = (int *)cpl_malloc(image1->nx * image1->ny * sizeof(*pout));
...
#undef CPL_MATH_ABS2
        ...
        self =
            pout ? cpl_image_wrap_double(image1->nx, image1->ny, pout) : NULL;
        break;
    }
...
#undef CPL_MATH_ABS2
        ...
        self = pout
                   ? cpl_image_wrap_double_complex(image1->nx, image1->ny, pout)
                   : NULL;
        break;
    }
#undef CPL_MATH_ABS1
...
if (im1 == NULL) {
    return cpl_error_set_message_(CPL_ERROR_NULL_INPUT, "First image is NULL");
}
else if (im2 == NULL) {
    return cpl_error_set_message_(CPL_ERROR_NULL_INPUT, "Second image is NULL");
}
...
case CPL_TYPE_T: {
    ...
    for (i = 0; i < nxy; i++) {
        if (pi[i] > hi_cut)
            pi[i] = (CPL_TYPE)assign_hi_cut;
        else if (pi[i] < lo_cut)
            pi[i] = (CPL_TYPE)assign_lo_cut;
    }
    break;
}
...
case CPL_TYPE_T: {
    ...
    if (cpl_tools_copy_window(po, in->pixels, sizeof(CPL_TYPE), in->nx, in->ny,
                              llx, lly, urx, ury)) {
        cpl_free(po);
...
case CPL_TYPE_T: {
    ...
    cpl_binary *bpm = NULL;
    const cpl_binary *bin =
        self->bpm ? cpl_mask_get_data_const(self->bpm) : NULL;
    ...
    if (direction == 1) {
        ...
        if (bin == NULL) {
            ...
            bin += drop_ll;
            for (j = 0; j < (size_t)(self->ny);
                 j++, pi += self->nx, bin += self->nx) {
                size_t k = 0;
                for (i = 0; i < nmed; i++) {
                    if (!bin[i])
                        med[k++] = pi[i];
...
case CPL_TYPE_T: {
    ...
    switch (rot) {
        ...
        case 2: {
            ...
            CPL_TYPE_ADD(cpl_tools_flip)
            ((CPL_TYPE *)self->pixels, (size_t)self->nx * (size_t)self->ny);
            break;
        }
        case 3: {
            CPL_TYPE *pi = (CPL_TYPE *)self->pixels;
