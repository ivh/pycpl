...
#undef CPL_CLASS
    ...
    /* Handle bad pixels */
    if (hasbad)
        cpl_image_reject_from_mask(out, bad);
    cpl_mask_delete(bad);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_warp_polynomial(cpl_image *out,
                          const cpl_image *in,
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_extract_subsample(const cpl_image *image,
                            cpl_size xstep,
                            cpl_size ystep)
{
    cpl_image *out_im;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_rebin(const cpl_image *image,
                cpl_size xstart,
                cpl_size ystart,
                cpl_size xstep,
                cpl_size ystep)
    ...
    if (image->bpm != NULL) {
        ...
        for (j = ystart - 1; j < ny; j++) {
            for (i = xstart - 1; i < nx; i++) {
                pos = (i - xstart + 1) / xstep +
                      ((j - ystart + 1) / ystep) * new_nx;
...
/**
  @ingroup cpl_image
  @brief    Interpolate a pixel from its neigbors
  ...
  This function interpolates a single pixel value as a weighted average of its
  neigbors, specified as being inside a circle or ellipse with the distance
  dependent weights provided by the caller.
...
/*----------------------------------------------------------------------------*/
double
cpl_image_get_interpolated(const cpl_image *source,
                           double xpos,
                           double ypos,
                           const cpl_vector *xprofile,
                           double xradius,
    ...
    switch (source->type) {
        case CPL_TYPE_DOUBLE:
            value = cpl_image_get_interpolated_double(
                ...
                source->bpm ? cpl_mask_get_data_const(source->bpm) : NULL, nx,
                ny, xtabsperpix, ytabsperpix, xpos, ypos, yweight, pxprof,
                xradius, pyprof, yradius, sqyradius, sqyxratio, pconfid);
            break;
        case CPL_TYPE_FLOAT:
...
cpl_error_code
cpl_image_fill_jacobian_polynomial(cpl_image *out,
                                   const cpl_polynomial *poly_x,
                                   const cpl_polynomial *poly_y)
{
    cpl_polynomial *dxdu;
...
cpl_error_code
cpl_image_fill_jacobian(cpl_image *out,
                        const cpl_image *deltax,
                        const cpl_image *deltay)
{
    cpl_image *dxdu;
