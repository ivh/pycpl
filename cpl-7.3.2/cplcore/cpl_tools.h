...
/* Flag to specify WCS key removal */
#define CPL_IO_WCS ((CPL_IO_MAX) << 1)

#define CPL_FITS_EXACT                                                \
    CXSTR("SIMPLE", 6), CXSTR("BSCALE", 6), CXSTR("BZERO", 5),        \
        CXSTR("EXTEND", 6), CXSTR("XTENSION", 8), CXSTR("BITPIX", 6), \
        CXSTR("PCOUNT", 6), CXSTR("GCOUNT", 6), CXSTR("DATE", 4),     \
        CXSTR("TFIELDS", 7)

#define CPL_FITS_EXACT_TILE                                             \
    CXSTR("ZIMAGE", 6), CXSTR("ZCMPTYPE", 8), CXSTR("ZNAXIS", 6),       \
        CXSTR("ZTENSION", 8), CXSTR("ZPCOUNT", 7), CXSTR("ZGCOUNT", 7), \
        CXSTR("ZBITPIX", 7), CXSTR("ZSCALE", 6), CXSTR("ZZERO", 5),     \
        CXSTR("ZBLANK", 6), CXSTR("ZHECKSUM", 8), CXSTR("ZDATASUM", 8)

#define CPL_FITS_EXACT_EXT CPL_FITS_EXACT, CPL_FITS_EXACT_TILE

#define CPL_FITS_EXACT_PRIM CXSTR("BLOCKED", 7), CPL_FITS_EXACT_EXT

#define CPL_FITS_START                                           \
    CXSTR("NAXIS", 5), CXSTR("TFORM", 5), CXSTR("TTYPE", 5),     \
        CXSTR("TUNIT", 5), CXSTR("TZERO", 5), CXSTR("TSCAL", 5), \
        CXSTR("TDIM", 4), CXSTR("TNULL", 5)

#define CPL_FITS_START_TILE \
    CXSTR("ZNAME", 5), CXSTR("ZVAL", 4), CXSTR("ZTILE", 5), CXSTR("ZNAXIS", 6)

#define CPL_FITS_START_PRIM CPL_FITS_START, CPL_FITS_START_TILE

#define CPL_FITS_START_EXT CXSTR("ORIGIN", 6), CPL_FITS_START_PRIM
...
#if defined fits_get_img_sizell && defined fits_read_pixll && \
    defined fits_create_imgll
...
#define cpl_tools_trace(LEVEL)                                       \
    do {                                                             \
        ...
        if (cpl_tools_trace_level == CPL_MSG_ERROR ||                \
            ...
            (cpl_tools_trace_level == CPL_MSG_ERROR                  \
                 ? cpl_msg_error                                     \
                 : (cpl_tools_trace_level == CPL_MSG_WARNING         \
                        ? cpl_msg_warning                            \
                        : (cpl_tools_trace_level == CPL_MSG_INFO     \
...
long long
cpl_tools_get_kth_long_long(long long *, cpl_size, cpl_size) CPL_ATTR_NONNULL;
cpl_size
cpl_tools_get_kth_cplsize(cpl_size *, cpl_size, cpl_size) CPL_ATTR_NONNULL;

double
cpl_tools_quickselection_double(double *, cpl_size, cpl_size) CPL_ATTR_NONNULL;
float
cpl_tools_quickselection_float(float *, cpl_size, cpl_size) CPL_ATTR_NONNULL;
...
long long cpl_tools_quickselection_long_long(long long *,
                                             cpl_size,
                                             cpl_size) CPL_ATTR_NONNULL;
cpl_size cpl_tools_quickselection_cplsize(cpl_size *,
                                          cpl_size,
                                          cpl_size) CPL_ATTR_NONNULL;
...
double
cpl_tools_get_variancesum_long_long(const long long *, cpl_size, double *);
double cpl_tools_get_variancesum_cplsize(const cpl_size *, cpl_size, double *);

double cpl_tools_get_variance_double(const double *,
                                     cpl_size,
                                     double *) CPL_ATTR_NONNULL;
double cpl_tools_get_variance_float(const float *,
                                    cpl_size,
                                    double *) CPL_ATTR_NONNULL;
double
cpl_tools_get_variance_int(const int *, cpl_size, double *) CPL_ATTR_NONNULL;
double
cpl_tools_get_variance_long(const long *, cpl_size, double *) CPL_ATTR_NONNULL;
double cpl_tools_get_variance_long_long(const long long *,
                                        cpl_size,
                                        double *) CPL_ATTR_NONNULL;
double cpl_tools_get_variance_cplsize(const cpl_size *,
                                      cpl_size,
                                      double *) CPL_ATTR_NONNULL;
...
long long
cpl_tools_get_median_long_long(long long *, cpl_size) CPL_ATTR_NONNULL;
cpl_size cpl_tools_get_median_cplsize(cpl_size *, cpl_size) CPL_ATTR_NONNULL;

cpl_error_code cpl_tools_copy_window(void *,
                                     const void *,
                                     size_t,
                                     cpl_size,
                                     cpl_size,
...
cpl_error_code cpl_tools_shift_window(void *,
                                      size_t,
                                      cpl_size,
                                      cpl_size,
                                      int,
...
cpl_error_code cpl_tools_sort_stable_pattern_cplsize(cpl_size const *,
                                                     ...
                                                     cpl_size *);
cpl_error_code
cpl_tools_sort_stable_pattern_int(int const *, cpl_size, int, int, cpl_size *);
...
cpl_error_code cpl_tools_permute_cplsize(cpl_size *,
                                         cpl_size,
                                         cpl_size *,
                                         cpl_size const *,
                                         cpl_size *,
                                         cpl_size const *);
cpl_error_code cpl_tools_permute_int(cpl_size *,
                                     cpl_size,
                                     int *,
                                     int const *,
                                     int *,
                                     int const *);
cpl_error_code cpl_tools_permute_long(cpl_size *,
                                      cpl_size,
                                      long *,
                                      long const *,
                                      long *,
                                      long const *);
cpl_error_code cpl_tools_permute_long_long(cpl_size *,
                                           cpl_size,
                                           long long *,
                                           long long const *,
                                           long long *,
                                           long long const *);
cpl_error_code cpl_tools_permute_float(cpl_size *,
                                       cpl_size,
                                       float *,
                                       float const *,
                                       float *,
                                       float const *);
cpl_error_code cpl_tools_permute_double(cpl_size *,
                                        cpl_size,
                                        double *,
                                        double const *,
                                        double *,
                                        double const *);
cpl_error_code cpl_tools_permute_string(cpl_size *,
                                        cpl_size,
                                        char **,
                                        char *const *,
                                        char **,
...
cpl_vector *cpl_vector_transform_mean(const cpl_vector *,
                                      double *) CPL_ATTR_NONNULL CPL_ATTR_ALLOC;
