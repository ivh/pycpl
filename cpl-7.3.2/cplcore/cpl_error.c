...
#ifndef CPL_LINENOSZ
#define CPL_LINENOSZ 6
#endif

#define MAX_WHERE_LENGTH \
    (2 + (MAX_NAME_LENGTH) + (MAX_FILE_LENGTH) + (CPL_LINENOSZ))
...
static cpl_error *
cpl_error_fill(const char *, cpl_error_code, const char *, unsigned);
static cpl_error_code
cpl_error_set_message_macro_(const char *,
                             cpl_error_code,
                             const char *,
                             unsigned,
                             const char *,
                             va_list) CPL_ATTR_PRINTF(5, 0)
...
cpl_error_reset(void)
{
    ...
    if (!cpl_error_read_only)
        cpl_error_status = CPL_FALSE;
 ...
 */
cpl_error_code
cpl_error_set_message_macro(const char *function,
                            cpl_error_code code,
                            const char *file,
                            unsigned line,
                            const char *text,
                            ...)
{
    ...
    lcode =
        cpl_error_set_message_macro_(function, code, file, line, text, arglist);
 ...
 */
cpl_error_code
cpl_error_set_fits_macro(const char *function,
                         cpl_error_code code,
                         int fitscode,
                         ...
                         ...)
{
    ...
    newformat = cpl_sprintf("\"%s\" from CFITSIO "
                            ...
                            "(ver. " CPL_STRINGIFY(CFITSIO_VERSION) ") "
                                                 ...
                                                 "%s%s=%d. %s",
                                cfitsio_msg, has_name ? fitsfunction : "",
                                has_name ? "()" : "error", fitscode, text);

    va_start(arglist, text);

    lcode = cpl_error_set_message_macro_(function, code, file, line, newformat,
                                         arglist);
 ...
 */
cpl_error_code
cpl_error_set_regex_macro(const char *function,
                          cpl_error_code code,
                          int regcode,
                          const regex_t *preg,
                          const char *file,
 ...
 */
cpl_error_code
cpl_error_set_wcs_macro(const char *function,
                        cpl_error_code code,
                        int wcserror,
                        ...
                        ...)
{
    ...
    if (wcserror < 0) {
        newformat = cpl_sprintf("%s%s=%d < 0. %s", has_name ? wcsfunction : "",
        ...
        newformat =
            cpl_sprintf("%s%s=%d. wcs_errmsg[%d] == NULL. %s",
                        has_name ? wcsfunction : "", has_name ? "()" : "error",
...
/*----------------------------------------------------------------------------*/
const char *
cpl_error_get_message_default(cpl_error_code code)
{
    const char *message;
 ...
 */
static cpl_error *
cpl_error_fill(const char *function,
               cpl_error_code code,
               const char *file,
               unsigned line)
{
 ...
 */
static cpl_error_code
cpl_error_set_message_macro_(const char *function,
                             cpl_error_code code,
                             const char *file,
                             unsigned line,
                             const char *text,
                             va_list arglist)
{
    ...
    char *message =
        (text != NULL && text[0] != '\0' && (text[0] != ' ' || text[1] != '\0'))
            ? cpl_vsprintf(text, arglist)
            : NULL;
    const char *usemsg = message ? message : text;

    const cpl_error_code lcode =
        cpl_error_set_message_one_macro(function, code, file, line, usemsg);
 ...
 */
cpl_error_code
cpl_error_set_message_one_macro(const char *function,
                                cpl_error_code code,
                                const char *file,
                                unsigned line,
                                const char *text)
{
    const cpl_error_code lcode =
        code == CPL_ERROR_HISTORY_LOST ? CPL_ERROR_UNSPECIFIED : code;
    ...
    if (!cpl_error_read_only && code != CPL_ERROR_NONE) {
        ...
        if (text != NULL && text[0] != '\0' &&
            ...
            (void)cx_snprintf((cxchar *)error->msg,
                              ...
                              (const cxchar *)"%s: %s",
                              cpl_error_get_message_default(lcode), text);
        }
    }
