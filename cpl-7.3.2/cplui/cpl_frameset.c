...
inline static void
_cpl_frameset_cache_push(const cpl_frameset *self,
                         cpl_frameset_cacheid id,
...
inline static void
_cpl_frameset_history_merge(cx_list *self,
                            cx_list *other,
...
_cpl_frameset_history_sort(cx_list *self,
                           const cx_multimap *frames,
                           cpl_frame_compare_func compare)
{
    if (cx_list_size(self) > 1) {
        ...
        cx_list_splice(tmp, cx_list_begin(tmp), self, position,
                       cx_list_end(self));
...
cpl_frameset_erase(cpl_frameset *self, const char *tag)
{
    ...
    while (position != last) {
        cx_assert(strcmp(cx_multimap_get_key(self->frames, position), tag) ==
                  0);
                      ...
                      cpl_size *nb_labels)
{
    ...
    *nb_labels = nlabels;
    ...
    cpl_msg_debug(cpl_func,
                  "%" CPL_SIZE_FORMAT " frames labelized with %" CPL_SIZE_FORMAT
                  " labels after %" CPL_SIZE_FORMAT " comparisons",
                  nframes, nlabels, ncomp);
...
cpl_frameset *
cpl_frameset_extract(const cpl_frameset *self,
