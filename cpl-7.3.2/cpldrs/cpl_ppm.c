...
static cpl_array *find_all_matches(const cpl_matrix *,
                                   const cpl_matrix *,
                                   double,
                                   const cpl_polynomial *,
                                   const cpl_polynomial *,
                                   cpl_matrix **,
                                   cpl_matrix **) CPL_ATTR_ALLOC;
...
static triangle *triangle_new(double,
                              double,
                              double,
                              double,
                              double,
...
cpl_bivector *
cpl_ppm_match_positions(const cpl_vector *peaks,
     ...
     */
        ...
        for (i = 1; i < n; i++) {
           ...
           */
                    ...
                    disp =
                        (lambda[i] - lambda[i - 1]) / (xpos[i] - xpos[i - 1]);
                }
         ...
         */
                ...
                hi_start = xpos[i - 1] +
                           (line[ilambda[i - 1] + j] - lambda[i - 1]) / disp;
     ...
     */
        ...
        if (n > 0) {
           ...
           */
                ...
                hi_start = xpos[n - 1] +
                           (line[ilambda[n - 1] + 1] - lambda[n - 1]) / disp;
             ...
             int idc)
{
    ...
    const double t1 = double_atan2(ya - yb, CPL_MATH_SQRT2 * error, xa - xb,
                                   CPL_MATH_SQRT2 * error, &dt1);
    const double t2 = double_atan2(ya - yc, CPL_MATH_SQRT2 * error, xa - xc,
                                   CPL_MATH_SQRT2 * error, &dt2);
...
static void
triangles_delete_holder(triangles **t)
{
    cpl_free((*t)->t);
    cpl_free(*t);
    *t = NULL;
    ...
    for (i = 0; i < n - 2; i++) {
        for (j = i + 1; j < n - 1; j++) {
            for (k = j + 1; k < n; k++) {
                ...
                pattern->t[count] =
                    triangle_new(x1, y1, x2, y2, x3, y3, err, i, j, k);
                count++;
                pattern->t[count] =
                    triangle_new(x2, y2, x1, y1, x3, y3, err, j, i, k);
                count++;
                pattern->t[count] =
                    triangle_new(x3, y3, x2, y2, x1, y1, err, k, j, i);
                count++;
            }
        }
    }
...
difference_of_angles(double a1, double a2)
{
    ...
    while (a1 < -CPL_MATH_PI)
        a1 += CPL_MATH_2PI;
    while (a1 > CPL_MATH_PI)
        a1 -= CPL_MATH_2PI;
    while (a2 < -CPL_MATH_PI)
        a2 += CPL_MATH_2PI;
    while (a2 > CPL_MATH_PI)
        a2 -= CPL_MATH_2PI;
...
static int
find_transform(const cpl_array *matches,
               ...
               cpl_polynomial *trans_y)
{
    ...
    error = cpl_polynomial_fit(trans_x, ppos, NULL, xdpos, NULL, CPL_FALSE,
                               NULL, &degree);
    ...
    if (error == CPL_ERROR_SINGULAR_MATRIX) {
        ...
        error = cpl_polynomial_fit(trans_x, ppos, NULL, xdpos, NULL, CPL_TRUE,
                                   NULL, degrees);
    ...
    if (!error) {
        error = cpl_polynomial_fit(trans_y, ppos, NULL, ydpos, NULL, CPL_FALSE,
                                   NULL, &degree);
...
static cpl_array *
find_all_matches(const cpl_matrix *pattern,
                 const cpl_matrix *data,
                 double radius,
                 const cpl_polynomial *trans_x,
    ...
    if (nvalid) {
        if (mpattern && mdata) {
            *mdata = cpl_matrix_new(2, nvalid);
            md = cpl_matrix_get_data_(*mdata);
            *mpattern = cpl_matrix_new(2, nvalid);
            mp = cpl_matrix_get_data_(*mpattern);
            count = 0;
            for (i = 0; i < pnc; i++) {
                j = cpl_array_get_int(matches, i, &null);
                if (null)
...
cpl_array *
cpl_ppm_match_points(const cpl_matrix *data,
                     cpl_size use_data,
                     double err_data,
                     const cpl_matrix *pattern,
                     ...
                     double *lin_angle)
{
    ...
    nd = cpl_matrix_get_ncol_(data);
    np = cpl_matrix_get_ncol_(pattern);
...
*/
    ...
    if (cpl_table_has_valid(table, "distance")) {
        ...
        cpl_msg_debug(cpl_func,
                      ...
                      median_angle * CPL_MATH_DEG_RAD,
                      rms_angle * CPL_MATH_DEG_RAD);
#endif
    }
    else {
    ...
    for (i = 0; i < p_triangle->n; i++) {
        if (n_triangle->t[i]) {
            if (fabs(scale[i] - median_scale) > scale_tolerance ||
                difference_of_angles(angle[i], median_angle) >
                    angle_tolerance) {
#ifdef CPL_PPM_DEBUG
                cpl_msg_debug(cpl_func,
                              "Discarding %d/%d: |%g - %g| > %g * %g or "
                              "diff_angle(%g, %g) > %g * |%g|",
                              i + 1, p_triangle->n, scale[i], median_scale,
                              tolerance, median_scale, angle[i], median_angle,
                              tolerance, median_angle);
#endif
                n_triangle->t[i] = NULL;
                cpl_table_set_invalid(table, "distance", i);
                cpl_table_set_invalid(table, "scale", i);
