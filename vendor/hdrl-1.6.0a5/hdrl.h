...
\subsection version160 Release version 1.6.0
The HDRL release version 1.6.0 can be obtained from the HDRL Git repository

https://gitlab.eso.org/pipelines/hdrl.git (tag hdrl-1.6.0)

In this release we updated and/or added the following algorithms:

- The HDRL catalogue has been updated to follow <a
  href="https://www.eso.org/sci/observing/phase3.html">ESO Phase 3
  conventions</a>. This ensures compatibility with ESO's Phase 3 data
  products and standards.

- The build system has been enhanced to support HDRL as a standalone
  third-party library. While HDRL remains usable as a convenience library
  as before, it can now be installed as a standalone library using the
  \c --enable-standalone configure option (false by default). This provides greater
  flexibility for integration into different build environments.
...
- Various quality control and bug-fix improvements have been included
  (see ChangeLog for detailed information).

- Licensing and documentation: clarified GPLv2-or-later licensing by
  homogenising headers and FSF address, replacing LICENSE with a canonical
  COPYING file, and adding GNU-style distribution files (README, INSTALL,
  AUTHORS, ChangeLog, NEWS) that are shipped in the tarball and installed
  under docdir for standalone builds.
