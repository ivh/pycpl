    ...
    double            * mode_fit_err)
{
  ...
  if( histo_nbins < 3 ) {
      ...
      cpl_msg_error(cpl_func,"Cannot do polynomial fit with less than 3 points.");
      ...
      *mode_fit = m + bin_size / 2.;
	  ...
	  if( fabs(value_at_max - m) > bin_size / 2. ) {
	      cpl_error_set(cpl_func, CPL_ERROR_ILLEGAL_INPUT);
	      cpl_msg_error(cpl_func, "Max too close to point distribution edge: abs(value_at_max+bin_size/2-m) > bin_size");
	      is_not_supported_fit = CPL_TRUE;
	  }

	  if(value_at_mode < max_parab) {
	      cpl_error_set(cpl_func, CPL_ERROR_ILLEGAL_INPUT);
	      cpl_msg_error(cpl_func, "Value at mode is not a valid maximum: value_at_mode < np.max(parab)");
	      is_not_supported_fit = CPL_TRUE;
	  }
  ...
  switch(method) {
    ...
    case HDRL_MODE_FIT:
      ...
      if (CPL_ERROR_NONE != hdrl_mode_fit(loc_vec, hmin, hmax, bsize, nbin, error_niter, mode,
		    mode_error) ) {
	  //cpl_error_set(cpl_func,CPL_ERROR_ILLEGAL_INPUT);
          cpl_msg_error(cpl_func,"Mode computation failed using method fit. Try method weight or median.");
      }
      break;
    ...
    case HDRL_MODE_WEIGHTED:
	  ...
	  cpl_msg_error(cpl_func,"Mode computation failed using method weight. Try method fit or median.");
    ...
    case HDRL_MODE_MEDIAN:
      ...
      if (CPL_ERROR_NONE != hdrl_mode_median(loc_vec, hmin, hmax, nbin,
					     error_niter, mode, mode_error) ) {
	  //cpl_error_set(cpl_func,CPL_ERROR_UNSUPPORTED_MODE);
	  cpl_msg_error(cpl_func,"Mode computation failed using method median. Try method fit or weight");
    ...
    default:
      cpl_msg_error(cpl_func,"Unsupported mode method. Supported methods are: fit, weight, median");
