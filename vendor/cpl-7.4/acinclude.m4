...
[
    ...
    CPLJAVA_INCLUDES='-I$(top_srcdir)/cpljava -I$(top_builddir)/cpljava'
...
[
    ...
    AC_ARG_WITH(java-includes,
                AS_HELP_STRING([--with-java-includes],
    ...
    AC_ARG_WITH(java-includes-md,
                AS_HELP_STRING([--with-java-includes-md],
...
[
    ...
    AC_ARG_ENABLE(gasgano,
                  AS_HELP_STRING([--enable-gasgano],
                                 [build the gasgano support library (deprecated) [[default=no]]]),
                  cpl_enable_gasgano=$enableval,
                  cpl_enable_gasgano=no)
...
[
    ...
    AC_ARG_WITH(system-cext,
                AS_HELP_STRING([--with-system-cext],
                               [Use the system cext installation for building [default=yes]]),
                [
                    cpl_with_system_cext=$withval
                ],
                [
                    cpl_with_system_cext="yes"
                ])
...
[
    ...
    AC_ARG_ENABLE(memory-mode,
                AS_HELP_STRING([--enable-memory-mode=M],
    ...
    AC_ARG_ENABLE(max-ptrs,
                AS_HELP_STRING([--enable-max-ptrs=MAXPTRS],
    ...
    if test "x$cpl_memory_flag" = xyes ; then
        CPL_CPPFLAGS="-DCPL_XMEMORY_MODE=$cpl_memory_mode"
        case $cpl_memory_mode in
        yes)
          CPL_CPPFLAGS="-DCPL_XMEMORY_MODE=0 -DCPL_XMEMORY_MAXPTRS=1"
          break ;;
        0|1)
          CPL_CPPFLAGS="-DCPL_XMEMORY_MODE=$cpl_memory_mode -DCPL_XMEMORY_MAXPTRS=1"
          break ;;
        2)
          CPL_CPPFLAGS="-DCPL_XMEMORY_MODE=2 $CPL_MAXPTRS_CFLAGS"
          break ;;
        *)
    ...
    AC_MSG_RESULT([CPL_CPPFLAGS=$CPL_CPPFLAGS])
    AC_SUBST(CPL_CPPFLAGS)
])
