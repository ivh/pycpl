...
cx_test_tree_dump(cx_tree *tree, cxint fmt)
{
    ...
    while (pos != cx_tree_end(tree)) {
        fprintf(stderr, "tree node %d at address %p has value ", i + 1, pos);
        ...
        switch (fmt) {
            ...
            case 1:
                fprintf(stderr, "%c", *((cxint *)cx_tree_get_value(tree, pos)));
                break;
...
main(void)
{
    ...
    cx_assert(cx_tree_begin(tree) == cx_tree_previous(tree, cx_tree_end(tree)));
    ...
    for (i = 0; i < strlen(letters); i++) {
        ...
        cx_assert(_pos == cx_tree_end(tree) ||
                  *((cxchar *)cx_tree_get_value(tree, _pos)) == letters[i + 1]);
