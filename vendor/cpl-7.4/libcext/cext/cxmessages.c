...
#define CX_MSG_PREFIX_BUFFER_SIZE          (CX_MSG_FORMAT_UNSIGNED_BUFFER_SIZE + 32)
...
static cx_log_level_flags cx_log_prefix =
    (CX_LOG_LEVEL_ERROR | CX_LOG_LEVEL_WARNING | CX_LOG_LEVEL_CRITICAL |
     CX_LOG_LEVEL_DEBUG);
...
cx_debug_init(void)
{
    ...
    if (value != NULL) {
        static const cx_debug_key keys[] = {{"fatal_warnings",
                                             CX_DEBUG_FATAL_WARNINGS},
                                            {"fatal_criticals",
                                             CX_DEBUG_FATAL_CRITICALS}};

        cx_debug_flags =
            cx_debug_parse_string(value, keys, CX_N_ELEMENTS(keys));
    }
...
cxuint
cx_log_set_handler(const cxchar *name, cx_log_level_flags levels,
        ...
        if (level & test_level) {
            ...
            if (!(test_level & CX_LOG_FLAG_RECURSION) &&
                ...
                if (test_level != test_level_saved) {
                    ...
                    domain = cx_log_domain_find(name ? name : "");
                    log_func =
                        cx_log_domain_get_handler(domain, test_level, &data);
                    domain = NULL;
            ...
            if (test_level & CX_LOG_FLAG_RECURSION) {
                ...
                cx_vsnprintf(buffer, 1024, format, args2);
                va_end(args2);
...
void
cx_critical(const cxchar *format, ...)
{
    va_list args;
...
void
cx_warning(const cxchar *format, ...)
{
    va_list args;
...
void
cx_message(const cxchar *format, ...)
{
    va_list args;
