...
void
cx_slist_splice(cx_slist *tlist, cx_slist_iterator position, cx_slist *slist,
                cx_slist_iterator first, cx_slist_iterator last)
{
    cx_slnode *before_first, *before_last;


    cx_assert(slist != NULL);
    cx_assert(first == _cx_slist_end(slist) || _cx_slnode_exists(slist, first));
    cx_assert(last == _cx_slist_end(slist) || _cx_slnode_follows(first, last));
    ...
    if (before_first != before_last) {
        ...
        cx_assert(slist != tlist || (_cx_slnode_follows(position, first) ||
