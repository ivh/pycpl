...
_cx_deque_shift_left(cx_deque *deque, cxsize n, cxsize position, cxsize size)
{
    ...
    if (n > 0) {
        ...
        memmove(&deque->members[start - n], &deque->members[start],
                size * sizeof(cxptr));
    }
...
_cx_deque_sort(cx_deque *deque, cx_compare_func compare)
{
    ...
    */
    ...
    if (_cx_deque_size(deque) > 1 && !_cx_deque_sorted(deque, compare)) {
        ...
        _cx_deque_transfer(&tmp, _cx_deque_end(&tmp), deque, middle,
                           _cx_deque_end(deque));
...
cx_deque_insert(cx_deque *deque, cx_deque_iterator position, cxcptr data)
{
    ...
    if (position == _cx_deque_size(deque)) {
        ...
        cx_deque_push_back(deque,
                           _cx_deque_get(deque, _cx_deque_size(deque) - 1));
...
void
cx_deque_splice(cx_deque *deque, cx_deque_iterator position, cx_deque *other,
                cx_deque_iterator first, cx_deque_iterator last)
{
    cx_assert(other != NULL);
    cx_assert(
        (first == _cx_deque_end(other)) ||
        ((first >= _cx_deque_begin(other)) && (first < _cx_deque_end(other))));
