...
typedef void (*cx_log_func)(const cxchar *, cx_log_level_flags, const cxchar *,
                            cxptr);
...
void cx_log_default_handler(const cxchar *, cx_log_level_flags, const cxchar *,
                            cxptr);
cx_log_func cx_log_set_default_handler(cx_log_func);
cxuint cx_log_set_handler(const cxchar *, cx_log_level_flags, cx_log_func,
                          cxptr);
...
void cx_log(const cxchar *, cx_log_level_flags, const cxchar *, ...)
    CX_GNUC_PRINTF(3, 4);
...
#    define cx_assert(expr)                                                  \
        do {                                                                 \
            if (expr) {                                                      \
                ...
                cx_log(CX_LOG_DOMAIN, CX_LOG_LEVEL_ERROR,                    \
                       "file %s: line %d: assertion failed: (%s)", __FILE__, \
                       __LINE__, #expr);                                     \
            }                                                                \
        } while (0)
#  endif /* !__GNUC__ */
