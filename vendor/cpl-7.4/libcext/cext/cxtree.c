...
_cx_tree_rotate_left(cx_tnode *node, cx_tnode **root)
{
    ...
    else if (node == _cx_tnode_left(_cx_tnode_parent(node))) {
...
_cx_tree_rotate_right(cx_tnode *node, cx_tnode **root)
{
    ...
    else if (node == _cx_tnode_right(_cx_tnode_parent(node))) {
        _cx_tnode_right(_cx_tnode_parent(node)) = y;
    }
    else {
        _cx_tnode_left(_cx_tnode_parent(node)) = y;
...
_cx_tree_rebalance(cx_tnode *node, cx_tnode **root)
{
    ...
    while (node != *root &&
        ...
        if (_cx_tnode_parent(node) == _cx_tnode_left(_cx_tnode_grandpa(node))) {
                             ...
                             cx_tnode **leftmost, cx_tnode **rightmost)
{
    ...
    if (y != node) {
        ...
        else if (_cx_tnode_left(_cx_tnode_parent(node)) == node) {
            _cx_tnode_left(_cx_tnode_parent(node)) = y;
        }
        else {
            _cx_tnode_right(_cx_tnode_parent(node)) = y;
...
void
cx_tree_equal_range(const cx_tree *tree, cxcptr key, cx_tree_iterator *begin,
...
void
cx_tree_erase_range(cx_tree *tree, cx_tree_iterator begin, cx_tree_iterator end)
{
    cx_assert(tree != NULL);
...
cx_tree_verify(const cx_tree *tree)
{
    ...
    for (it = _cx_tree_begin(tree); it != _cx_tree_end(tree);
        ...
        if (L &&
            _cx_tree_key_compare(tree, _cx_tnode_key(x), _cx_tnode_key(L))) {
            return FALSE;
        }

        if (R &&
            _cx_tree_key_compare(tree, _cx_tnode_key(R), _cx_tnode_key(x))) {
            return FALSE;
        }
