...
main(void)
{
    ...
    q = cpl_parameterlist_find(NULL, NULL);
    cpl_test_error(CPL_ERROR_NULL_INPUT);
    cpl_test_null(q);

    q = cpl_parameterlist_find(list, NULL);
    cpl_test_error(CPL_ERROR_NULL_INPUT);
    cpl_test_null(q);
    ...
    error = cpl_parameterlist_append(list, p[0]);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    p[1] = cpl_parameter_new_value("b", CPL_TYPE_STRING, "parameter2", NULL,
                                   "value2");
    cpl_test_nonnull(p[1]);
    error = cpl_parameterlist_append(list, p[1]);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
