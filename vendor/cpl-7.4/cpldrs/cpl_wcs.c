...
/**
 * @brief
 *  Create a copy of the given wcs.
 *
 * @param self The input wcs object to copy.
 *
 * @return
 ...
 *       <td class="ecr">
 *         The parameter <i>self</i> does not contain WCS information.
 *       </td>
 *     </tr>
 *   </table>
 * @enderror
 *
 * The function creates a deep copy of the given wcs  @em self,
 * i.e the created copy and the original wcs @em self do not share
 * any resources.
 */
cpl_wcs *
cpl_wcs_duplicate(const cpl_wcs *self)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, NULL);
    cpl_ensure(self->wcsptr != NULL, CPL_ERROR_ILLEGAL_INPUT, NULL);
    cpl_wcs *other = _cpl_wcs_init(self->wcsptr, self->istab);

    /* 
     * If the input WCS originates from an image, i.e. istab is zero, calling
     * _cpl_wcs_init() will only create an imdims member with the dimension
     * set to zero for each axis. Thus copy the actual image dimensions here
     * explicitly.
     */

    if (self->istab == 0) {
        for (cpl_size iaxis = 0; iaxis < self->naxis; ++iaxis) {
            cpl_array_set_int(other->imdims, iaxis,
                              cpl_array_get_int(self->imdims, iaxis, NULL));
        }
    }
    return other;
}
                 ...
                 cpl_propertylist **olist)
{
    ...
    *olist = NULL;
    ...
    if (retval) {
        ...
        const cpl_cstr *erasekeys[] = {CXSTR("PC1_1", 5),   CXSTR("PC1_2", 5),
                                       CXSTR("PC2_1", 5),   CXSTR("PC2_2", 5),
                                       CXSTR("CDELT1", 6),  CXSTR("CDELT2", 6),
                                       CXSTR("RESTFRQ", 7), CXSTR("RESTWAV", 7),
                                       CXSTR("END", 3)};
        ...
        *olist = cpl_propertylist_new();
        ...
        /* Need null-terminator for parsing a card */
        cardi[FLEN_CARD - 1] = '\0';
        ...
        */
        ...
        for (int k = 0; k < i; k++) {
            ...
            propi = cpl_property_new_from_string(cardi, NULL, NULL);
            if (propi == NULL) {
                (void)cpl_error_set_message_(cpl_error_get_code(),
                                             "Bad WCS FITS Card %d/%d", k, i);
                break;
            }
            (void)cpl_propertylist_set_property(*olist, propi);
        }
        free(o);
...
_cpl_wcs_init(struct wcsprm *wcs, int is_tab)
{
    ...
    if (wcs != NULL) {
        ...
        /*
         * Deep copy of the wcsprm structure. wcscopy is a macro defined for
         * backwards compatibility (before wcslib 3.6). Use the actual function
         * call here
         */
        wcssub(1, wcs, NULL, NULL, _wcs);
        wcsset(_wcs);
