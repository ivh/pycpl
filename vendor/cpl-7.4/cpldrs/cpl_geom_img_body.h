...
/*----------------------------------------------------------------------------*/
static void
CPL_TYPE_ADD(cpl_geom_ima_offset_xcorr_subw)(double *correl,
                                             const cpl_image *im1,
                                             const cpl_image *im2,
                                             cpl_size x_1,
                                             cpl_size y_1,
...

{
    ...
    const CPL_TYPE *pi1 =
        CPL_TYPE_ADD_CONST(cpl_image_get_data)(im1) + x_1 + y_1 * nx;
    const CPL_TYPE *pi2 =
        CPL_TYPE_ADD_CONST(cpl_image_get_data)(im2) + x_2 + y_2 * nx;
    ...
    for (; l <= s_hy_max; l++, pxc += 1 + 2 * s_hx) {
        ...
        for (; k <= s_hx_max; k++) {
            ...
            const cpl_size m_hx_max =
                CX_MIN(m_hx, nx - 1 - CX_MAX(x_1 + k, x_2));
            const cpl_size m_hy_max =
                CX_MIN(m_hy, ny - 1 - CX_MAX(y_1 + l, y_2));
            const cpl_size npix =
                (1 + m_hx_max - m_hx_min) * (1 + m_hy_max - m_hy_min);
...
/*----------------------------------------------------------------------------*/
static void
CPL_TYPE_ADD(cpl_geom_img_offset_saa)(cpl_image *final,
                                      cpl_image *contrib,
                                      cpl_size rmin,
                                      cpl_size rmax,
                                      double start_x,
...

{
    ...
    double *acc =
        (double *)cpl_malloc((size_t)nx * (size_t)nima * sizeof(*acc));
    ...
    int *ncontrib = (int *)cpl_calloc((size_t)nx, sizeof(int));
    cpl_size *offset_ij = cpl_malloc(2 * (size_t)nima * sizeof(*offset_ij));
    double *rsc = (double *)cpl_malloc((size_t)nima * 8 * sizeof(*rsc));
    cpl_boolean *isint =
        (cpl_boolean *)cpl_calloc((size_t)nima, sizeof(*isint));
    cpl_size j, p;
    ...
    for (p = 0; p < nima; p++) {
        ...
        if (tabx == 0 && taby == 0) {
            ...
            const double sumrs =
                (interp_kernel[tabsperpix + tabx] + interp_kernel[tabx] +
                 interp_kernel[tabsperpix - tabx] +
                 interp_kernel[2 * tabsperpix - tabx]) *
                (interp_kernel[tabsperpix + taby] + interp_kernel[taby] +
    ...
    for (j = 0; j < ny; j++) {
        ...
        for (p = 0; p < nima; p++) {
            ...
            if (py > 1 && py < (sizey - 2)) {
                ...
                const cpl_size i1 =
                    CX_MIN(sizex - 2 - offset_ij[0 + p * 2], nx);
            ...
            if (ncontrib[i] > rtot) {
                ...
                if (rmax > 0)
                    (void)cpl_tools_get_kth_double(acci + rmin,
                                                   ...
                                                   ncontrib[i] - rtot);
#else
                cpl_geom_img_get_min_max_double(acci, ncontrib[i], rmin, rmax);
#endif
...
/*----------------------------------------------------------------------------*/
static void
CPL_TYPE_ADD(cpl_geom_img_offset_saa_all)(cpl_image *final,
                                          cpl_image *contrib,
                                          double start_x,
                                          double start_y,
                                          const cpl_imagelist *ilist,
...

{
    ...
    for (p = 0; p < nima; p++) {
        ...
        if (tabx == 0 && taby == 0) {
            ...
            for (py = py0; py < py1;
                 py++, poj += nx, pcj += nx, pbj += nx, pij += sizex) {
            ...
            for (py = py0; py < py1;
                ...
                for (px = px0; px < px1; px++) {
                    ...
                    poj[px] +=
                        ...
                        rsc[5] * (rsc[0] * pij[px - 1] + rsc[1] * pij[px] +
                                  rsc[2] * pij[px + 1] + rsc[3] * pij[px + 2]) +
