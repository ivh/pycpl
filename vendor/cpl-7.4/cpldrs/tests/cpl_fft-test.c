...
static void
    cpl_fft_imagelist_test_image(cpl_size, cpl_size, cpl_size, cpl_type);
...
cpl_fft_image_test(void)
{
    ...
    for (ityp = 0; ityp < (int)(sizeof(imtypes) / sizeof(imtypes[0])); ityp++) {
        ...
        for (ityp2 = 0; ityp2 < (int)(sizeof(imtypes) / sizeof(imtypes[0]));
            ...
            for (idir = 0; idir < 2;
                 idir++, mode = CPL_FFT_BACKWARD, imgin = img2, imgout = img3) {
                ...
                else if ((imtype & CPL_TYPE_INT) || (imtype2 & CPL_TYPE_INT)) {
                    cpl_test_eq_error(CPL_ERROR_TYPE_MISMATCH, error);
                }
                else if ((imtype &
                          (CPL_TYPE_FLOAT | CPL_TYPE_DOUBLE | CPL_TYPE_INT)) !=
                         (imtype2 &
                          (CPL_TYPE_FLOAT | CPL_TYPE_DOUBLE | CPL_TYPE_INT))) {
                    cpl_test_eq_error(CPL_ERROR_TYPE_MISMATCH, error);
                }
...
static void
cpl_fft_image_test_one(cpl_size nx, cpl_size ny, cpl_type type)
{
    ...
    cpl_test_image_abs(image3h, image4,
                       80.0 * (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                       : FLT_EPSILON));
    ...
    cpl_test_image_abs(image3c, image1c,
                       type == CPL_TYPE_DOUBLE ? 128.0 * DBL_EPSILON
                                               : 40.0 * FLT_EPSILON);
    ...
    cpl_test_image_abs(image1r, image2,
                       6.0 * (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                      : FLT_EPSILON));
    ...
    cpl_test_image_abs(image4c, image4,
                       2.0 * (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                      : FLT_EPSILON));
...
static void
cpl_fft_imagelist_test_one(cpl_size nx, cpl_size ny, cpl_size nz, cpl_type type)
{
    const int rigor = CPL_FFT_FIND_MEASURE;
    ...
    for (i = 0; i < nz; i++) {
        ...
        cpl_image *image3c;
        cpl_image *image3h =
            cpl_image_new(nx / 2 + 1, ny, type | CPL_TYPE_COMPLEX);
        cpl_image *image5 = cpl_image_new(nx, ny, type);
    ...
    for (i = 0; i < nz; i++) {
        ...
        cpl_image *image4 = cpl_image_extract(image3r, 1, 1, nx / 2 + 1, ny);
    ...
    /* That half has to match the transform onto the half-sized image */
    cpl_test_imagelist_abs(ilist3h, ilist4,
                           80.0 * (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                           : FLT_EPSILON));
    ...
    cpl_test_imagelist_abs(ilist3c, ilist1c,
                           2.0 * (nx + ny) *
                               (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                        : FLT_EPSILON));
    ...
    for (i = 0; i < nz; i++) {
        ...
        cpl_image *image4 = cpl_image_extract(image3c, 1, 1, nx / 2 + 1, ny);
    ...
    /* The back-transformed must match the original image */
    cpl_test_imagelist_abs(ilist1r, ilist2,
                           6.0 * (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                          : FLT_EPSILON));
    ...
    cpl_test_imagelist_abs(ilist3r, ilist3c,
                           8.0 * (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                          : FLT_EPSILON));
    ...
    cpl_test_imagelist_abs(ilist1r, ilist4r,
                           6.0 * (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                          : FLT_EPSILON));

    cpl_test_imagelist_abs(ilist4c, ilist4,
                           2.0 * (type == CPL_TYPE_DOUBLE ? DBL_EPSILON
                                                          : FLT_EPSILON));
...
/*----------------------------------------------------------------------------*/
static void
cpl_fft_imagelist_test_image(cpl_size nx,
                             cpl_size ny,
                             cpl_size nz,
                             cpl_type type)
{
    ...
    if (timei1 > 0.0 && timel1 > 0.0) {
        cpl_msg_info(cpl_func,
                     "List vs single %d X %d X %d (%s): %g <=> %g "
                     "[s] (%g <=> %g [MFLOP/s])",
                     (int)nx, (int)ny, (int)nz, cpl_type_get_name(type), timel1,
                     timei1, 1e-6 * (double)flopl1 / timel1,
        ...
        cpl_msg_info(cpl_func,
                     ...
                     "[s] (%g <=> %g [MFLOP])",
                     (int)nx, (int)ny, (int)nz, cpl_type_get_name(type), timel1,
                     timei1, 1e-6 * (double)flopl1, 1e-6 * (double)flopi1);
    }
}
...
/*----------------------------------------------------------------------------*/
static void
cpl_fft_image_test_correlate(cpl_size nx, cpl_size ny, cpl_type type)
{
