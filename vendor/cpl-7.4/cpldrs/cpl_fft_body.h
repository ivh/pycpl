...
CPL_TYPE_ADD(cpl_fft_image)(cpl_image *self,
                            ...
                            cpl_boolean is_last)
{
    ...
    if (mode & CPL_FFT_FORWARD) {
        ...
        CPL_FFTW_TYPE *out_b =
            (CPL_FFTW_TYPE *)CPL_TYPE_ADD_COMPLEX(cpl_image_get_data)(self);
        ...
        if (typin & CPL_TYPE_COMPLEX) {
            ...
            in_b = (CPL_FFTW_TYPE *)CPL_TYPE_ADD_COMPLEX_CONST(
...
#pragma omp critical(cpl_fft_fftw)
#endif
                {
                    ...
                    *pbufin = CPL_FFTW_ADD(malloc)(
                        nxin * sizeof(CPL_FFTW_TYPE) * nyin);
                    *pbufout = CPL_FFTW_ADD(malloc)(
                        nxin * sizeof(CPL_FFTW_TYPE) * nyin);

                    /* FIXME: If unaligned then drop FFTW_PRESERVE_INPUT */
                    pforw = CPL_FFTW_ADD(
                        plan_dft_2d)(nyin, nxin, *pbufin, *pbufout,
                                     FFTW_FORWARD, rigor | FFTW_PRESERVE_INPUT);
                }
            ...
            out_bt = in_b == out_b
                         ? *pbufout
                         : cpl_fft_aligned((void *)out_b, *pbufout, alignmask);
...
#pragma omp critical(cpl_fft_fftw)
#endif
                {
                    ...
                    *pbufin =
                        CPL_FFTW_ADD(malloc)(nxin * sizeof(CPL_TYPE) * nyin);
                    *pbufout = CPL_FFTW_ADD(malloc)(
                        nxh * sizeof(CPL_FFTW_TYPE) * nyin);

                    /* FIXME: If unaligned then drop FFTW_PRESERVE_INPUT */
                    pforw = CPL_FFTW_ADD(
                        plan_dft_r2c_2d)(nyin, nxin, (CPL_TYPE *)*pbufin,
                                         *pbufout, rigor | FFTW_PRESERVE_INPUT);
                }
                if (pplan != NULL)
                    *pplan = pforw;
            }
...
#pragma omp critical(cpl_fft_fftw)
        ...
        const CPL_FFTW_TYPE *in_b =
            (const CPL_FFTW_TYPE *)CPL_TYPE_ADD_COMPLEX_CONST(
                cpl_image_get_data)(other);
        size_t alignmask;
...
#pragma omp critical(cpl_fft_fftw)
#endif
                {
                    ...
                    *pbufout = CPL_FFTW_ADD(malloc)(
                    ...
                    /* FIXME: If unaligned then drop FFTW_PRESERVE_INPUT */
                    pback =
                        CPL_FFTW_ADD(plan_dft_2d)(nyin, nxin, *pbufin, *pbufout,
                                                  FFTW_BACKWARD,
                                                  FFTW_PRESERVE_INPUT | rigor);
                }
...
#pragma omp critical(cpl_fft_fftw)
#endif
                {
                    ...
                    *pbufin = CPL_FFTW_ADD(malloc)(nxh * sizeof(CPL_FFTW_TYPE) *
                                                   nyin);
                    *pbufout =
                        CPL_FFTW_ADD(malloc)(nxout * sizeof(CPL_TYPE) * nyin);
