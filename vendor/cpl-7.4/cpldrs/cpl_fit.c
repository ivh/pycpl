...
cpl_fit_imagelist_polynomial_window_(cpl_imagelist *,
                                     const cpl_vector *,
                                     const cpl_imagelist *,
                                     cpl_size,
                                     cpl_size,
...
static void cpl_fit_imagelist_residual_double(cpl_image *,
                                              cpl_size,
                                              cpl_size,
                                              const cpl_vector *,
                                              const cpl_vector *,
...
static void cpl_fit_imagelist_residual_float(cpl_image *,
                                             cpl_size,
                                             cpl_size,
                                             const cpl_vector *,
                                             const cpl_vector *,
...
static void cpl_fit_imagelist_residual_int(cpl_image *,
                                           cpl_size,
                                           cpl_size,
                                           const cpl_vector *,
                                           const cpl_vector *,
...
static void cpl_fit_imagelist_fill_float(cpl_imagelist *,
                                         cpl_size,
                                         cpl_size,
                                         const cpl_matrix *);

static void cpl_fit_imagelist_fill_int(cpl_imagelist *,
                                       cpl_size,
                                       cpl_size,
                                       const cpl_matrix *);
...
static int
bigauss_derivative(const double[], const double[], double[]) CPL_ATTR_NONNULL;
...
cpl_error_code
cpl_fit_lvmq(const cpl_matrix *x,
             ...
             cpl_matrix **covariance)
{
    return cpl_fit_lvmq_(x, sigma_x, y, sigma_y, a, ia, NULL, NULL, f, dfda,
                         relative_tolerance, tolerance_count, max_iterations,
                                    ...
                                    cpl_image *fiterror)
{
    ...
    */
    ...
    for (i = 0; i < nc; i++) {
        cpl_image *image =
            cpl_image_wrap(nx, ny, pixeltype,
                           cpl_malloc((size_t)nx * (size_t)ny *
    ...
    if (redo == NULL ||
        cpl_mask_get_first_window(redo, 1, 1, nx, ny, CPL_BINARY_0) >= 0) {
        /* Some (or all) interpolations are free of bad pixels */

        error =
            cpl_fit_imagelist_polynomial_window_(self, x_pos, values, llx, lly,
                                                 mindeg, is_symsamp, fiterror);
    }


    if (!error && redo != NULL &&
        cpl_mask_get_first_window(redo, 1, 1, nx, ny, CPL_BINARY_1) >= 0) {
        /* Some (or all) interpolations have bad pixels */

        error = cpl_fit_imagelist_polynomial_bpm(self, redo, x_pos, values, llx,
                                                 lly, mindeg, fiterror);
    }
                                     ...
                                     cpl_image *fiterror)
{
    ...
    if (is_eqzero) {
        ...
        double *dmh = cpl_matrix_get_data_(mh);
    ...
    if (!error) {
        ...
        if (mindeg > 1) {
            ...
            double *ppow = (double *)cpl_malloc((size_t)np * sizeof(*ppow));
        ...
        switch (cpl_image_get_type(first)) {
            case CPL_TYPE_DOUBLE:
                error =
                    cpl_fit_imagelist_polynomial_double(self, mh, mv, x_pos,
                                                        values, llx, lly, xpow,
                                                        -xmean, fiterror);
                break;
            case CPL_TYPE_FLOAT:
                error =
                    cpl_fit_imagelist_polynomial_float(self, mh, mv, x_pos,
                                                       values, llx, lly, xpow,
                                                       -xmean, fiterror);
                break;
            case CPL_TYPE_INT:
...
/*----------------------------------------------------------------------------*/
cpl_imagelist *
cpl_fit_imagelist_polynomial(const cpl_vector *x_pos,
                             const cpl_imagelist *values,
                             cpl_size mindeg,
                             cpl_size maxdeg,
                             cpl_boolean is_symsamp,
                             ...
                             cpl_image *fiterror)
{
    ...
    cpl_imagelist *self =
        cpl_fit_imagelist_polynomial_window(x_pos, values, 1, 1, nx, ny, mindeg,
                                            maxdeg, is_symsamp, pixeltype,
                                            fiterror);
...
cpl_error_code
cpl_fit_image_gaussian(const cpl_image *im,
                       ...
                       cpl_matrix **phys_cov)
{
    ...
    if (xpos < 1 || ypos < 1 || xpos > mx || ypos > my || xsize > mx ||
        ysize > my)
        ...
        if (invalid) {
            ...
            if (normalisation > 0.0) {
                for (i = 1; i < nx; i++) {
                    ...
                    const double next =
                        bad ? 0.0 : cpl_image_get(box, i + 1, iycen, &bad);
    ...
    if (ia[3]) {
        ...
        cpl_array_set_double(uppersan, 3,
                             (double)nx + 3.0 * CPL_MAX(xsize, xsigma));
    }
    if (ia[4]) {
        /* Do not allow the center to be far outside the box limit */
        cpl_array_set_double(lowersan, 4, 1.0 - 3.0 * CPL_MAX(ysize, ysigma));
        cpl_array_set_double(uppersan, 4,
                             (double)ny + 3.0 * CPL_MAX(ysize, ysigma));
    }
    ...
    if (do_wrap) {
        ...
        if (ebox)
            dval_data = (double *)cpl_image_get_data_double_const(ebox);
        CPL_DIAG_PRAGMA_POP;
    }
    else {
    ...
    if (has_bpm) {
        ...
        for (j = 0; j < ny; j++) {
            for (i = 0; i < nx; i++) {
                ...
                if (!bad) {
                    ...
                    if (ebox) {
                        const double evalue =
                            cpl_image_get(ebox, i + 1, j + 1, &bad);
                        if (!bad)
                            dval_data[row++] = evalue;
                    }
                    else {
                        row++;
                    }
        ...
        for (j = 0; j < ny; j++) {
            for (i = 0; i < nx; i++, row++) {
                ...
                if (ebox)
                    dval_data[row] = cpl_image_get_(ebox, i + 1, j + 1);
            }
        }
        assert(row == nrow);
    }
    ...
    if (dvalues) {
        ...
        if (i < nrow) {
            /* Create error message with value before it is deallocated */
            const cpl_error_code code =
                cpl_error_set_message_(CPL_ERROR_ILLEGAL_INPUT,
                                       "Non-positive error %g found in "
                                       "error image, sample %d/%d",
                                       dval_data[i], 1 + (int)i, (int)nrow);
        ...
        if (red_chisq != NULL)
            *red_chisq = 0.0;
        status = cpl_fit_lvmq_(positions, NULL, values, dvalues, a, ia,
                               lowersan, uppersan, bigauss, bigauss_derivative,
                               CPL_FIT_LVMQ_TOLERANCE, CPL_FIT_LVMQ_COUNT,
                               CPL_FIT_LVMQ_MAXITER, rms, red_chisq, &own_cov);
    }
    else {
        status = cpl_fit_lvmq_(positions, NULL, values, NULL, a, ia, lowersan,
                               uppersan, bigauss, bigauss_derivative,
                               CPL_FIT_LVMQ_TOLERANCE, CPL_FIT_LVMQ_COUNT,
                               CPL_FIT_LVMQ_MAXITER, rms, NULL, NULL);
    }
    ...
    if (status == CPL_ERROR_NONE) {
        ...
        cpl_array_set_double(parameters, 3,
                             cpl_vector_get_(a, 3) + (double)(llx - 1));
        cpl_array_set_double(parameters, 4,
                             cpl_vector_get_(a, 4) + (double)(lly - 1));
            ...
            for (i = 0; i < 7; i++) {
                if (ia[i])
                    cpl_array_set_double(err_params, i,
                                         sqrt(cpl_matrix_get_(own_cov, i, i)));
            }
        }
        ...
        if (fabs(R) <= DR) {
            ...
            A = S_x * S_y *
                sqrt(2 * (1 - R * R) * cos(2 * theta) /
                     ((S_x * S_x + S_y * S_y) * cos(2 * theta) + S_y * S_y -
                      S_x * S_x));
            B = S_x * S_y *
                sqrt(2 * (1 - R * R) * cos(2 * theta) /
                     ((S_x * S_x + S_y * S_y) * cos(2 * theta) - S_y * S_y +
        ...
        if (phys_cov) {
            ...
            cpl_matrix_set_(jacobian, 0, 0, 0.0); /* d(theta)/d(p0) */
            cpl_matrix_set_(jacobian, 0, 1, 0.0); /* d(theta)/d(p1) */
            ...
            if (DR == 0.0)
                cpl_matrix_set_(jacobian, 0, 2, 0.0);
            else
                cpl_matrix_set_(jacobian, 0, 2, (theta_plus - theta) / DR);
            /* d(theta)/d(p2) */
            cpl_matrix_set_(jacobian, 0, 3, 0.0); /* d(theta)/d(p3) */
            cpl_matrix_set_(jacobian, 0, 4, 0.0); /* d(theta)/d(p4) */
            ...
            if (fabs(R) <= DR) {
                ...
                theta_plus =
                    0.5 * atan2(2 * R, (S_x_plus * S_x_plus - S_y * S_y) /
                                           S_x_plus / S_y);
            }

            if (DS_x == 0.0)
                cpl_matrix_set_(jacobian, 0, 5, 0.0);
            else
                cpl_matrix_set_(jacobian, 0, 5, (theta_plus - theta) / DS_x);
            /* d(theta)/d(p5) */
            ...
            if (fabs(R) <= DR) {
                ...
                theta_plus =
                    0.5 * atan2(2 * R, (S_x * S_x - S_y_plus * S_y_plus) / S_x /
                                           S_y_plus);
            }

            if (DS_y == 0.0)
                cpl_matrix_set_(jacobian, 0, 6, 0.0);
            else
                cpl_matrix_set_(jacobian, 0, 6, (theta_plus - theta) / DS_y);
            /* d(theta)/d(p6) */
            ...
            cpl_matrix_set_(jacobian, 1, 0, 0.0); /* d(A)/d(p0) */
            cpl_matrix_set_(jacobian, 1, 1, 0.0); /* d(A)/d(p1) */
            ...
            if (DR == 0.0)
                cpl_matrix_set_(jacobian, 1, 2, 0.0);
            else
                cpl_matrix_set_(jacobian, 1, 2, (A_plus - A) / DR);
            /* d(A)/d(p2) */
            cpl_matrix_set_(jacobian, 1, 3, 0.0); /* d(A)/d(p3) */
            cpl_matrix_set_(jacobian, 1, 4, 0.0); /* d(A)/d(p4) */
            ...
            if (DS_x == 0.0)
                cpl_matrix_set_(jacobian, 1, 5, 0.0);
            else
                cpl_matrix_set_(jacobian, 1, 5, (A_plus - A) / DS_x);
            /* d(A)/d(p5) */
            ...
            if (DS_y == 0.0)
                cpl_matrix_set_(jacobian, 1, 6, 0.0);
            else
                cpl_matrix_set_(jacobian, 1, 6, (A_plus - A) / DS_y);
            /* d(A)/d(p6) */
            ...
            cpl_matrix_set_(jacobian, 2, 0, 0.0); /* d(B)/d(p0) */
            cpl_matrix_set_(jacobian, 2, 1, 0.0); /* d(B)/d(p1) */
            ...
            if (DR == 0.0)
                cpl_matrix_set_(jacobian, 2, 2, 0.0);
            else
                cpl_matrix_set_(jacobian, 2, 2, (B_plus - B) / DR);
            /* d(B)/d(p2) */
            cpl_matrix_set_(jacobian, 2, 3, 0.0); /* d(B)/d(p3) */
            cpl_matrix_set_(jacobian, 2, 4, 0.0); /* d(B)/d(p4) */
            ...
            if (DS_x == 0.0)
                cpl_matrix_set_(jacobian, 2, 5, 0.0);
            else
                cpl_matrix_set_(jacobian, 2, 5, (B_plus - B) / DS_x);
            /* d(B)/d(p5) */
            ...
            if (DS_y == 0.0)
                cpl_matrix_set_(jacobian, 2, 6, 0.0);
            else
                cpl_matrix_set_(jacobian, 2, 6, (B_plus - B) / DS_y);
            /* d(B)/d(p6) */
...
cpl_gaussian_eval_2d(const cpl_array *self, double x, double y)
{
    ...
    if (!cpl_errorstate_is_equal(prestate)) {
        ...
        value = B + A / (CPL_MATH_2PI * S_x * S_y * sqrt(1 - R * R)) *
                        exp(-0.5 / (1 - R * R) *
                            (x_n * x_n + y_n * y_n - 2.0 * R * x_n * y_n));
    }
    else if (fabs(R) > 1.0) {
        (void)cpl_error_set_message_(CPL_ERROR_ILLEGAL_OUTPUT, "fabs(R=%g) > 1",
                                     R);
    }
    else {
...
/*----------------------------------------------------------------------------*/
static cpl_size
cpl_fit_imagelist_polynomial_find_block_size(cpl_size np,
                                             cpl_size nc,
                                             cpl_boolean do_err,
                                             cpl_type ip,
                                             cpl_type op,
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_fit_imagelist_polynomial_bpm(cpl_imagelist *self,
                                 const cpl_mask *redo,
                                 const cpl_vector *x_pos,
                                 const cpl_imagelist *values,
                                 cpl_size llx,
...

{
    ...
    while (!error && (found = memchr(found + 1, CPL_BINARY_1,
        ...
        error = cpl_fit_imagelist_polynomial_one(self, fit1d, xgood, ygood, i,
                                                 j, x_pos, values, llx, lly,
                                                 mindeg, fiterror);
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_fit_imagelist_polynomial_one(cpl_imagelist *self,
...

{
    ...
    if (igood == 0) {
        ...
        cpl_size ndistinct;
        const cpl_error_code err2 =
            cpl_vector_count_distinct(vxcopy, &ndistinct);
        ...
        error = err2 ? err2
                     : cpl_polynomial_fit_1d(fit1d, vxgood, vygood, NULL,
