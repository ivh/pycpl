...
/*---------------------------------------------------------------------------*/
cpl_error_code
cpl_fft_image(cpl_image *self, const cpl_image *other, cpl_fft_mode mode)
{
    return cpl_fft_image_(self, other, mode) ? cpl_error_set_where_()
                                             : CPL_ERROR_NONE;
}
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_fft_imagelist(cpl_imagelist *self,
                  ...
                  cpl_fft_mode mode)
{
    ...
    */
    ...
    for (i = 0; i < sizein; i++) {
        ...
        if (cpl_fft_image__(imgout, imgin, mode, &plan, &bufin, &bufout,
                            i + 1 == sizein))
            break;
    }
