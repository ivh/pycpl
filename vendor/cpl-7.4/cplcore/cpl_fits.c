...
/**
  ...
  @see cpl_init() to control the FITS I/O mode when CPL is started
   ...
   file is opened and closed during that call. However repeated calls on
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_fits_set_mode(cpl_fits_mode mode)
{
    if (mode == CPL_FITS_START_CACHING) {
        ...
        return cpl_error_set_message_(CPL_ERROR_UNSUPPORTED_MODE, "mode=%d",
                                      (int)mode);
    }
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_fits_count_extensions(const char *filename)
{
    fitsfile *fptr;
    int error = 0;
    ...
    if (fits_get_num_hdus(fptr, &next, &error)) { /* ffthdu() */
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_fits_find_extension(const char *filename, const char *extname)
{
    fitsfile *fptr;
    int error = 0;
    cpl_size ext_num = 0;
...
int
cpl_fits_get_nb_extensions(const char *filename)
{
...
int
cpl_fits_get_extension_nb(const char *filename, const char *extname)
{
