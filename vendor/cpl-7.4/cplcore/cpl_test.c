...
/*----------------------------------------------------------------------------*/
double
cpl_test_get_walltime(void)
{
#ifdef CPL_HAVE_CLOCK_GETTIME

    struct timespec buf;

    return clock_gettime(CLOCK_REALTIME, &buf)
               ? 0.0
...
/*----------------------------------------------------------------------------*/
size_t
cpl_test_get_bytes_vector(const cpl_vector *self)
{
    return self == NULL ? 0
...
/*----------------------------------------------------------------------------*/
size_t
cpl_test_get_bytes_matrix(const cpl_matrix *self)
{
    return self == NULL
               ? 0
...
/*----------------------------------------------------------------------------*/
size_t
cpl_test_get_bytes_image(const cpl_image *self)
{
    return self == NULL ? 0
...
/*----------------------------------------------------------------------------*/
size_t
cpl_test_get_bytes_imagelist(const cpl_imagelist *self)
{
    return self == NULL ? 0
...
/*----------------------------------------------------------------------------*/
void
cpl_test_init_macro(const char *filename,
                    const char *report,
...
#pragma omp master
#endif
    {
        ...
        if (filename != NULL) {
            ...
            const char *slashpos =
                bslashpos > fslashpos ? bslashpos : fslashpos;
            if (slashpos != NULL)
                filename = slashpos + 1;
        ...
        if (errnopre != 0) {
            ...
            cpl_msg_debug(cpl_func,
                          "%s() was called with errno=%d: %s (Unless "
        ...
        if (errnoini != 0) {
            /* May be useful for debugging - but see the above errno comment */
            cpl_msg_debug(cpl_func,
                          "cpl_init() set errno=%d: %s (Unless "
                          "you are debugging cpl_init() you can ignore "
                          "this message)",
                          errnoini, strerror(errnoini));
        }
    }
...
/*----------------------------------------------------------------------------*/
void
cpl_test_macro(int errnopre,
               double twallpre,
               cpl_flops flopspre,
               cpl_errorstate statepre,
               cpl_size expression,
               cpl_boolean fail_on_zero,
               const char *expr_txt,
               const char *function,
               const char *file,
               unsigned line)
{
    char *message =
        cpl_sprintf(fail_on_zero ? "(%s) = %" CPL_SIZE_FORMAT
                                 : "(%s) = %" CPL_SIZE_FORMAT " <=> 0",
                    expr_txt, expression);
    const cpl_boolean bval = fail_on_zero ? (expression ? CPL_TRUE : CPL_FALSE)
                                          : (expression ? CPL_FALSE : CPL_TRUE);

    cpl_test_one(errnopre, twallpre, flopspre, statepre, bval, message,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_null_macro(int errnopre,
                    double twallpre,
                    cpl_flops flopspre,
                    cpl_errorstate statepre,
                    const void *pointer,
                    ...
                    unsigned line)
{
    char *message = cpl_sprintf("(%s) = %p == NULL", pointer_string, pointer);

    cpl_test_one(errnopre, twallpre, flopspre, statepre,
                 pointer == NULL ? CPL_TRUE : CPL_FALSE, message, CPL_FALSE,
                 function, file, line);
...
/*----------------------------------------------------------------------------*/
void
cpl_test_nonnull_macro(int errnopre,
                       double twallpre,
                       cpl_flops flopspre,
                       cpl_errorstate statepre,
                       const void *pointer,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_eq_macro(int errnopre,
                  double twallpre,
                  cpl_flops flopspre,
                  cpl_errorstate statepre,
                  cpl_size first,
...
{
    char *message =
        cpl_sprintf("(%s) = %" CPL_SIZE_FORMAT "; (%s) = %" CPL_SIZE_FORMAT,
                    first_string, first, second_string, second);

    cpl_test_one(errnopre, twallpre, flopspre, statepre,
                 first == second ? CPL_TRUE : CPL_FALSE, message, CPL_FALSE,
                 function, file, line);
...
/*----------------------------------------------------------------------------*/
void
cpl_test_noneq_macro(int errnopre,
                     double twallpre,
                     cpl_flops flopspre,
                     cpl_errorstate statepre,
                     cpl_size first,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_eq_ptr_macro(int errnopre,
                      double twallpre,
                      cpl_flops flopspre,
                      cpl_errorstate statepre,
                      const void *first,
                      ...
                      unsigned line)
{
    char *message = cpl_sprintf("(%s) = %p; (%s) = %p", first_string, first,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_noneq_ptr_macro(int errnopre,
                         double twallpre,
                         cpl_flops flopspre,
                         cpl_errorstate statepre,
                         const void *first,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_eq_string_macro(int errnopre,
                         double twallpre,
                         cpl_flops flopspre,
                         cpl_errorstate statepre,
                         const char *first,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_noneq_string_macro(int errnopre,
                            double twallpre,
                            cpl_flops flopspre,
                            cpl_errorstate statepre,
                            const char *first,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_fits_macro(int errnopre,
                    double twallpre,
                    cpl_flops flopspre,
                    cpl_errorstate statepre,
                    const char *filename,
                    ...
                    unsigned line)
{
    ...
    if (filename == NULL) {
        ...
        const char *redir =
            cpl_msg_get_level() < CPL_MSG_WARNING
                ? ""
        ...
        message =
            cpl_sprintf(CPL_TEST_FITS " on file %s: %s", filename_string, cmd);
...
/*----------------------------------------------------------------------------*/
void
cpl_test_eq_mask_macro(int errnopre,
                       double twallpre,
                       cpl_flops flopspre,
                       cpl_errorstate statepre,
                       const cpl_mask *first,
                       const char *first_string,
                       const cpl_mask *second,
                       const char *second_string,
                       const char *function,
                       const char *file,
                       unsigned line)
{
    ...
    if (!cpl_errorstate_is_equal(mystate)) {
        ...
        message =
            cpl_sprintf("%s <=> %s input error", first_string, second_string);
    }
    else if (nx1 != nx2 || ny1 != ny2) {
        expression = CPL_FALSE;
        message =
            cpl_sprintf("%s <=> %s incompatible input, nx: %" CPL_SIZE_FORMAT
                        " <=> %" CPL_SIZE_FORMAT ", ny: %" CPL_SIZE_FORMAT
                        " <=> %" CPL_SIZE_FORMAT,
                        first_string, second_string, nx1, nx2, ny1, ny2);
    }
        ...
        message =
            cpl_sprintf("%s(%" CPL_SIZE_FORMAT ",%" CPL_SIZE_FORMAT
                        ...
                        " of %" CPL_SIZE_FORMAT " x %" CPL_SIZE_FORMAT " "
                        "differ(s))",
                        first_string, 1 + k % nx1, 1 + k / nx1,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_leq_macro(int errnopre,
                   double twallpre,
                   cpl_flops flopspre,
                   cpl_errorstate statepre,
                   double value,
                   ...
                   unsigned line)
{
    const cpl_boolean expression = (value <= tolerance) ? CPL_TRUE : CPL_FALSE;
    char *message = cpl_sprintf("%s = %g <= %g = %s", value_string, value,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_lt_macro(int errnopre,
                  double twallpre,
                  cpl_flops flopspre,
                  cpl_errorstate statepre,
                  double value,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_abs_macro(int errnopre,
                   double twallpre,
                   cpl_flops flopspre,
                   cpl_errorstate statepre,
                   double first,
                   ...
                   unsigned line)
{
    ...
    char *message = cpl_sprintf("|%s - %s| = |%g - %g| = |%g| <= %g = %s",
                                ...
                                first - second, tolerance, tolerance_string);
...
/*----------------------------------------------------------------------------*/
void
cpl_test_abs_complex_macro(int errnopre,
                           double twallpre,
                           cpl_flops flopspre,
                           cpl_errorstate statepre,
                           ...
                           unsigned line)
{
    ...
    char *message =
        cpl_sprintf("|%s - %s| = |(%g%+gi) - (%g%+gi)| = "
                    "|%g%+gi| = %g <= %g = %s",
                    first_string, second_string, creal(first), cimag(first),
                    creal(second), cimag(second), creal(first - second),
                    cimag(first - second), cabs(first - second), tolerance,
                    tolerance_string);
...
/*----------------------------------------------------------------------------*/
void
cpl_test_rel_macro(int errnopre,
                   double twallpre,
                   cpl_flops flopspre,
                   cpl_errorstate statepre,
                   double first,
                   ...
                   unsigned line)
{
    ...
    if (tolerance < 0.0) {
        ...
        expression = CPL_TRUE;
        message =
            cpl_sprintf("%s = %g = %s. (Tolerance %s = %g)", first_string,
                        first, second_string, tolerance_string, tolerance);
    }
    else if (first == 0.0) {
        /* Not needed. Used only for prettier messaging */
        expression = CPL_FALSE;
        message = cpl_sprintf("%s = zero; %s = non-zero (%g). (Tolerance "
                              "%s = %g)",
                              first_string, second_string, second,
                              tolerance_string, tolerance);
    }
    else if (second == 0.0) {
        /* Not needed. Used only for prettier messaging */
        ...
        message = cpl_sprintf("|%s - %s|/|%s| = |%g - %g|/|%g| = |%g|/|%g|"
                              " <= %g = %s",
                              first_string, second_string, first_string, first,
                              second, first, first - second, first, tolerance,
                              tolerance_string);
    }
    else {
        ...
        message = cpl_sprintf("|%s - %s|/|%s| = |%g - %g|/|%g| = |%g|/|%g|"
                              " <= %g = %s",
                              first_string, second_string, second_string, first,
                              second, second, first - second, second, tolerance,
                              tolerance_string);
    }
...
/*----------------------------------------------------------------------------*/
void
cpl_test_vector_abs_macro(int errnopre,
                          double twallpre,
                          cpl_flops flopspre,
                          cpl_errorstate statepre,
                          ...
                          unsigned line)
{
    ...
    if (!cpl_errorstate_is_equal(mystate)) {
        ...
        message = cpl_sprintf("%s <=> %s (tol=%s) input error:", first_string,
        ...
        if (cpl_errorstate_is_equal(mystate)) {
            ...
            message = cpl_sprintf("|%s(%" CPL_SIZE_FORMAT
                                  ") - %s(%" CPL_SIZE_FORMAT ")| = "
                                  "|%g - %g| = |%g| <= %g = %s",
                                  first_string, pos, second_string, pos, val1,
                                  val2, difval, tolerance, tolerance_string);
        }
        else {
            cpl_error_set(cpl_func, cpl_error_get_code());
            ...
            message =
                cpl_sprintf("%s <=> %s (tol=%s) input error:", first_string,
                            second_string, tolerance_string);
        }
    }
...
/*----------------------------------------------------------------------------*/
void
cpl_test_matrix_abs_macro(int errnopre,
                          double twallpre,
                          cpl_flops flopspre,
                          cpl_errorstate statepre,
                          ...
                          unsigned line)
{
    ...
    if (!cpl_errorstate_is_equal(mystate)) {
        ...
        if (cpl_errorstate_is_equal(mystate)) {
            ...
            message =
                cpl_sprintf("|%s(%" CPL_SIZE_FORMAT ",%" CPL_SIZE_FORMAT
                            ") - %s(%" CPL_SIZE_FORMAT ",%" CPL_SIZE_FORMAT
                            ")| = |%g - %g| = "
                            "|%g| <= %g = %s",
                            first_string, irow, icol, second_string, irow, icol,
                            val1, val2, difval, tolerance, tolerance_string);
        }
        else {
            cpl_error_set(cpl_func, cpl_error_get_code());
...
/*----------------------------------------------------------------------------*/
void
cpl_test_array_abs_macro(int errnopre,
                         double twallpre,
                         cpl_flops flopspre,
                         cpl_errorstate statepre,
                         ...
                         unsigned line)
{
    ...
    if (tolerance < 0.0) {
        ...
        message = cpl_sprintf("array1=%s; array2=%s. Negative tolerance %s = "
                              "%g",
                              first_string, second_string, tolerance_string,
                              tolerance);
    }
...
#ifdef CPL_SIZE_FORMAT
        message =
            cpl_sprintf("%s(%" CPL_SIZE_FORMAT ", %s) <=> %s(%" CPL_SIZE_FORMAT
                        ", %s) (tol=%s) input error:",
        ...
        message =
            cpl_sprintf("%s(%" CPL_SIZE_FORMAT ", %s) <=> %s(%" CPL_SIZE_FORMAT
                        ...
                        first_string, nx, cpl_type_get_name(type1),
                        second_string, cpl_array_get_size(second),
                        cpl_type_get_name(type2), tolerance_string, nbad1,
                        nbad2);
#else
        ...
        const cpl_error_code error =
            (is_pos ? cpl_array_get_maxpos : cpl_array_get_minpos)(diff, &posx);

        int is_bad1 = CPL_FALSE;
        int is_bad2 = CPL_FALSE;
        ...
        if (!error && cpl_errorstate_is_equal(mystate)) {
            ...
            message =
                cpl_sprintf("|%s(%" CPL_SIZE_FORMAT
                            ",%s, %s) - %s(%" CPL_SIZE_FORMAT ",%s, %s)| = "
                            "|%g - %g| = |%g| <= %g = %s",
                            first_string, posx, rejstr1,
                            cpl_type_get_name(type1), second_string, posx,
                            rejstr2, cpl_type_get_name(type2), val1, val2,
                            difval, tolerance, tolerance_string);
        }
        else {
            cpl_error_set(cpl_func, cpl_error_get_code());
...
/*----------------------------------------------------------------------------*/
void
cpl_test_image_abs_macro(int errnopre,
                         double twallpre,
                         cpl_flops flopspre,
                         cpl_errorstate statepre,
                         ...
                         unsigned line)
{
    ...
    if (tolerance < 0.0) {
        ...
        message =
            cpl_sprintf("%s(%" CPL_SIZE_FORMAT ",%" CPL_SIZE_FORMAT
                        ", %s) <=> %s(%" CPL_SIZE_FORMAT ",%" CPL_SIZE_FORMAT
                        ", %s) (tol=%s) input error:",
                        first_string, nx, ny, stype1, second_string,
                        cpl_image_get_size_x(second),
                        cpl_image_get_size_y(second), stype2, tolerance_string);
    }
        ...
        const cpl_error_code icode =
            cpl_stats_fill_from_image(&istats, diff,
            ...
            if (cpl_image_get_type(cdiff) & CPL_TYPE_COMPLEX) {
                ...
                message =
                    cpl_sprintf("|%s(%" CPL_SIZE_FORMAT ",%" CPL_SIZE_FORMAT
                                ...
                                second_string, posx, posy, rejstr2, stype2,
                                creal(val1), creal(val2), cimag(val1),
                                cimag(val2), difval, tolerance,
                                tolerance_string);
            }
            else {
...
/*----------------------------------------------------------------------------*/
void
cpl_test_image_rel_macro(int errnopre,
                         double twallpre,
                         cpl_flops flopspre,
                         cpl_errorstate statepre,
                         const cpl_image *first,
                         ...
                         unsigned line)
{
    ...
    if (tolerance < 0.0) {
        ...
        cpl_image *imabs1 = (cpl_image_get_type(first) & CPL_TYPE_COMPLEX)
                                ? cpl_image_extract_mod(first)
                                : cpl_image_abs_create(first);
        cpl_image *imabs2 = (cpl_image_get_type(second) & CPL_TYPE_COMPLEX)
                                ? cpl_image_extract_mod(second)
                                : cpl_image_abs_create(second);
        ...
        cpl_stats istats;
        const cpl_error_code icode =
            cpl_stats_fill_from_image(&istats, posit,
                                      CPL_STATS_MIN | CPL_STATS_MINPOS);
        const cpl_stats *stats = icode ? NULL : &istats;
            ...
            if (cpl_image_get_type(cdiff) & CPL_TYPE_COMPLEX) {
                ...
                message =
                    cpl_sprintf("|%s(%" CPL_SIZE_FORMAT ",%" CPL_SIZE_FORMAT
                                ...
                                second_string, posx, posy, rejstr2, stype2,
                                creal(val1), creal(val2), cimag(val1),
                                cimag(val2), creal(val1) - creal(val2),
                ...
                message =
                    cpl_sprintf("|%s(%" CPL_SIZE_FORMAT ",%" CPL_SIZE_FORMAT
                                ...
                                first_string, posx, posy, rejstr1, stype1,
                                second_string, posx, posy, rejstr2, stype2,
                                val1, val2, val1 - val2, mval);
            }

            if (!expression && cpl_msg_get_level() <= CPL_MSG_ERROR)
                cpl_stats_dump(stats, CPL_STATS_MIN | CPL_STATS_MINPOS, stderr);
        }
        else {
            cpl_error_set(cpl_func, cpl_error_get_code());
...
/*----------------------------------------------------------------------------*/
void
cpl_test_imagelist_abs_macro(int errnopre,
                             double twallpre,
                             cpl_flops flopspre,
                             cpl_errorstate statepre,
                             ...
                             unsigned line)
{
    ...
    if (!cpl_errorstate_is_equal(mystate)) {
        ...
        message = cpl_sprintf("|%s(%" CPL_SIZE_FORMAT ") - %s(%" CPL_SIZE_FORMAT
                              ")| <= %g = %s",
                              first_string, sz1, second_string, sz2, tolerance,
                              tolerance_string);
        ...
        for (i = 0; i < sz1; i++) {
            ...
            char *img1string =
                cpl_sprintf("image %" CPL_SIZE_FORMAT " in first list", 1 + i);
            char *img2string =
                cpl_sprintf("image %" CPL_SIZE_FORMAT " in second list", 1 + i);
...
/*----------------------------------------------------------------------------*/
void
cpl_test_polynomial_abs_macro(int errnopre,
                              double twallpre,
                              cpl_flops flopspre,
                              cpl_errorstate statepre,
                              ...
                              unsigned line)
{
    ...
    if (cpl_errorstate_is_equal(mystate)) {
        ...
        message =
            cpl_sprintf("(dimension %d <=> %d. intol degree=%d) |%s - %s| "
                        "<= %g = %s",
                        (int)dim1, (int)dim2, retval ? retval - 1 : 0,
                        first_string, second_string, tolerance,
                        tolerance_string);
    }
    else {
        expression = CPL_FALSE;
...
/*----------------------------------------------------------------------------*/
void
cpl_test_errorstate_macro(int errnopre,
                          double twallpre,
                          cpl_flops flopspre,
                          cpl_errorstate statepre,
                          ...
                          unsigned line)
{
    ...
    char *message = cpl_sprintf("%s <=> %d (%s)", errorstate_string,
...
/*----------------------------------------------------------------------------*/
void
cpl_test_error_macro(int errnopre,
                     double twallpre,
                     cpl_flops flopspre,
                     cpl_errorstate statepre,
                     ...
                     unsigned line)
{
    char *message = cpl_sprintf("(%s) = %d (%s) <=> %d (%s)", error_string,
                                error, cpl_error_get_message_default(error),
                                cpl_error_get_code(), cpl_error_get_message());

    cpl_test_one(errnopre, twallpre, flopspre, statepre,
                 cpl_error_get_code() == error ? CPL_TRUE : CPL_FALSE, message,
                 CPL_TRUE, function, file, line);
...
/*----------------------------------------------------------------------------*/
void
cpl_test_eq_error_macro(int errnopre,
                        double twallpre,
                        cpl_flops flopspre,
                        cpl_errorstate statepre,
                        cpl_error_code first,
                        const char *first_string,
                        cpl_error_code second,
                        const char *second_string,
                        const char *function,
                        const char *file,
                        unsigned line)
...
{
    char *message =
        cpl_sprintf("(%s) = %d (%s) <=> (%s) = %d (%s) "
                    "<=> %d (%s)",
                    first_string, first, cpl_error_get_message_default(first),
                    second_string, second,
                    cpl_error_get_message_default(second), cpl_error_get_code(),
                    cpl_error_get_message_default(cpl_error_get_code()));

    cpl_test_one(errnopre, twallpre, flopspre, statepre,
                 (first == second && cpl_error_get_code() == first) ? CPL_TRUE
                                                                    : CPL_FALSE,
                 message, CPL_TRUE, function, file, line);
...
/*----------------------------------------------------------------------------*/
void
cpl_test_memory_is_empty_macro(int errnopre,
                               double twallpre,
                               cpl_flops flopspre,
                               cpl_errorstate statepre,
                               const char *function,
...
cpl_test_end(cpl_size nfail)
{
    ...
    const double cputime = (double)buf.tms_utime / (double)sysconf(_SC_CLK_TCK);
    const double systime = (double)buf.tms_stime / (double)sysconf(_SC_CLK_TCK);
    const double chcputime =
        (double)buf.tms_cutime / (double)sysconf(_SC_CLK_TCK);
    const double chsystime =
        (double)buf.tms_cstime / (double)sysconf(_SC_CLK_TCK);
    ...
    if (cpl_test_elap > 0.0) {
        cpl_msg_info(cpl_func, "Actual time to test [s]: %g", cpl_test_elap);
        cpl_msg_info(cpl_func,
                     "The computational speed during this test "
                     "[MFLOP/s]: %g",
                     1e-6 * (double)nflops / cpl_test_elap);
    }
    else {
    ...
    if (errnopre != 0) {
        cpl_msg_warning(cpl_func, "%s() was called with errno=%d: %s", cpl_func,
                        errnopre, strerror(errnopre));
    }

    /* Make sure that the failure is written */
    if (cpl_msg_get_level() == CPL_MSG_OFF)
    ...
    if (!ok) {
        cpl_msg_error(cpl_func, "This failure may indicate a bug in the tested "
                                "code");
        cpl_msg_error(cpl_func,
                      "You can contribute to the improvement of the "
                      "software by emailing the logfile '%s' and the configure "
                      "logfile 'config.log' to %s",
...
/*----------------------------------------------------------------------------*/
static void
cpl_test_reset(cpl_errorstate self)
{
...
/*----------------------------------------------------------------------------*/
static void
cpl_test_one(int errnopre,
             double twallpre,
             cpl_flops flopspre,
             cpl_errorstate statepre,
             cpl_boolean expression,
             const char *message,
             cpl_boolean expect_error,
             const char *function,
             const char *file,
             unsigned line)
{
    const int myerrno = errno; /* Local copy, in case errno changes in here */
    char *errnopre_txt = errnopre == 0
                             ? NULL
                             : cpl_sprintf(" Prior to this test errno=%d: %s.",
                                           errnopre, strerror(errnopre));
    char *myerrno_txt = myerrno == errnopre
                            ? NULL
                            : cpl_sprintf(" This test set errno=%d: %s.",
        ...
        cpl_errorstate_is_equal(cleanstate)
            ...
            : (expect_error
                   ...
                   : (cpl_errorstate_is_equal(statepre)
                          ...
                          : (statepre == cleanstate
                                 ? " CPL error(s) set during this test."
                                 : " CPL error(s) set prior to and during this "
                                   "test.")));
...
#pragma omp master
#endif
    {
        ...
        if (flopspre > cpl_test_flops_prev) {
            ...
            const cpl_flops cpl_test_flops_between =
                flopspre - cpl_test_flops_prev;

            if (cpl_test_time_between > 0.0) {
                flopprev_txt =
                    cpl_sprintf(" (%g FLOPs after the previous "
        ...
        if (cpl_test_flops_now > flopspre) {
            ...
            const cpl_flops cpl_test_flops_during =
                cpl_test_flops_now - flopspre;

            if (cpl_test_time_during > 0.0) {
                floptest_txt =
                    cpl_sprintf(" (%g FLOPs during this test at "
                                "[MFLOP/s]: %g).",
    ...
    if (flopprev_txt == NULL)
        flopprev_txt = cpl_strdup("");
    if (floptest_txt == NULL)
        floptest_txt = cpl_strdup("");
...
#pragma omp atomic
        ...
        (has_error ? cpl_msg_info : cpl_msg_debug)(
            ...
            cpl_test_count, file, line, message, error_msg, errnopre_msg,
            myerrno_msg, flopprev_txt, floptest_txt);
        cpl_errorstate_dump(cleanstate, CPL_FALSE,
                            has_error ? cpl_errorstate_dump_info
                                      : cpl_errorstate_dump_debug);
    }
    else {
        cpl_msg_error(function,
                      "Test %" CPL_SIZE_FORMAT " failed at %s:%u: "
                      "%s.%s%s%s%s%s",
...
/*----------------------------------------------------------------------------*/
static void
cpl_errorstate_dump_debug(unsigned self, unsigned first, unsigned last)
{
...
/*----------------------------------------------------------------------------*/
static void
cpl_errorstate_dump_info(unsigned self, unsigned first, unsigned last)
{
    const cpl_boolean is_reverse = first > last ? CPL_TRUE : CPL_FALSE;
    const unsigned newest = is_reverse ? first : last;
    const unsigned oldest = is_reverse ? last : first;
...
cpl_test_get_description(void)
{
    return "CPL version: " PACKAGE_VERSION
           ...
           "WCSLIB unavailable\n"
                                                    ...
                                                    "version: " CPL_FFTW_VERSION
                                                    "\n"
                                                    "FFTW (single precision) "
                                                    "version:"
                                                    " " CPL_FFTWF_VERSION "\n"
#else
                                                    "FFTW (normal and single "
                                                    "precision) available\n"
#endif
        ...
        CPL_XSTRINGIFY(__STRICT_ANSI__) ": " CPL_STRINGIFY(__STRICT_ANSI__) "\n"
                             ...
                             "gcc version (major number): " CPL_STRINGIFY(__GNUC__) "\n"
                                           ...
                                           "gcc version (minor "
                                           "number): " CPL_STRINGIFY(__GNUC_MINOR__) "\n"
#endif
#ifdef __GNUC_PATCHLEVEL__
                                                               "gcc version "
                                                               "(patch "
                                                               "level):"
                                                               " " CPL_STRINGIFY(__GNUC_PATCHLEVEL__) "\n"
#endif
#ifdef __VERSION__
...
/*----------------------------------------------------------------------------*/
static char *
cpl_test_fits_file(const char *filename, const char *filename_string)
{
    char *self = NULL;
    ...
    if (error) {
        self = cpl_sprintf("%s => %s stat() returned %d: %s", filename_string,
                           filename, error, strerror(errno));
    }
    else {
