...
static const char *cpl_xmemory_type[] = {"free", "malloc", "calloc", "realloc"};
...
/*----------------------------------------------------------------------------*/
inline void *
cpl_xmemory_malloc_count(size_t size)
{
...
/*----------------------------------------------------------------------------*/
inline void *
cpl_xmemory_calloc_count(size_t nmemb, size_t size)
{
...
#pragma omp atomic
        ...
        fprintf(stderr,
                "cpl_xmemory fatal error: calloc(%zu, %zu) returned "
                "NULL:\n",
                nmemb, size);
        perror(NULL);
        cpl_xmemory_fatal = 1;
...
/*----------------------------------------------------------------------------*/
inline void *
cpl_xmemory_realloc_count(void *oldptr, size_t size)
{
    void *ptr;
    ...
    if (oldptr == NULL) {
        ...
        fprintf(stderr,
                "cpl_xmemory error: Ignoring realloc() to %zu bytes on "
        ...
        if (ptr == NULL) {
            if (size != 0) {
                fprintf(stderr,
                        "cpl_xmemory fatal error: realloc(%p, %zu) "
                        "returned NULL:\n",
                        oldptr, size);
                perror(NULL);
                cpl_xmemory_fatal = 1;
...
/*----------------------------------------------------------------------------*/
void
cpl_xmemory_free_count(void *ptr)
{
    if (ptr != NULL) {
#ifndef _OPENMP
        /* Need to disable this non-threadsafe check with thread-support */
        if (cpl_xmemory_ncells == 0) {
            fprintf(stderr,
                    "cpl_xmemory error: Ignoring free() on "
                    "unallocated pointer (%p)\n",
                    ptr);
            return;
        }
...
/*----------------------------------------------------------------------------*/
void *
cpl_xmemory_malloc(size_t size)
{
    void *ptr;
    size_t pos;
...
/*----------------------------------------------------------------------------*/
void *
cpl_xmemory_calloc(size_t nmemb, size_t size)
{
    void *ptr;
    size_t pos;
...
/*----------------------------------------------------------------------------*/
void *
cpl_xmemory_realloc(void *oldptr, size_t size)
{
    void *ptr;
    ...
    if (oldptr != NULL) {
        if (cpl_xmemory_table_size == 0) {
            fprintf(stderr,
                    "cpl_xmemory error: Ignoring realloc() of %zu bytes"
...
cpl_xmemory_free(void *ptr)
{
    ...
    if (cpl_xmemory_ncells == 0) {
        fprintf(stderr,
                "cpl_xmemory error: Ignoring free() on unallocated "
                "pointer (%p)\n",
                ptr);
        return;
    }
...
/*----------------------------------------------------------------------------*/
void
cpl_xmemory_status(int mode)
{
    if (mode > 0) {
        const size_t rowsize =
            sizeof(void *) + sizeof(size_t) + sizeof(unsigned char);
        ...
        if (mode == 2) {
            fprintf(stderr,
                    ...
                    cpl_xmemory_sum_cells, cpl_xmemory_max_cells,
                    cpl_xmemory_alloc_max, cpl_xmemory_table_size_max,
        ...
        if (mode == 2 && cpl_xmemory_ncells > 0) {
            ...
            fprintf(stderr,
                    ...
                    cpl_xmemory_alloc_ram, cpl_xmemory_table_size,
...
/*----------------------------------------------------------------------------*/
static void
cpl_xmemory_init(void)
{
    if (CPL_XMEMORY_RESIZE_THRESHOLD > 1.0) {
        fprintf(
            stderr,
            "cpl_xmemory fatal error: Memory table resize "
            "threshold " CPL_XSTRINGIFY(
                CPL_XMEMORY_RESIZE_THRESHOLD) " must be less "
                                              "than or equal to 1, "
    ...
    if (CPL_XMEMORY_RESIZE_THRESHOLD <= 0.0) {
        fprintf(
            ...
            "cpl_xmemory fatal error: Memory table resize "
            "threshold " CPL_XSTRINGIFY(
                CPL_XMEMORY_RESIZE_THRESHOLD) " must be positive"
                                              ", not " CPL_STRINGIFY(CPL_XMEMORY_RESIZE_THRESHOLD) "\n");
    ...
    if (CPL_XMEMORY_RESIZE_FACTOR <= 1.0) {
        fprintf(
            stderr,
            "cpl_xmemory fatal error: Memory table resize "
            "factor " CPL_XSTRINGIFY(
                CPL_XMEMORY_RESIZE_FACTOR) " must be greater "
...
/*----------------------------------------------------------------------------*/
static void
cpl_xmemory_init_alloc(void)
{
    {
        cpl_xmemory_p_val = calloc(cpl_xmemory_table_size, sizeof(void *));
        cpl_xmemory_p_type =
            calloc(cpl_xmemory_table_size, sizeof(unsigned char));
    ...
    if (cpl_xmemory_p_val == NULL || cpl_xmemory_p_size == NULL ||
        ...
        fprintf(stderr,
                "cpl_xmemory fatal error: calloc() of memory table "
                "failed with size %zu:\n",
...
cpl_xmemory_resize(void)
{
    ...
    if (cpl_xmemory_table_size <= old_size) {
        ...
        fprintf(stderr,
                "cpl_xmemory fatal error: Memory table could not be "
                "resized, %zu < %zu:\n",
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_xmemory_addcell(size_t pos,
                    const void *pointer,
                    size_t size,
                    unsigned char type)
{
...
cpl_xmemory_findcell(const void *ptr)
{
    ...
    /* Locate pointer in main table */
    do {
        if (cpl_xmemory_p_val[ii] == ptr)
            return ii; /* Found */
        if (++ii == cpl_xmemory_table_size)
            ii = 0; /* Wrap around */
    } while (ii != pos);
...
/*----------------------------------------------------------------------------*/
inline static size_t
cpl_xmemory_findfree(const void *ptr)
{
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_xmemory_remcell(size_t pos)
{
    /* Set pointer to NULL */
