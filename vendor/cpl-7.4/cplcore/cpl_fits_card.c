...
typedef struct cpl_fits_value
{
    ...
    int nmemb; /* For string data: number of bytes in buffer
               ...
               */
    /* Buffer for decoded string value with at least one internal quote
       - will use at least 3 chars (from quotes) less than FLEN_VALUE 
       - and no NULL-terminator */
    char unquote[FLEN_VALUE - 4];
 ...
 * @return The created property, or NULL on failure
 * @see _cpl_propertylist_fill_from_fits()
 * @note While a newly created property normally has a NULL-comment, a property
 *       created here has a non-NULL (but possibly zero-length) comment
 *
 */
/*----------------------------------------------------------------------------*/
cpl_property *
cpl_property_new_from_string(const char *cardi,
                             const cpl_memcmp *names,
                             const cpl_regexp *regexp)
{
    char keystr[FLEN_KEYWORD];
    cpl_property *myprop = NULL;
    const char *commentmem = NULL;
    cpl_boolean get_comment = CPL_TRUE;
    cpl_fits_value parseval;
    int compos = 0; /* May be unmodified (on unparsable value) */
    int comlen;
    char type;
    ...
    /* A "HIERARCH " start of a key is skipped, so a key can never be too long */
    cx_assert(keylen < FLEN_KEYWORD);
    ...
    switch (type) {
        ...
        default: {
            ...
            if (errno) {
#ifdef _OPENMP
#pragma omp critical(cpl_strerror)
#endif
                {
                    /* strerror() is not thread safe, but no matter since 
                       creating the error message is not supposed to be
                       performance critical. */
                    const char *errorstr = strerror(errno);

                    (void)cpl_error_set_message_(CPL_ERROR_BAD_FILE_FORMAT,
                                                 "Bad value in FITS card with"
                                                 " key='%*s', '='-pos=%d, "
                                                 "parse-pos=%d, errno=%d: %s",
                                                 keylen, keystr, valinlen + 1,
                                                 compos + 1, errno, errorstr);
                }
                return NULL;
            }
            else if (valinlen < compos && compos < FLEN_CARD - 1) {
                const char badchar[2] = {cardi[compos], '\0'};

                (void)cpl_error_set_message_(CPL_ERROR_BAD_FILE_FORMAT,
                                             "Bad value in FITS card with key="
                                             "'%*s', '='-pos=%d, parse-pos=%d,"
                                             " parse-char=0x%02x (\"%s\")",
                                             keylen, keystr, valinlen + 1,
                                             compos + 1, (int)*badchar,
                                             badchar);
                return NULL;
            }
            else {
...
cpl_fits_fill_card(char *card, const cpl_property *self)
{
    ...
    if (!dovalin) {
        ...
        return cpl_error_set_message_(cpl_error_get_code(),
                                      "Could not set value for key: %s", name);
...
/**
  ...
  @param sz      The number of characters to convert, non-positive does nothing
  @return void
  @note (Almost) no input validation in this internal function
  @see memcpy()
...
{
#ifndef HAVE_FITS_HEADEND_FFPBYT
...
/**
  ...
  @param strval  A buffer of FLEN_VALUE+1 (or more) characters
  @param plen    The number of converted characters, or undefined on error
  @param maxlen  The maximum number of characters allowed, at least 4
  @param self    The property to fill from
                    ...
                    const cpl_property *self)
{
    ...
    switch (type) {
        ...
        case CPL_TYPE_FLOAT: {
            ...
            /* Default CFITSIO float precision: 7, ffpky() */
            *plen = sprintf(retval, "%.7G", value);
            ...
            if (memchr(retval, '.', *plen) == NULL &&
                ...
                if (CPL_UNLIKELY(fptype == FP_NAN || fptype == FP_INFINITE)) {
                    (void)cpl_error_set_message_(CPL_ERROR_ILLEGAL_INPUT,
                                                 "FITS card does not allow "
                                                 "special float: %s",
                                                 retval);
                    return NULL;
                }
        ...
        case CPL_TYPE_DOUBLE: {
            ...
            /* Default CFITSIO double precision: 15, ffpky() */
        ...
        case CPL_TYPE_FLOAT_COMPLEX: {
            ...
            *plen = sprintf(retval, "(%.7G, %.7G)", val2[0], val2[1]);
            ...
            if (CPL_UNLIKELY(fptype0 == FP_NAN || fptype0 == FP_INFINITE ||
                             fptype1 == FP_NAN || fptype1 == FP_INFINITE)) {
                (void)cpl_error_set_message_(CPL_ERROR_ILLEGAL_INPUT,
                                             "FITS card does not allow "
                                             "special float complex: %s",
                                             retval);
                return NULL;
            }
        } break;
        ...
        case CPL_TYPE_CHAR: {
            ...
            cpl_fits_txtcpy(retval + 1 + (int)(c == '\''), &c, 1);
        ...
        case CPL_TYPE_STRING: {
            ...
            const char *qpos;
            /* The size is the string length excl. the null-byte */
            const int valuesize = (int)cpl_property_get_size_(self) - 1;
            int mysz = 0;
            int readpos = 0;
            int qredo = 0;
            /* Need space also for enclosing quotes */
            int remsize = CX_MIN(maxlen - mysz - 2, valuesize - readpos);


            retval[mysz++] = '\'';

            /* Each quote inside a string is escaped by writing the quote
               once as the last chararacter of one substring and then a second
               time as the first chararacter of the subsequent substring.
               E.g. O'HARA is written in two steps:
                  O' and then 'HARA, producing O''HARA
            */
            while (CPL_LIKELY(remsize > 0) &&
                   CPL_UNLIKELY((qpos = memchr(value + readpos, '\'',
                                               remsize)) != NULL)) {
                const int copysz = 1 + qpos - (value + readpos);

                cpl_fits_txtcpy(retval + mysz, value + readpos - qredo,
                                copysz + qredo);

                mysz += copysz + qredo;
                readpos += copysz;
                qredo = 1; /* Write found quote twice to escape it */
                remsize = CX_MIN(maxlen - mysz - 2, valuesize - readpos);
            }

            /* If there is not enough space to write an escaped quote
               at the end of the FITS card, the already copied quote then
               becomes the enclosing end quote of the truncated string */
            if (CPL_LIKELY(maxlen >= mysz + 1 + qredo)) {
                cx_assert(remsize >= 0);
                cpl_fits_txtcpy(retval + mysz, value + readpos - qredo,
                                remsize + qredo);
                mysz += remsize + qredo;

                /* Space pad like CFITSIO and end string */
                remsize = 1 + CX_MAX(0, CX_MIN(maxlen - 2, 9) - mysz);
                (void)memcpy(retval + mysz,
                             (const char *)"        '" + 9 - remsize, remsize);
                mysz += remsize;
            }
    ...
    if (*plen > maxlen) {
        ...
        cx_assert(*plen <= FLEN_VALUE);
        retval[*plen] = '\0'; /* Null-termination cannot overflow buffer */
        (void)cpl_error_set_message_(CPL_ERROR_ILLEGAL_INPUT,
...
cpl_fits_set_value(char *card, int *plen, const cpl_property *self)
{
    ...
    if (*myval == '\'') {
        if (*plen > 8 && *plen + mysz + 2 < FITS_CARD_LEN) {
            /* A non-truncated HIERARCH card gets a space in front of the = */
...
/**
  ...
  @param iparsed   Index from where to start parsing
  ...
  On error, *pjparsed is set to iparsed (nothing was parsed)
  ...
  causes an overflow, the resulting number is deemed float, per the FITS
  standard 4.0, section 4.2.4:
  "If only the integer part is present, the decimal point may be omit-
  ted, in which case the floating-point number is indistinguishable
  from an integer."
                    ...
                    int *pjparsed)
{
    ...
    *pdval = strtod(card + iparsed, &endptr);
    ...
    if (CPL_UNLIKELY(errno != 0)) {
        cx_assert(errno == ERANGE); /* Only possibility from strtod() */
        cx_assert(endptr >= card);

        /* An underflow with a non-zero value is preferable to an error */
        if (-1.0 < *pdval && *pdval < 1.0 && *pdval != 0.0)
            errno = 0;
        else {
            const char *errorstr = strerror(errno);
            ...
            (void)cpl_error_set_message_(
                ...
                "Floating-point over- or under-flow in "
                "FITS card at char-pos=%d->%d, errno=%d: "
                "%s",
                iparsed + 1, (int)(endptr - card), errno, errorstr);
            *pjparsed = iparsed; /* Nothing was parsed */
            return 0;
        }
    }

    if (CPL_UNLIKELY(isalpha(*(endptr - 1)))) {
        /* While strtod() converts the various lower and upper case variations
           of the strings 'NaN' and '+/-/Inf' (that end with a letter) these
           strings are not valid FITS card values. */
        cx_assert(endptr >= card);
        (void)cpl_error_set_message_(CPL_ERROR_BAD_FILE_FORMAT,
                                     "Non-numerical floating-point value in "
                                     "FITS card at char-pos=%d->%d",
                                     iparsed + 1, (int)(endptr - card));
        *pjparsed = iparsed; /* Nothing was parsed */
        return 0;
    }
...
/**
  ...
  while the card itself is. In either case the string is _not_ NULL-terminated,
  instead its length is provided.

  On error, *pjparsed may be unmodified and errno may be set.
                   ...
                   int *pjparsed)
{
    if (card[iparsed] != '=' || cx_string_size_(keywlen) == 0 ||
        ...
        *pjparsed = cx_string_size_(keywlen) ? 8 : 0; /* Card may be blank */
        ...
        /* - skip it and any leading spaces. Rely on null terminator */
        while (card[++iparsed] == ' ')
            ;
        /* A FITS-validator would verify that at least one space was skipped */

        if (CPL_LIKELY(iparsed < FITS_CARD_LEN)) {
            /* card[iparsed] now points to the first value byte */
            int compos = FITS_CARD_LEN; /* Assume no comment */
            ...
            switch (card[iparsed]) {
                case '\'': { /* character string starts with a quote */
                    ...
                    iparsed++;
                    while (CPL_LIKELY((nextquote = memchr(card + iparsed, '\'',
                                                          FITS_CARD_LEN -
                                                              iparsed)) != NULL)
                           /* Rely on null terminator */
                           && CPL_UNLIKELY(nextquote[1] == '\'')) {
                        /* O''HARA -> O'HARA. Strings w. quote(s) are rare */
                        ...
                        /* iparsed updated to point past the 2nd quote */
                        iparsed = 2 + nextquote - card;
                    }

                    if (CPL_UNLIKELY(nextquote == NULL)) {
                        (void)cpl_error_set_message_(CPL_ERROR_BAD_FILE_FORMAT,
                                                     "Bad string value in FITS "
                                                     "card has no end quote. "
                                                     "String start=%d",
                                                     iparsed + 1);
                        *pjparsed = iparsed; /* Invalid string, for error msg */
                        return 0;
                    }

                    if (CPL_UNLIKELY(vallen > 0)) {
                ...
                case '(': {
                    ...
                    for (i = 0; i < 2; i++) {
                       ...
                       since that is all we can store */
                        ...
                        if (CPL_UNLIKELY(ntype != -1)) {
                            (void)cpl_error_set_message_(
                                CPL_ERROR_BAD_FILE_FORMAT,
                                "Bad %s part of complex FITS card value at "
                                "pos=%d",
                                i ? "imaginary" : "real", iparsed + 1);
                            break;
                        }
                        ...
                        /* Rely on null terminator */
                        if (CPL_UNLIKELY(card[iparsed] != dsep[i])) {
                            (void)cpl_error_set_message_(
                                CPL_ERROR_BAD_FILE_FORMAT,
                                "Bad complex FITS card with no %s '%c' at "
                                "pos=%d",
                                i ? "closing" : "separating", dsep[i],
                                iparsed + 1);
                            break;
                        }
                        iparsed++;
                    ...
                    if (CPL_UNLIKELY(i < 2)) {
                        *pjparsed = iparsed; /* Invalid value, for error msg */
                        return 0;
                    }

                    pparseval->val.x = dval[0] + dval[1] * _Complex_I;
                    pparseval->nmemb = 1;
                    /* complex datatype "(1.2, -3.4)" */
                    pparseval->tcode = 'X';
                    break;
                }
                ...
                default: { /* Numerical type */
                    ...
                    if (CPL_UNLIKELY(!ntype)) {
                        *pjparsed = iparsed; /* Invalid number, for error msg */
                        return 0;
                    }
            ...
            /* Parsing for value done. Comment may already have been found
               or card may be exhausted */
            if (CPL_LIKELY(iparsed < compos)) {
                /* A valid card may have a comment preceded by zero, one
                   or more blanks. Look for the comment separator. */

                const char *comchar;
                const int remsize = FITS_CARD_LEN - iparsed;

                assert(iparsed >= 0);
                assert(iparsed < FITS_CARD_LEN);

                comchar = memchr(card + iparsed, '/', remsize);

                if (CPL_LIKELY(comchar != NULL))
                    compos = (int)(comchar - card); /* Start of comment */

                /* Non-comment/-blank directly after the value is not only
                   an error, but it is also unclear how the card should be
                   interpreted so give up */
                if (CPL_UNLIKELY(card[iparsed] != ' ') &&
                    CPL_UNLIKELY(iparsed < compos)) {
                    (void)cpl_error_set_message_(CPL_ERROR_BAD_FILE_FORMAT,
                                                 "Bad FITS card %c-value "
                                                 "with trailing non-blank "
                                                 "at pos=%d",
                                                 pparseval->tcode, iparsed + 1);
                    *pjparsed = iparsed; /* Invalid value, for error msg */
                    return 0;
                }
            }
...
{
    /* The comment is not a comment if it is empty. Rely on null terminator */
    if (card[jparsed++] == '/') {
        ...
        if (jparsed < FITS_CARD_LEN) {
            *plen = FITS_CARD_LEN - jparsed;
            ...
            while (*plen > 0 && card[jparsed + *plen - 1] == ' ') {
                *plen -= 1;
            }
