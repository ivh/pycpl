...
static cpl_imagelist *cpl_imagelist_load_one(const char *,
                                             cpl_type,
                                             cpl_size,
                                             cpl_boolean,
                                             cpl_size,
...
/**
  ...
  Type can be CPL_TYPE_DOUBLE, CPL_TYPE_FLOAT, CPL_TYPE_INT or
...
/*----------------------------------------------------------------------------*/
cpl_imagelist *
cpl_imagelist_load(const char *filename, cpl_type im_type, cpl_size xtnum)
{
    cpl_imagelist *self =
        cpl_imagelist_load_one(filename, im_type, xtnum, CPL_FALSE, 0, 0, 0, 0);
...
/*----------------------------------------------------------------------------*/
cpl_imagelist *
cpl_imagelist_load_window(const char *filename,
                          cpl_type im_type,
                          cpl_size xtnum,
                          cpl_size llx,
                          cpl_size lly,
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_imagelist_get_size(const cpl_imagelist *imlist)
{
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_imagelist_get(cpl_imagelist *imlist, cpl_size inum)
{
...
/*----------------------------------------------------------------------------*/
const cpl_image *
cpl_imagelist_get_const(const cpl_imagelist *imlist, cpl_size inum)
{
    cpl_ensure(imlist != NULL, CPL_ERROR_NULL_INPUT, NULL);
    cpl_ensure(inum >= 0, CPL_ERROR_ILLEGAL_INPUT, NULL);
    cpl_ensure(inum < imlist->ni, CPL_ERROR_ACCESS_OUT_OF_RANGE, NULL);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_imagelist_set(cpl_imagelist *self, cpl_image *im, cpl_size pos)
{
    ...
    if (pos < self->ni && im == self->images[pos])
    ...
    if (pos == self->ni) {
        self->ni++;
        self->images =
            cpl_realloc(self->images, (size_t)self->ni * sizeof(cpl_image *));
    }
    else {
        ...
        for (i = 0; i < self->ni; i++) {
            if (i != pos && self->images[i] == self->images[pos])
                break;
        }
        if (i == self->ni) {
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_imagelist_unset(cpl_imagelist *self, cpl_size pos)
{
    cpl_image *out;
...
/*----------------------------------------------------------------------------*/
void
cpl_imagelist_empty(cpl_imagelist *self)
{
    if (self != NULL) {
...
/*----------------------------------------------------------------------------*/
void
cpl_imagelist_unwrap(cpl_imagelist *self)
{
    if (self != NULL) {
...
/*----------------------------------------------------------------------------*/
void
cpl_imagelist_delete(cpl_imagelist *self)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_imagelist_cast(cpl_imagelist *self,
                   ...
                   cpl_type type)
{
    if (self == NULL) {
        ...
        for (i = j = 0; i < n; i++) {
            ...
            if (cpl_imagelist_set(dest, tmpimg, m + (cpl_size)i))
                break;
...
/*----------------------------------------------------------------------------*/
cpl_imagelist *
cpl_imagelist_duplicate(const cpl_imagelist *imlist)
{
    cpl_imagelist *out;
    cpl_size i;
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_imagelist_erase(cpl_imagelist *imlist, const cpl_vector *valid)
{
    cpl_size nkeep = 0;
    cpl_size i;
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_imagelist_save(const cpl_imagelist *self,
                   const char *filename,
                   cpl_type type,
                   ...
                   unsigned mode)
{
    ...
    return cpl_image_save_((const cpl_image *const *)self->images, self->ni,
                           CPL_TRUE, filename, type, plist, mode)
               ? cpl_error_set_where_()
               : CPL_ERROR_NONE;
}
...
/*----------------------------------------------------------------------------*/
int
cpl_imagelist_is_uniform(const cpl_imagelist *imlist)
{
    cpl_type type;
    cpl_size nx, ny;
    cpl_size i;
    ...
    for (i = 1; i < imlist->ni; i++) {
        if (cpl_image_get_size_x(imlist->images[i]) != nx)
            return (int)i + 1;
        if (cpl_image_get_size_y(imlist->images[i]) != ny)
            return (int)i + 1;
        if (cpl_image_get_type(imlist->images[i]) != type)
            return (int)i + 1;
    }
    return 0;
}
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_imagelist_dump_structure(const cpl_imagelist *self, FILE *stream)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_imagelist_dump_window(const cpl_imagelist *self,
                          cpl_size llx,
                          cpl_size lly,
                          cpl_size urx,
                          cpl_size ury,
                          FILE *stream)
{
    ...
    for (i = 0; i < self->ni; i++) {
        ...
        cpl_ensure_code(fprintf(stream, CPL_IMSG, (int)i, (int)self->ni) >=
                            imsgmin,
                        CPL_ERROR_FILE_IO);
...
cpl_imagelist_load_one(const char *filename,
                       cpl_type im_type,
                       cpl_size xtnum,
                       ...
                       cpl_size ury)
{
    ...
    if (cpl_io_fits_open_diskfile(&fptr, filename, READONLY, &status)) {
        (void)cpl_error_set_fits(
            CPL_ERROR_FILE_IO, status, fits_open_diskfile,
            "filename='%s', im_type=%u, xtnum=%" CPL_SIZE_FORMAT, filename,
            im_type, xtnum);
        return NULL;
    }
    ...
    if (selfsize > 0 && naxis == 3) {
        ...
        for (iplane = 1; iplane < naxes[2]; iplane++) {
            image =
                cpl_image_load_(fptr, &naxis, naxes, &pix_type, filename,
                                iplane, xtnum, do_window, llx, lly, urx, ury);
            if (image == NULL)
                break;
