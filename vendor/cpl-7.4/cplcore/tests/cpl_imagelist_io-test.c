...
static void cpl_imagelist_save_compression_bench(cpl_size,
                                                 cpl_size,
                                                 cpl_size,
                                                 cpl_size);
...
main(void)
{
    ...
    error =
        cpl_imagelist_save(imlist, NULL, CPL_TYPE_FLOAT, NULL, CPL_IO_CREATE);
    cpl_test_eq_error(error, CPL_ERROR_NULL_INPUT);

    error = cpl_imagelist_save(NULL, file, CPL_TYPE_FLOAT, NULL, CPL_IO_CREATE);
    cpl_test_eq_error(error, CPL_ERROR_NULL_INPUT);

    error =
        cpl_imagelist_save(imlist, file, CPL_TYPE_FLOAT, NULL, CPL_IO_CREATE);
    cpl_test_eq_error(error, CPL_ERROR_ILLEGAL_INPUT);
    ...
    cpl_imagelist_dump_window(imlist, (IMAGESZ / 2) - boxsize,
                              (IMAGESZ / 2) - boxsize, (IMAGESZ / 2) + boxsize,
                              (IMAGESZ / 2) + boxsize, stream);
    ...
    error =
        cpl_imagelist_save(imlist2, ".", CPL_TYPE_FLOAT, NULL, CPL_IO_CREATE);
    cpl_test_eq_error(error, CPL_ERROR_FILE_IO);

    remove(file);
    error =
        cpl_imagelist_save(imlist2, file, CPL_TYPE_FLOAT, NULL, CPL_IO_APPEND);
    cpl_test_eq_error(error, CPL_ERROR_FILE_IO);
    ...
    error =
        cpl_imagelist_save(imlist, file, CPL_TYPE_FLOAT, NULL, CPL_IO_EXTEND);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    cpl_test_fits(file);
    ...
    /* Try to read from before CUBE */
    nullimg = cpl_image_load(file, CPL_TYPE_UNSPECIFIED, -1, 0);
    cpl_test_error(CPL_ERROR_BAD_FILE_FORMAT);
    cpl_test_null(nullimg);

    /* Append error test */
    error =
        cpl_imagelist_save(imlist, file, CPL_TYPE_DOUBLE, NULL, CPL_IO_APPEND);
    cpl_test_eq_error(error, CPL_ERROR_ILLEGAL_INPUT);
    ...
    error =
        cpl_imagelist_save(imlist, file, CPL_TYPE_FLOAT, NULL, CPL_IO_APPEND);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    cpl_test_fits(file);
    cpl_test_eq(cpl_fits_count_extensions(file), next);
    ...
    /* Try to read from an invalid window */
    nulllist = cpl_imagelist_load_window(file, CPL_TYPE_UNSPECIFIED, 0,
    ...
    mext = cpl_fits_count_extensions(FILENAME);
    cpl_test_error(CPL_ERROR_NONE);

    plist = cpl_propertylist_new();
    error = cpl_propertylist_save(plist, FILENAME, CPL_IO_EXTEND);
    cpl_test_eq_error(error, CPL_ERROR_NONE);

    /* Try to read an invalid type */
    nulllist = cpl_imagelist_load(FILENAME, CPL_TYPE_DOUBLE_COMPLEX, mext);
    cpl_test_error(CPL_ERROR_INVALID_TYPE);
    cpl_test_null(nulllist);

    /* Try to read from an extension with no data unit */
    nulllist = cpl_imagelist_load(FILENAME, CPL_TYPE_UNSPECIFIED, mext + 1);
    cpl_test_error(CPL_ERROR_DATA_NOT_FOUND);
    cpl_test_null(nulllist);
    /* Try to read from beyond EOF */
    nulllist = cpl_imagelist_load(FILENAME, CPL_TYPE_UNSPECIFIED, mext + 2);
    cpl_test_error(CPL_ERROR_BAD_FILE_FORMAT);
    cpl_test_null(nulllist);
...
/*----------------------------------------------------------------------------*/
static void
cpl_imagelist_save_compression_test(void)
{
        ...
        for (ibpp = 0; ibpp < nbpp; ibpp++) {
            ...
            for (icomp = 0; icomp < ncomp; icomp++) {
                ...
                for (size_t icomp2 = 0; icomp2 < icomp; icomp2++) {
                    ...
                    error =
                        cpl_imagelist_save(imglist, FILENAME, bpps[ibpp], NULL,
                                           CPL_IO_EXTEND | comp_method |
                                               comp_method2);
                    cpl_test_eq_error(error, CPL_ERROR_ILLEGAL_INPUT);
                    ...
                    for (size_t icomp3 = 0; icomp3 < icomp2; icomp3++) {
                        ...
                        error =
                            cpl_imagelist_save(imglist, FILENAME, bpps[ibpp],
                                               NULL,
                                               CPL_IO_EXTEND | comp_method |
                ...
                if (
                    ...
                    img_load =
                        cpl_image_load(FILENAME, CPL_TYPE_UNSPECIFIED, 0, ext);
                    cpl_test_error(CPL_ERROR_NONE);
...
/*----------------------------------------------------------------------------*/
static void
cpl_imagelist_save_compression_bench(cpl_size nx,
                                     cpl_size ny,
                                     cpl_size nz,
                                     cpl_size nr)
{
    ...
    error = cpl_imagelist_save(imglist, FILENAME, CPL_TYPE_UNSPECIFIED, NULL,
                               CPL_IO_EXTEND | CPL_IO_COMPRESS_RICE);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    cpl_test_fits(FILENAME);
...
#pragma omp parallel for private(i) reduction(+ : tsum)
    ...
    for (i = 0; i < nr; i++) {
        ...
        cpl_imagelist *imgload =
            cpl_imagelist_load(FILENAME, CPL_TYPE_UNSPECIFIED, 1);
    ...
    cpl_msg_info(cpl_func,
                 "Time to load Rice-compressed image list %d X %d "
                 "X %d %d times [s]: %g",
                 (int)nx, (int)ny, (int)nz, (int)nr, tsum);
