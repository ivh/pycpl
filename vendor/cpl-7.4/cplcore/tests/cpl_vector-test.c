...
static void cpl_vector_fit_gaussian_test_2(void);
...
main(void)
{
    ...
    cpl_vector_fit_gaussian_test_2();
    ...
    for (k = POLY_SIZE - 1; k > 0; k--) {
        ...
        if (k & 1) {
            cpl_test_zero(
                cpl_vector_add_scalar(tmp_vec2, cpl_vector_get(taylor, k - 1)));
        }
        else {
            cpl_test_zero(
                cpl_vector_subtract_scalar(tmp_vec2,
    ...
    */
    for (k = 1; k < POLY_SIZE; k++) {
        ...
        cpl_test_zero(
            cpl_vector_multiply_scalar(vtmp, cpl_vector_get(taylor, k)));
    ...
    if (!cpl_io_fits_is_enabled()) {
        ...
        if (system("perl -pi -e 'BEGIN{sleep(1)};/NAXIS1/ and s/" CPL_STRINGIFY(VECTOR_SIZE) "/sprintf(\"%d\"," CPL_STRINGIFY(VECTOR_SIZE-1) ")/e' "
                                                                 "cpl_vector_"
                                                                 "save.fits") ==
                   0) {
            tmp_vec2 = cpl_vector_load("cpl_vector_save.fits", 0);
    ...
    if (sizeof(cpl_size) == 4) {
        ...
        if (!cpl_io_fits_is_enabled()) {
            ...
            if (system("perl -pi -e '/NAXIS1/ and s/       " CPL_STRINGIFY(VECTOR_SIZE) "/2200000000/' cpl_vector_save.fits") ==
                       0) {
                tmp_vec2 = cpl_vector_load("cpl_vector_save.fits", 0);
    ...
    cpl_test_leq(cpl_vector_product(sinus, cosinus), DBL_EPSILON * VECTOR_SIZE);
    ...
    error = cpl_vector_fit_gaussian(NULL, NULL, NULL, NULL, CPL_FIT_ALL, NULL,
    ...
    for (i = 1; i < VECTOR_SIZE; i++)
        cpl_test_leq(data[i - 1], data[i]);

    cpl_test_zero(cpl_vector_sort(sinus, CPL_SORT_DESCENDING));
    data = cpl_vector_get_data(sinus);
    cpl_test_nonnull(data);
    for (i = 1; i < VECTOR_SIZE; i++)
        cpl_test_leq(data[i], data[i - 1]);
    ...
    for (i = 0; i < 2 * VECTOR_SIZE; i++)
        data[i] = sin(i * CPL_MATH_2PI / VECTOR_SIZE);
    ...
    */
    ...
    cpl_msg_info("",
                 "Largest cross-correlation rounding error [DBL_EPSILON]: "
                 "%g",
                 emax / DBL_EPSILON);
...
cpl_vector_corr_bench(int n)
{
    ...
    /* Fill the vector cosinus */
    for (i = 0; i < nsize; i++)
        data[i] = cos(i * CPL_MATH_2PI / nsize);
...
#pragma omp parallel for private(i)
    ...
    if (secs > 0.0) {
        cpl_msg_info("",
                     "Speed during %d correlations of size %" CPL_SIZE_FORMAT
                     " in %g secs [Mflop/s]: %g (%g)",
                     n, nsize, secs, flops0 / secs / 1e6, (double)flops0);
...
/*----------------------------------------------------------------------------*/
static void
cpl_vector_get_stdev_bench(int n)
{
    double secs;
    const cpl_size nsize = 10 * VECTOR_SIZE * VECTOR_SIZE;
    cpl_vector *cosinus = cpl_vector_new(nsize);
...
#pragma omp parallel for private(i)
    ...
    if (secs > 0.0) {
        cpl_msg_info(cpl_func,
                     "Speed during %d standard devs of size %" CPL_SIZE_FORMAT
                     "in %g secs [Mflop/s]: %g (%g)",
                     n, nsize, secs, flops0 / secs / 1e6, (double)flops0);
        cpl_msg_info(cpl_func, "Processing rate [MB/s]: %g",
                     1e-6 * (double)bytes / secs);
...
cpl_vector_save_bench(int n)
{
    ...
    for (i = 0; i < n; i++) {
        cpl_test_zero(cpl_vector_save(vec, filename, CPL_TYPE_DOUBLE, qclist,
                                      CPL_IO_CREATE));
    }

    secs = cpl_test_get_cputime() - secs;

    cpl_msg_info(cpl_func, "Time spent saving %d %d-sized vectors [s]: %g", n,
                 nvals, secs);
...
/*----------------------------------------------------------------------------*/
static void
cpl_vector_fit_gaussian_test_one(FILE *stream)
{
    const int N = 50;
    ...
    const double in_sigma = 10.0, in_centre = 25.0, peak = 769.52;

    int n;
    double pos = 0.0, centre, offset, sigma, area, mse, chisq;
    cpl_error_code error;
    ...
    for (n = 0; n < N; n++) {
        ...
        cpl_test_eq_error(error, CPL_ERROR_NONE);
        error =
            cpl_vector_set(yval, n,
                           peak * exp(-d * d / (2.0 * in_sigma * in_sigma)));
        cpl_test_eq_error(error, CPL_ERROR_NONE);
    ...
    error =
        cpl_vector_fit_gaussian(xval, NULL, yval, ysig, CPL_FIT_ALL, &centre,
                                &sigma, &area, &offset, &mse, &chisq, &cov);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
...
/*----------------------------------------------------------------------------*/
/**
  @brief    Reproduce PIPE-10284
  @return   void
 */
/*----------------------------------------------------------------------------*/
static void
cpl_vector_fit_gaussian_test_2(void)
{
    double xdata[] = {4092, 4093, 4094, 4095};
    double ydata[] = {0.176547, 0.628888, 2.50761, 30.4113};
    double ysigd[] = {0.092203, 0.092203, 0.092203, 0.092203};

    cpl_vector *x = cpl_vector_wrap(4, xdata);
    cpl_vector *y = cpl_vector_wrap(4, ydata);
    cpl_vector *sigma_y = cpl_vector_wrap(4, ysigd);

    double x0, sigma, area, offset, mse;
    cpl_matrix *covariance = NULL;

    cpl_error_code code =
        cpl_vector_fit_gaussian(x, NULL, y, sigma_y, CPL_FIT_ALL, &x0, &sigma,
                                &area, &offset, &mse, NULL, NULL);
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code =
        cpl_vector_fit_gaussian(x, NULL, y, sigma_y, CPL_FIT_ALL, &x0, &sigma,
                                &area, &offset, &mse, NULL, &covariance);

    /* The test case is ill-conditioned, so may fail - on some platforms */
    if (code != CPL_ERROR_NONE) {
        cpl_test_eq_error(code, CPL_ERROR_SINGULAR_MATRIX);

        cpl_test_null(covariance);
    }
    else {
        cpl_test_nonnull(covariance);
        cpl_matrix_delete(covariance);
    }

    cpl_vector_unwrap(x);
    cpl_vector_unwrap(y);
    cpl_vector_unwrap(sigma_y);
...
/*----------------------------------------------------------------------------*/
static void
cpl_vector_valarray_bench_one(cpl_vector *a, cpl_vector *b)
{
...
/*----------------------------------------------------------------------------*/
static void
cpl_vector_valarray_bench(cpl_size n)
{
...
/*----------------------------------------------------------------------------*/
static void
cpl_vector_cycle_test(cpl_size n)
{
    const cpl_boolean do_plot =
        cpl_msg_get_level() <= CPL_MSG_INFO ? CPL_TRUE : CPL_FALSE;
    cpl_vector *src;
    cpl_vector *dest;
    cpl_error_code code;
