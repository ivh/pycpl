...
typedef enum _cpl_perm_mode_ cpl_perm_mode;
...
static void cpl_tools_get_median_bench(int, int, cpl_perm_mode);
static void cpl_tools_sort_bench(int, int, cpl_boolean);
static void cpl_tools_test(void);
...
main(int argc, char *argv[])
{
    ...
    if (argc > 1 && argv[1] != NULL && atoi(argv[1]) > 0) {
        const cpl_size n = atoi(argv[1]);
        const cpl_size m = 200000000 / n;

        cpl_tools_get_median_bench(m, n, CPL_SORTED);
        cpl_tools_get_median_bench(m, n, CPL_PARTSORTED);
        cpl_tools_get_median_bench(m, n, CPL_UNSORTED);
    }
    else {
        cpl_tools_test();
    }
...
/**
  @internal
  @brief    Test the functions of this module
  @return Nothing
 */
/*----------------------------------------------------------------------------*/
static void
cpl_tools_test(void)
{
    ...
    /* Find median */
    cpl_test_eq(p15[(NBC2 - 1) / 2],
                cpl_tools_quickselection_int(c15, NBC2, (NBC2 - 1) / 2));
    ...
    /* Normal median case with odd number: 11 element median computation */
    cpl_test_eq_ptr(c15, memcpy(c15, p15, sizeof(p15)));
    ...
    cpl_tools_get_median_bench(1, 5, CPL_UNSORTED);
    cpl_tools_get_median_bench(1, 30, CPL_UNSORTED);
    cpl_tools_get_median_bench(1, 31, CPL_UNSORTED);

    cpl_tools_get_median_bench(1, 5, CPL_PARTSORTED);
    cpl_tools_get_median_bench(1, 30, CPL_PARTSORTED);
    cpl_tools_get_median_bench(1, 31, CPL_PARTSORTED);
...
#endif
        cpl_tools_get_median_bench(5, 25000, CPL_UNSORTED);
        cpl_tools_get_median_bench(5, 25000, CPL_PARTSORTED);
        cpl_tools_get_median_bench(5, 25001, CPL_UNSORTED);
        cpl_tools_get_median_bench(5, 25001, CPL_PARTSORTED);
        cpl_tools_get_median_bench(5, 200000, CPL_UNSORTED);
        cpl_tools_get_median_bench(5, 200001, CPL_UNSORTED);
        cpl_tools_get_median_bench(5, 2000000, CPL_UNSORTED);
        cpl_tools_get_median_bench(5, 2000001, CPL_UNSORTED);
        cpl_tools_get_median_bench(5000000, 8, CPL_UNSORTED);
    }
}
...
/*----------------------------------------------------------------------------*/
static void
cpl_tools_get_kth_quickselection_bench(int m, int n, cpl_boolean do_sortmost)
{
    double tqsel = 0.0;
    double tkth = 0.0;
    int i;
    ...
    cpl_msg_info(cpl_func,
                 "Benchmarked kth <=> QuickSelect%s %d x %d [s]: "
                 "%g <=> %g",
                 do_sortmost ? " (NS)" : "", m, n, tkth, tqsel);
...
/**
  ...
  @param m          The number of repeats
  @param n          The number of elements
  @param perm_mode  -1: Sorted, 0: unsorted: 1: Sort all but three elements
  @return    void
  @see cpl_median-test.c for a more extensive test of the median function.

*/
/*----------------------------------------------------------------------------*/
static void
cpl_tools_get_median_bench(int m, int n, cpl_perm_mode perm_mode)
{
    double tmedian = 0.0;
    const size_t bufsz =
        (sizeof(double) + sizeof(float) + sizeof(int)) * (size_t)n;

    cpl_ifalloc mybuf;
    cpl_ifalloc_set(&mybuf, 2 * bufsz);

    void *original = cpl_ifalloc_get(&mybuf);
    double *dd = (double *)original;
    float *df = (float *)(dd + n);
    int *di = (int *)(df + n);
    void *copy = (void *)(di + n);
    double *cd = (double *)copy;
    float *cf = (float *)(cd + n);
    int *ci = (int *)(cf + n);
    const char *sort_str = "unsorted";

    double sec0;

    if (perm_mode == CPL_UNSORTED) {
        for (int j = 0; j < n; j++) {
            di[j] = (j & 1) ? -j : j;
        }
    }
        ...
        sort_str = "sorted";
        if (perm_mode != CPL_SORTED) {
            di[n / 2 - 1] = di[n / 2];
            di[n / 2] = n;
            di[n / 2 + 1] = -1;
            sort_str = "part-sorted";
        }
    }
    ...
    for (int j = 0; j < m; j++) {
        (void)memcpy(copy, original, bufsz);
    ...
    cpl_msg_info(cpl_func, "Benchmarked %s %d-median X %d [s]: %g (%g)",
                 sort_str, n, m, tmedian, tmedian / 3.0 / m);
}
...
/*----------------------------------------------------------------------------*/
static void
cpl_tools_sort_bench(int m, int n, cpl_boolean do_sortmost)
{
    double tqs = 0.0;
    double tcpl = 0.0;
    int i;
