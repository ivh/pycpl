...
static void
benchmark(cpl_filter_mode filter, cpl_border_mode border_mode, unsigned);

static double rand_gauss(void);
static void test_cpl_image_filter_double(unsigned,
                                         unsigned,
                                         unsigned,
                                         unsigned,
                                         unsigned,
                                         ...
                                         unsigned,
                                         unsigned);
static void test_cpl_image_filter_float(unsigned,
                                        unsigned,
                                        unsigned,
                                        unsigned,
                                        unsigned,
                                        ...
                                        unsigned,
                                        unsigned);
static void test_cpl_image_filter_int(unsigned,
                                      unsigned,
                                      unsigned,
                                      unsigned,
                                      unsigned,
...
static void
test_cpl_image_filter(int, int, int, int, cpl_filter_mode, cpl_border_mode);

#ifdef CPL_FILTER_TEST_AVERAGE_FAST

static void image_filter_average_ref_double(double *,
                                            const double *,
                                            int,
                                            int,
                                            int,
...
static void image_filter_average_ref_float(float *,
                                           const float *,
                                           int,
                                           int,
                                           int,
...
static void
image_filter_average_ref_int(int *, const int *, int, int, int, int, unsigned);

#elif 1

static void image_filter_average_bf_double(double *,
                                           const double *,
                                           int,
                                           int,
                                           int,
...
static void image_filter_average_bf_float(float *,
                                          const float *,
                                          int,
                                          int,
                                          int,
...
static void
image_filter_average_bf_int(int *, const int *, int, int, int, int, unsigned);

#else
static cpl_error_code
filter_average_bf(cpl_image *, const cpl_image *, unsigned, unsigned, unsigned);
...
test_filter(void)
{
    ...
    const cpl_border_mode border_mode[] = {CPL_BORDER_NOP, CPL_BORDER_FILTER,
                                           CPL_BORDER_CROP, CPL_BORDER_COPY};
    const int dopix[] = {1, 2, 3, 4, 7, 8};
    const int dor[] = {0, 1, 2, 3, 7};

    unsigned ifilt;

    for (ifilt = 0; ifilt < sizeof(filter_mode) / sizeof(filter_mode[0]);
         ifilt++) {
        unsigned ibord;

        for (ibord = 0; ibord < sizeof(border_mode) / sizeof(border_mode[0]);
             ibord++) {
            unsigned ix;
            ...
            if ((filter_mode[ifilt] == CPL_FILTER_STDEV ||
                ...
                border_mode[ibord] != CPL_BORDER_FILTER)
                continue;
            ...
            for (ix = 0; ix < sizeof(dopix) / sizeof(dopix[0]); ix++) {
                ...
                for (iy = 0; iy < sizeof(dopix) / sizeof(dopix[0]); iy++) {
                    ...
                    for (irx = 0; irx < sizeof(dor) / sizeof(dor[0]); irx++) {
                        ...
                        for (iry = 0; iry < sizeof(dor) / sizeof(dor[0]);
                             iry++) {
                            const int ry = 2 * dor[iry] + 1 >= ny ? (ny - 1) / 2
                                                                  : dor[iry];

                            if (2 * rx + 1 <= nx && 2 * ry + 1 <= ny) {
...
static void
benchmark(cpl_filter_mode filter,
          cpl_border_mode border_mode,
          unsigned benchsize)
{
    test_cpl_image_filter_float(benchsize, benchsize, 1, 1, filter, border_mode,
                                100, 3);
    test_cpl_image_filter_double(benchsize, benchsize, 1, 1, filter,
                                 border_mode, 100, 3);
    test_cpl_image_filter_int(benchsize, benchsize, 1, 1, filter, border_mode,
                              100, 3);

    test_cpl_image_filter_float(benchsize, benchsize, 2, 2, filter, border_mode,
                                20, 3);
    test_cpl_image_filter_double(benchsize, benchsize, 2, 2, filter,
                                 border_mode, 20, 3);
    test_cpl_image_filter_int(benchsize, benchsize, 2, 2, filter, border_mode,
                              20, 3);

    test_cpl_image_filter_float(benchsize, benchsize, 14, 14, filter,
                                border_mode, 20, 3);
    test_cpl_image_filter_double(benchsize, benchsize, 14, 14, filter,
                                 border_mode, 20, 3);
    test_cpl_image_filter_int(benchsize, benchsize, 14, 14, filter, border_mode,
                              20, 3);
    return;
}
