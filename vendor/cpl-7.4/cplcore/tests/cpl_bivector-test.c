...
main(void)
{
    ...
    const double interpol[POL_SIZE] = {CPL_MATH_LN10, CPL_MATH_E, CPL_MATH_PI};
    int i;
    ...
    cpl_test_vector_abs(cpl_bivector_get_x(tmp_fun), cpl_bivector_get_x(sinus),
                        48.0 * FLT_EPSILON);
    ...
    error =
        cpl_vector_copy(cpl_bivector_get_x(tmp_fun), cpl_bivector_get_x(sinus));
    cpl_test_eq_error(error, CPL_ERROR_NONE);
...
cpl_bivector_sort_ok(cpl_bivector *self,
                     ...
                     int line)
{
    ...
    if (error != CPL_ERROR_NONE) {
        ...
        cpl_vector *tosort = cpl_vector_duplicate(
            (mode == CPL_SORT_BY_X ? cpl_bivector_get_x_const
...
cpl_bivector_sort_test(void)
{
    ...
    error =
        cpl_bivector_sort(toolong, tosort, CPL_SORT_ASCENDING, CPL_SORT_BY_X);
...
/*----------------------------------------------------------------------------*/
static void
cpl_bivector_sort_random(int size)
{
