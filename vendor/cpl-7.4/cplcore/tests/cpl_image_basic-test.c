...
main(void)
{
    ...
    cpl_size nb_cut;
    /* Cyclic "shift" */
    const cpl_size new_pos[9] = {9, 1, 2, 3, 4, 5, 6, 7, 8};
    double err;
    double value;
    int is_rejected;
    cpl_mask *mask;
    ...
    for (i = 1; i <= 2; i++) {
        for (j = 1; j <= 2; j++, value++) {
            ...
            if (value != 0.0)
                cpl_test_abs(vald, 1.0, 0.0);
            if (value != 0.0)
                cpl_test_abs(vali, 1.0, 0.0);
        }
    }
    ...
    for (i = 0; i < IMAGESZ; i++) {
        for (j = 0; j < IMAGESZ; j++) {
            value = sin(i * CPL_MATH_2PI / IMAGESZ) *
                    sin(j * CPL_MATH_2PI / IMAGESZ);
            if (cpl_image_set(imd, i + 1, j + 1, value))
                break;
            if (ddata[i + j * IMAGESZ] != value)
                break;
            if (cpl_image_get(imd, i + 1, j + 1, &is_rejected) != value)
                break;
            if (is_rejected)
                break; /* Not really useful */
        }
    ...
    cpl_test_zero(
        cpl_image_add_scalar(imd2, cpl_vector_get(taylor, POLY_SIZE - 1)));

    for (k = POLY_SIZE - 1; k > 0; k--) {
        cpl_test_zero(cpl_image_multiply(imd2, imd));
        if (k & 1) {
            cpl_test_zero(
                cpl_image_add_scalar(imd2, cpl_vector_get(taylor, k - 1)));
        }
        else {
            cpl_test_zero(
                cpl_image_subtract_scalar(imd2,
    ...
    */
    for (k = 1; k < POLY_SIZE; k++) {
        ...
        cpl_test_zero(
            cpl_image_multiply_scalar(imtmp, cpl_vector_get(taylor, k)));
    ...
    cpl_test_abs(modf(IMAGESZ * IMAGESZ - cpl_image_get_flux(imtmp), &err), 0.0,
                 DBL_EPSILON);
    ...
    error = cpl_image_move(NULL, nb_cut, new_pos);
    cpl_test_eq_error(error, CPL_ERROR_NULL_INPUT);
    error = cpl_image_move(imd1, nb_cut, NULL);
    cpl_test_eq_error(error, CPL_ERROR_NULL_INPUT);
    error = cpl_image_move(imd1, 0, new_pos);
    cpl_test_eq_error(error, CPL_ERROR_ILLEGAL_INPUT);
    error = cpl_image_move(imd1, IMAGESZ + 1, new_pos);
    cpl_test_eq_error(error, CPL_ERROR_ILLEGAL_INPUT);

    imtmp = cpl_image_duplicate(imd1);

    error = cpl_image_move(imd1, nb_cut, &nb_cut);
    cpl_test_eq_error(error, CPL_ERROR_NONE);

    nb_cut = 3;
    error = cpl_image_move(imd1, nb_cut - 1, new_pos);
    cpl_test_eq_error(error, CPL_ERROR_ILLEGAL_INPUT);

    for (k = 0; k < nb_cut * nb_cut; k++) {
        error = cpl_image_move(imd1, nb_cut, new_pos);
        cpl_test_eq_error(error, CPL_ERROR_NONE);
    }
    cpl_test_image_abs(imd1, imtmp, 0.0);
    cpl_image_delete(imd1);
    cpl_image_delete(imtmp);
    ...
    cpl_image_threshold(imi, -DBL_MAX, (double)MAGIC_PIXEL, 0.0,
                        (double)MAGIC_PIXEL);
    cpl_image_threshold(imf, -DBL_MAX, (double)MAGIC_PIXEL, 0.0,
                        (double)MAGIC_PIXEL);
    cpl_image_threshold(imd, -DBL_MAX, (double)MAGIC_PIXEL, 0.0,
                        (double)MAGIC_PIXEL);
    ...
    imtmp = cpl_image_extract(imi, IMAGESZ / 4, IMAGESZ / 4, IMAGESZ / 2,
                              IMAGESZ / 2);
    cpl_test_nonnull(imtmp);
    cpl_image_delete(imi);
    imi = imtmp;
    imtmp = NULL;
    imtmp = cpl_image_extract(imf, IMAGESZ / 4, IMAGESZ / 4, IMAGESZ / 2,
                              IMAGESZ / 2);
    cpl_test_nonnull(imtmp);
    cpl_image_delete(imf);
    imf = imtmp;
    imtmp = NULL;
    imtmp = cpl_image_extract(imd, IMAGESZ / 4, IMAGESZ / 4, IMAGESZ / 2,
                              IMAGESZ / 2);
    cpl_test_nonnull(imtmp);
    cpl_image_delete(imd);
    imd = imtmp;
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 2; j++) {
            if (i == 0)
                cpl_image_set(imtmp, i + 1, j + 1, 2.0);
...
static void
cpl_image_complex_mult(size_t o1, size_t o2, size_t n)
{
    ...
    cpl_msg_info(cpl_func,
                 "Float complex vector multiply, "
                 "size: %zu, offsets: %zu %zu",
                 n * n, o1, o2);
...
static void
cpl_image_complex_mult_d(size_t o1, size_t o2, size_t n)
{
    /* test multiplication, compares vector version with scalar version */
    const size_t nl = n + 2;
    ...
    cpl_msg_info(cpl_func,
                 "Double complex vector multiply, "
                 "size: %zu, offsets: %zu %zu",
                 n * n, o1, o2);
...
cpl_image_copy_test(void)
{
    ...
    cpl_test_abs(cpl_image_get_flux_window(im1, 1, 1, 1, IMAGESZ), IMAGESZ,
                 0.0);
    cpl_test_abs(cpl_image_get_flux_window(im1, 2, 1, 2, IMAGESZ), IMAGESZ,
                 0.0);
    cpl_test_abs(cpl_image_get_flux_window(im1, 3, 1, IMAGESZ, IMAGESZ), 0.0,
                 0.0);
    ...
    cpl_test_abs(cpl_image_get_flux(im1), 2.0 * IMAGESZ, 0.0);
    cpl_test_abs(cpl_image_get_flux_window(im1, 1, 1, IMAGESZ, 1), IMAGESZ,
                 0.0);
    cpl_test_abs(cpl_image_get_flux_window(im1, 1, 2, IMAGESZ, 2), IMAGESZ,
                 0.0);
    cpl_test_abs(cpl_image_get_flux_window(im1, 1, 3, IMAGESZ, IMAGESZ), 0.0,
                 0.0);
    ...
    for (itype = 0; itype < sizeof(ttype) / sizeof(ttype[0]); itype++) {
        ...
        for (ipix = 0; ipix < sizeof(npix) / sizeof(npix[0]); ipix++) {
            ...
            for (jpix = 0; jpix < sizeof(npix) / sizeof(npix[0]); jpix++) {
                ...
                error =
                    cpl_image_fill_noise_uniform(raw, (double)-ny, (double)nx);
                cpl_test_eq_error(error, CPL_ERROR_NONE);

                /* About 1/4 of the pixels are flagged */
                bpm =
                    cpl_mask_threshold_image_create(raw, -0.25 * ny, 0.25 * nx);
...
/*----------------------------------------------------------------------------*/
static void
cpl_image_collapse_median_test(void)
{
    const int mx = IMAGESZ | 1;
    const int my = IMAGESZ / 2 | 1;
    const cpl_type types[] = {CPL_TYPE_DOUBLE_COMPLEX, CPL_TYPE_FLOAT_COMPLEX};
    ...
    for (ityp = 0; ityp < (int)(sizeof(imtypes) / sizeof(imtypes[0])); ityp++) {
        ...
        for (idrop = 0; idrop * 2 < 5; idrop++) {
            ...
            error = cpl_image_filter_mask(filt, img, mask, CPL_FILTER_MEDIAN,
                                          CPL_BORDER_CROP);
            cpl_test_eq_error(error, CPL_ERROR_NONE);
            ...
            for (jdrop = 0; jdrop <= 2 * idrop; jdrop++) {
                cpl_image *med =
                    cpl_image_collapse_median_create(img, 0, jdrop,
                                                     2 * idrop - jdrop);
                cpl_image *img1d =
                    cpl_image_extract(filt, 1, 1 + jdrop, mx, 1 + jdrop);
            ...
            for (jdrop = 0; jdrop <= 2 * idrop; jdrop++) {
                cpl_image *med =
                    cpl_image_collapse_median_create(img, 1, jdrop,
                                                     2 * idrop - jdrop);
                cpl_image *img1d =
                    cpl_image_extract(filt, 1 + jdrop, 1, 1 + jdrop, my);
                int k, is_bad, is_bad2;
...
/*----------------------------------------------------------------------------*/
static void
cpl_image_flip_turn_test(const cpl_image *self)
{
    cpl_error_code error;
...
/*----------------------------------------------------------------------------*/
static void
cpl_image_get_fwhm_test(void)
{
    /* Test case for DFS12346 */
    const double x[] = {
        -13.9055, -2.24811, -4.60207, 12.0745, -9.29113, 19.4861,
        17.6999,  32.2417,  68.0725,  120.824, 172.361,  242.671,
        302.048,  355.937,  348.458,  309.819, 240.489,  179.107,
        117.223,  66.3061,  32.7849,  22.9367,
    };
...
/*----------------------------------------------------------------------------*/
static void
cpl_image_divide_test_one(void)
{
    ...
    for (int i1type = 0; i1type < 2; i1type++) {
        for (int i2type = 0; i2type < 2; i2type++) {
            for (int i3type = 0; i3type < 2; i3type++) {
                const double tol = htype[i1type] == CPL_TYPE_FLOAT ||
                ...
                cpl_image *img1 =
                    cpl_image_new(IMAGESZ, IMAGESZ, htype[i1type]);
                cpl_image *img2 =
                    cpl_image_new(IMAGESZ, IMAGESZ, htype[i2type]);
                cpl_image *img3 =
                    cpl_image_new(IMAGESZ, IMAGESZ, htype[i3type]);
                cpl_image *img1copy;
                cpl_image *img2copy;
                cpl_error_code code;
                ...
                img3 = cpl_image_new(IMAGESZ + 1, IMAGESZ,
                                     htype[i1type] | CPL_TYPE_COMPLEX);
...
/*----------------------------------------------------------------------------*/
static void
cpl_image_bitwise_test(cpl_size nx, cpl_size ny)
{
    cpl_error_code (*fimage[3])(cpl_image *, const cpl_image *,
                                const cpl_image *) = {cpl_image_and,
                                                      cpl_image_or,
                                                      cpl_image_xor};
    cpl_error_code (*fscalar[3])(cpl_image *, const cpl_image *,
                                 cpl_bitmask) = {cpl_image_and_scalar,
                                                 cpl_image_or_scalar,
    ...
    code = cpl_image_fill_noise_uniform(im1, -cpl_tools_ipow(2.0, 31),
...
/*----------------------------------------------------------------------------*/
static void
cpl_image_divide_create_test(void)
{
    ...
    if (a == NULL || b == NULL || data_a == NULL || data_b == NULL)
        return;
