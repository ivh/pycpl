...
/* Filename for saving that is expected to fail */
#define FAILFILE BASE CPL_STRINGIFY(-fail) ".fits"

#define KEYBASE  "ESO KEY"

/* Invalid FITS headers needed for testing cannot be writted by CPL.
   So write the valid FITS file and invalidate it without CPL. */
#define RMHIERARCHVALQUOTES              \
    "perl -pi -e 'BEGIN{$/=\\80;};' -e " \
    "'s/^(HIERARCH [^\\047]+= +)\\047([^\\047]*)\\047/$1 $2 /;' "

#define SETQQUOTE " -e 's/Q/\\047/g;' "

#define KEYFORM   KEYBASE "%05d"

#if FLEN_KEYWORD == 72
#define CPL_KEY_EXTRA ""
#else
/* CFITSIO 3.38 increases FLEN_KEYWORD to 75 */
#define CPL_KEY_EXTRA "234"
#endif
...
#define CPL_FITS_IGNORE                                            \
    "SIMPLE|BSCALE|BZERO|EXTEND|WCSAXES"                           \
    "|XTENSION|BITPIX|NAXIS[0-9]*|PCOUNT|GCOUNT|DATE"              \
    "|CRPIX[0-9]+|CRVAL[0-9]+|CDELT[0-9]+|CRDER[0-9]+|CSYER[0-9]+" \
    "|TCRVL[0-9]+|TCRPX[0-9]+|TCTYP[0-9]+|TCUNI[0-9]+"             \
    "|TFIELDS|TBCOL[0-9]+|TFORM[0-9]+|TTYPE[0-9]+|TUNIT[0-9]+"     \
    "|TSCAL[0-9]+|TZERO[0-9]+|TNULL[0-9]+|TDISP[0-9]+|TDIM[0-9]+"  \
    "|TCDLT[0-9]+|PC[0-9]+_[0-9]+|PV[0-9]+_[0-9]+"                 \
    "|TDMIN[0-9]+|TDMAX[0-9]+|TLMIN[0-9]+|TLMAX[0-9]+"             \
    "|COMMENT|HISTORY|MJD-OBS|PSCAL[0-9]+|PZERO[0-9]+|PTYPE[0-9]+"

#define CPL_FITS_IGNORE_PRIM        \
    "^(" CPL_FITS_IGNORE "|BLOCKED" \
    ")$"

#define CPL_FITS_IGNORE_EXT "^(" CPL_FITS_IGNORE "|ORIGIN[0-9]*)$"

#define CPL_FITS_COMPRKEYS                           \
    "^("                                             \
    "ZIMAGE|ZCMPTYPE|ZNAXIS"                         \
    "|ZTENSION|ZPCOUNT|ZGCOUNT|ZNAME[0-9]|ZVAL[0-9]" \
    "|ZTILE[0-9]|ZBITPIX|ZNAXIS[0-9]|ZSCALE|ZZERO"   \
    "|ZBLANK|ZHECKSUM|ZDATASUM"                      \
    ")$"

/* The (minimal) length of a complete ISO-8601 date w. separators */
#define ISO8601_LEN     19
#define ISO8601_EXAMPLE "2000-02-29T23:59:59.999"
...
static int
cpl_test_property_compare_name(const cpl_property *, const cpl_property *);
...
static void cpl_propertylist_test_load_bad(void);

static void cpl_propertylist_test_load_bad_key(void);
static void cpl_propertylist_test_too_long_key(void);

static void cpl_propertylist_test_full_card(void);
static void cpl_propertylist_test_too_long_card(void);
static void cpl_propertylist_test_underflow(void);

static void cpl_propertylist_compare(const cpl_propertylist *,
                                     const cpl_propertylist *) CPL_ATTR_NONNULL;

static void
cpl_propertylist_test_file(const char *, cpl_size *) CPL_ATTR_NONNULL;

static void cpl_propertylist_test_local(void);

static cpl_boolean
cpl_propertylist_test_fits_names(const cpl_propertylist *) CPL_ATTR_NONNULL;
static char *cpl_toupper(const char *) CPL_ATTR_NONNULL CPL_ATTR_ALLOC;
/*-----------------------------------------------------------------------------
                                  Main
 -----------------------------------------------------------------------------*/
...
/**
  @internal
  @brief    Unit tests, optionally with CLI-provided files for I/O-testing
  @param    argc   Number of CLI strings
  @param    argv   Array of CLI strings
  @return zero iff the tests were OK
  @note The testing with CLI-provided files is experimental and unsupported
    ...
    if (argc == 1) {
        ...
        if (nonposix_locale == (locale_t)0) {
            cpl_msg_warning(cpl_func,
                            "Skipping test with unavailable "
            ...
            cpl_msg_info(cpl_func,
                         "Testing with non-POSIX locale (float: "
                         "%g): " CPL_NON_POSIX_LOCALE,
                         1.23);
        ...
        cpl_msg_info(cpl_func, "Testing with default locale (float: %g)", 1.23);
#endif
        cpl_propertylist_test_local();
    }

    /* Test the files provided via the CLI (incl. the excutable itself...) */
...
static void
cpl_propertylist_test_local(void)
{
    ...
    cpl_type types[] = {CPL_TYPE_CHAR,          CPL_TYPE_BOOL,
                        CPL_TYPE_INT,           CPL_TYPE_LONG,
                        CPL_TYPE_FLOAT,         CPL_TYPE_DOUBLE,
                        CPL_TYPE_STRING,        CPL_TYPE_FLOAT_COMPLEX,
    ...
    const cpl_msg_severity pre_level = cpl_msg_get_level();
    int bench_size;
    const char *strval;
    int syscode;
...
#define MYFILE FILENAME(MYTEST)
    ...
    _plist = cpl_propertylist_load(MYFILE, 0);
    cpl_test_error(CPL_ERROR_NONE);
    ...
    for (i = 0; i < nprops; i++) {
        ...
        if (types[i] == CPL_TYPE_CHAR) {
            ...
            cpl_test_eq(cpl_property_get_string(p2)[0],
                        cpl_property_get_char(p1));
        }
    ...
    cpl_test_eq(cpl_propertylist_insert_char(plist, keys[0], keys[0 + nprops],
                                             'a'),
                0);
    ...
    cpl_test_eq(cpl_propertylist_insert_bool(plist, keys[1], keys[1 + nprops],
                                             0),
                0);
    ...
    cpl_test_eq(cpl_propertylist_insert_int(plist, keys[2], keys[2 + nprops],
                                            0),
                0);
    ...
    cpl_test_eq(cpl_propertylist_insert_string(plist, keys[6], keys[6 + nprops],
                                               ""),
                0);
...
#define MYFILE FILENAME(MYTEST)
    ...
    code =
        cpl_propertylist_update_string(plist2, "ESO OBS PI-COI NAME", "NAME");
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code =
        cpl_propertylist_set_comment(plist2, "TOFITS", "Test TOFITS function");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    (void)remove(MYFILE);
    status = 0;

    fits_create_diskfile(&fptr, MYFILE, &status);
    ...
    */
    ...
    code = cpl_propertylist_to_fitsfile(fptr, plist2, NULL, CPL_FALSE);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    */
    ...
    status = 0;
    fits_open_diskfile(&_fptr, MYFILE, READONLY, &status);
    cpl_test_zero(status);
    ...
    */
    ...
    plist = cpl_propertylist_load_regexp(MYFILE, 0, "^ESO .*", 0);
    cpl_test_error(CPL_ERROR_NONE);
    cpl_test_nonnull(plist);
    cpl_test_zero(cpl_propertylist_is_empty(plist));
    ...
    value = cpl_fits_count_extensions(MYFILE);
    ...
    nulllist = cpl_propertylist_load_regexp(MYFILE, 0, NULL, 0);
    cpl_test_error(CPL_ERROR_NULL_INPUT);
    cpl_test_null(nulllist);
    ...
    nulllist = cpl_propertylist_load(MYFILE, -1);
    cpl_test_error(CPL_ERROR_ILLEGAL_INPUT);
    cpl_test_null(nulllist);

    nulllist = cpl_propertylist_load_regexp(MYFILE, -1, ".", 0);
    cpl_test_error(CPL_ERROR_ILLEGAL_INPUT);
    cpl_test_null(nulllist);
    ...
    nulllist = cpl_propertylist_load(MYFILE, value + 1);
    cpl_test_error(CPL_ERROR_DATA_NOT_FOUND);
    cpl_test_null(nulllist);

    nulllist = cpl_propertylist_load_regexp(MYFILE, value + 1, ".", 0);
    cpl_test_error(CPL_ERROR_DATA_NOT_FOUND);
    cpl_test_null(nulllist);

    nulllist = cpl_propertylist_load_regexp(MYFILE, 0, "\\", 0);
    cpl_test_error(CPL_ERROR_ILLEGAL_INPUT);
    cpl_test_null(nulllist);
...
#define MYFILE FILENAME(MYTEST)
    ...
    /* These two will be written, but as commentary cards,
       so they cannot be re-read with their original type,
       and the FITS value indicator and quote-encoding remains... */
    cpl_propertylist_append_char(plist, "HISTORY", '\'');
    cpl_propertylist_append_char(plist, "COMMENT", '\'');
    cpl_test_error(CPL_ERROR_NONE);

    t = cpl_table_new(1);
    code = cpl_table_save(t, plist, NULL, MYFILE, CPL_IO_CREATE);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    cpl_test_fits(MYFILE);

    /* Two additional COMMENT cards will be added, so count only HISTORY */
    plist2 = cpl_propertylist_load_regexp(MYFILE, 0, "HISTORY", 0);
    cpl_test_error(CPL_ERROR_NONE);
    cpl_test_nonnull(plist2);
    ...
    if (plist == NULL) {
        ...
        code = cpl_image_save(NULL, BASE "_sorted.fits", CPL_TYPE_UCHAR, plist,
                              CPL_IO_CREATE);
        cpl_test_eq_error(code, CPL_ERROR_NONE);
...
#define MYFILE FILENAME(MYTEST)
    ...
    cpl_msg_set_level(CPL_MSG_ERROR); /* Suppress lots of expected warnings */
    code = cpl_propertylist_to_fitsfile(fptr, plist, NULL, CPL_FALSE);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    cpl_msg_set_level(pre_level);
...
#define MYFILE FILENAME(MYTEST)
    ...
    code = cpl_propertylist_save(plist, MYFILE, CPL_IO_APPEND);
    cpl_test_eq_error(code, CPL_ERROR_ILLEGAL_INPUT);
    cpl_test_fits(MYFILE);

    cpl_propertylist_update_string(plist, keys[6], "updated string");
    code = cpl_propertylist_save(plist, MYFILE, CPL_IO_EXTEND);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    cpl_test_fits(MYFILE);
...
#define MYFILE FILENAME(MYTEST)
    ...
    */
    ...
    cpl_propertylist_append_string(plist, "HIERARCH ESO TEST1",
                                   "One string without any comment");
    cpl_propertylist_append_string(plist, "HIERARCH ESO TEST2",
                                   "Two string without any comment");
    ...
    cpl_propertylist_append_string(plist, "HISTORY",
                                   "One history string without any comment");
    cpl_propertylist_append_string(plist, "HISTORY",
                                   "Two history string without any comment");
    cpl_propertylist_append_string(plist, "HISTORY",
                                   "Three history string without any comment");
    ...
    */
    ...
    code = cpl_propertylist_to_fitsfile(fptr, plist, ")|(", CPL_FALSE);
    cpl_test_eq_error(code, CPL_ERROR_ILLEGAL_INPUT);

    code = cpl_propertylist_to_fitsfile(fptr, plist, to_rm, CPL_FALSE);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code = cpl_propertylist_to_fitsfile(fptr, NULL, to_rm, CPL_FALSE);
    cpl_test_eq_error(code, CPL_ERROR_NULL_INPUT);

    code = cpl_propertylist_to_fitsfile(NULL, plist, to_rm, CPL_FALSE);
    cpl_test_eq_error(code, CPL_ERROR_NULL_INPUT);
    ...
    plist = cpl_propertylist_load(MYFILE, 0);
    cpl_test_error(CPL_ERROR_NONE);
    cpl_test_nonnull(plist);
...
#define MYFILE FILENAME(MYTEST)
    ...
    /* extension header */
    plist2 = cpl_propertylist_load(MYFILE, 1);
    ...
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    cpl_test_fits(MYFILE);
    code = cpl_table_save(t, NULL, plist, MYFILE, CPL_IO_EXTEND);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    cpl_test_fits(MYFILE);
    cpl_propertylist_delete(plist);

    /* Check if compressed keywords were removed from headers*/
    plist = cpl_propertylist_load(MYFILE, 1);
    ...
    plist = cpl_propertylist_load(MYFILE, 2);
    cpl_test_zero(cpl_propertylist_has(plist, "ZTENSION"));
    cpl_test_zero(cpl_propertylist_has(plist, "ZNAXIS"));
    cpl_test_zero(cpl_propertylist_has(plist, "ZNAXIS1"));
    cpl_test_zero(cpl_propertylist_has(plist, "ZNAXIS2"));
...
#define MYFILE FILENAME(MYTEST)
    ...
    cpl_test_eq(cpl_propertylist_get_size(plist), 6);
    cpl_test_eq(cpl_propertylist_get_int(plist, "HIERARCH ESO TEST1 APPEND"),
                3);
...
#define MYFILE FILENAME(MYTEST)
    ...
    cpl_test_zero(cpl_propertylist_save(plist, MYFILE, CPL_IO_CREATE));
    cpl_test_fits(MYFILE);
...
#define MYFILE FILENAME(MYTEST)
    ...
    code =
        cpl_propertylist_insert_string(plist, keys[6], keys[6 + nprops], NULL);
    cpl_test_eq_error(CPL_ERROR_NULL_INPUT, code);
...
#define MYFILE FILENAME(MYTEST)
    ...
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    code = cpl_propertylist_set_comment(plist, "DELKEY1",
                                        "Undefined value"
    ...
    /* The below keys are at their maximum length,  */
    code = cpl_propertylist_append_double(plist,
                                          "HIERARCH ESO "
    ...
    dval1 =
        cpl_propertylist_get_double(plist, "HIERARCH ESO "
                                           "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                           "ABCDEFGHIJ ABCDEFGHIJ AB");
    cpl_test_error(CPL_ERROR_NONE);
    ...
    cpl_propertylist_empty(plist);
    /* This key exceeds the maximum length by 1  */
    code = cpl_propertylist_append_double(plist,
                                          "HIERARCH ESO "
                                          "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
    ...
    code = cpl_propertylist_save(plist, FAILFILE, CPL_IO_CREATE);
    cpl_test_eq_error(code, CPL_ERROR_ILLEGAL_INPUT);
    ...
    for (int isign = -1; isign < 2; isign++) {
        ...
        /* This floating point value cannot be saved */
        code =
            cpl_propertylist_append_double_complex(plist, "BADFLOAT", badval);
        cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    cpl_propertylist_test_underflow();

    cpl_propertylist_delete(plist);
    cpl_propertylist_delete(plist2);

    cpl_propertylist_test_numeric_load_bad();
    cpl_propertylist_test_load_bad();
    cpl_propertylist_test_load_bad_key();
    cpl_propertylist_test_too_long_key();
    cpl_propertylist_test_full_card();
    cpl_propertylist_test_too_long_card();
...
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_compare(const cpl_propertylist *plist1,
                         const cpl_propertylist *plist2)
{
    ...
    for (cpl_size j = 0; j < sz1; j++) {
        ...
        char *nameup = cpl_toupper(name);

        if (strcmp(name, nameup)) {
            cpl_msg_warning(cpl_func, "Lower-case key: %s <=> %s", name,
                            nameup);
            if (p2 == NULL) {
                p2 = cpl_propertylist_get_property_const(plist2, nameup);
            }
        }
        cpl_free(nameup);
        ...
        if (p2 != NULL) {
            cpl_size ninstance = 1;
            if (sz1 > sz2) {
                const cpl_cstr *exactkey[1] = {
                    CXSTR(name, cpl_property_get_size_name(p1))};
                if (cpl_propertylist_copy_name_(pcopy, plist1, 0, NULL, 1,
                                                exactkey, 1)) {
                    cpl_test_error(CPL_ERROR_NONE);
                }
                else {
                    ninstance = sz1 - cpl_propertylist_get_size(pcopy);
                }
                cpl_propertylist_empty(pcopy);
            }
            else if (sz2 > sz1) {
                const cpl_cstr *exactkey[1] = {
                    CXSTR(name, cpl_property_get_size_name(p2))};
                if (cpl_propertylist_copy_name_(pcopy, plist2, 0, NULL, 1,
                                                exactkey, 1)) {
                    cpl_test_error(CPL_ERROR_NONE);
                }
                else {
                    ninstance = sz2 - cpl_propertylist_get_size(pcopy);
                }
                cpl_propertylist_empty(pcopy);
            }
            cpl_property_eq(p1, p2, ninstance);
        }

        if (cpl_test_get_failed() != nfailed) {
            cpl_msg_warning(cpl_func, "FAILED(%d/%d<=>%d): %s", (int)j,
                            (int)sz1, (int)sz2, name);
        }
    }
...
static void
cpl_property_eq(const cpl_property *self,
                ...
                cpl_size ninstance)
{
    ...
    if (cpl_test_get_failed() == nfailed) {
        ...
        if (strcmp(name1, name2)) {
            name1up = cpl_toupper(name1);
            name2up = cpl_toupper(name2);
            if (!strcmp(name1up, name2up)) {
                cpl_msg_warning(cpl_func, "Lower-case key: %s <=> %s", name1,
                                name2);
                name1 = name1up;
                name2 = name2up;
            }
        ...
        if (cpl_test_get_failed() == nfailed) {
            ...
            if (ninstance != 1) {
                cpl_msg_warning(cpl_func,
                                "Skipping compare of %d-instance, "
                                "%s/%s-type key: %s",
                                (int)ninstance, cpl_type_get_name(t1),
                                cpl_type_get_name(t2), name1);
            }
            else if (cpl_test_get_failed() == nfailed) {
                switch (t1) {
                    case CPL_TYPE_BOOL: {
                    ...
                    case CPL_TYPE_FLOAT: {
                        /* Can fail if the floating-point value is written
                           with more significant digits than the machine
                           precision supports. */
                    ...
                    case CPL_TYPE_DOUBLE: {
                        ...
                        cpl_test_rel(v1, v2, DBL_EPSILON * 24.0);
                        break;
                    }
                    case CPL_TYPE_STRING: {
...
/*----------------------------------------------------------------------------*/
static int
cpl_test_property_compare_name(const cpl_property *p1, const cpl_property *p2)
{
    /*
    * Compare the two properties
    */
    return strcmp(cpl_property_get_name(p1), cpl_property_get_name(p2));
}
...
/**
  @internal
  @brief    Load, save and reload the headers of the named file, and compare

  The originally and the re-loaded propertylists are compared.
  They should ideally be identical, but the check may fail because:
  1) Floating point values are encoded with more significant digts than the
     machine precision can handle.
  2) Non-unique, non-commentary cards of which only one is saved

  Invalid or non-FITS files are supported to provide fuzz-testing of
  cpl_propertylist_load() - and if meaningful of cpl_fits_count_extensions().
 

 */
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_file(const char *filename, cpl_size *pmaxsz)
{
    const cpl_errorstate prevstate = cpl_errorstate_get();
    cpl_size nhdu = 1; /* Assume 1 HDU */
    cpl_size jhdu = 0; /* May write fewer headers than those read */

    for (cpl_size ihdu = 0; ihdu < nhdu; ihdu++) {
        const cxchar *badkeys = ihdu ? /* Copied from cpl_propertylist_save() */
                                    CPL_FITS_IGNORE_EXT "|" CPL_FITS_COMPRKEYS
                                     : CPL_FITS_IGNORE_PRIM
                                    "|" CPL_FITS_COMPRKEYS;

        /* Test whether the header can be loaded */
        cpl_propertylist *plist1 = cpl_propertylist_load(filename, ihdu);
        cpl_propertylist *plist2;
        cpl_size sz1, sz2, mfailed;
        cpl_error_code code;
        cpl_boolean key_valid;
        cpl_size nfail;


        if (plist1 == NULL) {
            cpl_error_code cexpect = CPL_ERROR_BAD_FILE_FORMAT;

            if (ihdu == 0) {
                FILE *ffail = fopen(filename, "r");

                if (ffail) {
                    (void)fclose(ffail);
                }
                else {
                    cexpect = CPL_ERROR_FILE_IO;
                }
            }
            cpl_msg_warning(cpl_func, "Skipping tests of %s file: %s",
                            cexpect == CPL_ERROR_BAD_FILE_FORMAT ? "non-FITS"
                                                                 : "unreadable",
                            filename);
            cpl_errorstate_dump(prevstate, CPL_FALSE,
                                cpl_errorstate_dump_one_info);
            cpl_test_error(cexpect);
            continue;
        }

        if (ihdu == 0) {
            nhdu = 1 + cpl_fits_count_extensions(filename);

            if (cpl_error_get_code()) { /* File is (presumably) invalid... */
                cpl_msg_warning(cpl_func,
                                "Limiting tests of file with invalid FITS "
                                "extension(s) to primary header: %s",
                                filename);
                ...
                cpl_msg_info(cpl_func, "Testing %d header(s) from %s",
                             (int)nhdu, filename);
        ...
        key_valid = cpl_propertylist_test_fits_names(plist1);
        cpl_test_error(CPL_ERROR_NONE);

        sz1 = cpl_propertylist_get_size(plist1);
        cpl_test_error(CPL_ERROR_NONE);
        if (key_valid && sz1 > *pmaxsz) {
            cpl_msg_info(cpl_func,
                         "Found new largest header: %d > %d cards "
                         "in extension %d in %s",
                         (int)sz1, (int)*pmaxsz, (int)ihdu, filename);
            *pmaxsz = sz1;
        }

        /* Test that the header can be saved */
        code = cpl_propertylist_save(plist1, FILENAME(00),
                                     jhdu ? CPL_IO_EXTEND : CPL_IO_CREATE);
        if (code) { /* File is (presumably) invalid... */
            if (key_valid) {
                cpl_test_eq_error(code, cpl_error_get_code());
                cpl_msg_warning(cpl_func,
                                "Skipping further tests of unsavable "
                                "FITS header %d/%d: %s",
                                (int)ihdu, (int)nhdu, filename);
            }
            else {
                cpl_test_eq_error(code, CPL_ERROR_ILLEGAL_INPUT);
                cpl_msg_warning(cpl_func,
                                "Skipping further tests of header "
                                "with invalid FITS key name(s) %d/%d: %s",
                                (int)ihdu, (int)nhdu, filename);
            }
        ...
        /* Some files fail a DATE-OBS format validation by fitsverify */
        if (cpl_propertylist_has(plist1, "DATE-OBS")) {
            cpl_property *pdobs =
                cpl_propertylist_get_property(plist1, "DATE-OBS");
            if (cpl_property_get_type(pdobs) == CPL_TYPE_STRING &&
                cpl_property_get_size(pdobs) < ISO8601_LEN) {
                cpl_msg_warning(cpl_func,
                                ISO8601_EXAMPLE " replaces short "
                                                "DATE-OBS(%s) from: %s[%d]",
                                cpl_property_get_string(pdobs), filename,
                                (int)ihdu);
                cpl_property_set_string(pdobs, ISO8601_EXAMPLE);
            }
        }
        ...
        if (nfail != cpl_test_get_failed()) {
            cpl_msg_error(cpl_func, "Error saving header %d/%d from: %s",
                          (int)ihdu, (int)nhdu, filename);
        }

        /* Test that the reduced header can be loaded */
        plist2 = cpl_propertylist_load(MYFILE, jhdu);
        cpl_test_error(CPL_ERROR_NONE);
        cpl_test_nonnull(plist2);
        ...
        if (sz1 > sz2) {
            ...
            if (cpl_test_get_failed() == nfailed) {
                cpl_msg_warning(cpl_func,
                                "Header %d seemingly has %d non-"
                                "unique card(s): %s",
                                (int)ihdu, (int)(sz1 - sz2), filename);
            }
        }
        else {
            cpl_test_eq(sz1, sz2);
        }
        if (mfailed != cpl_test_get_failed()) {
            cpl_msg_warning(cpl_func,
                            "Failed load/save/load cycle for header %d of: %s",
                            (int)ihdu, filename);
        }
...
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_numeric_type(void)
{
    const char *keys[] = {
        "EQUINOX",
        "EPOCH",
        ...
        "PC001_01",
        "PC0001_1",
        /* The below keys are not actually valid,
           causing fitsverify to issue a warning */
        "CRPIX999",
        "CRPIX099",
        "CRPIX000",
        "CRPIX00",
    ...
    /* Longest possible string w. a quote fitting in a FITS card */
    code = cpl_propertylist_append_string(plist, "STRINGAB",
                                          "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                          "ABCDEFGHIJ ABCDEFGHIJ ABCDE "
                                          "O'HARA");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    /* Longest possible string w. two quotes fitting in a FITS card */
    code = cpl_propertylist_append_string(plist, "STRINGAC",
                                          "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                          "ABCDEFGHIJ ABCDEFGH "
                                          "O'HARA O'HARA");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    /* String has its 3rd, final quote truncated */
    code = cpl_propertylist_append_string(plist, "STRINGAD",
                                          "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                          "ABCDEFGHIJ ABCDEFGH "
                                          "O'HARA O'HARA'");
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code =
        cpl_imagelist_save(imlist, MYFILE, CPL_TYPE_INT, plist, CPL_IO_CREATE);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    syscode = system("perl -pi -e 's/EPOCH   /ESOCH   /' " MYFILE);
    if (!syscode) {
        cpl_test_fits(MYFILE);
    }
}
...
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_numeric_load_ok(void)
{
    struct numerical
    {
        const char *number;
    ...
    struct numerical okval[] = {
        ...
        {"-0000000000000.", CPL_TYPE_DOUBLE, 0},
        {"+0000000000000.50000000000000000000000000", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.50000000000000000000000000", CPL_TYPE_DOUBLE, -.5},

        /* Floating-point constants that are FITS and C89 compliant */
        {"+.50000000000000000000000000000000000E000", CPL_TYPE_DOUBLE, .5},
        {"-.50000000000000000000000000000000000E000", CPL_TYPE_DOUBLE, -.5},
        {"+0000000000000.5000000000000000000000E000", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.5000000000000000000000E000", CPL_TYPE_DOUBLE, -.5},
        {"+0000000000000.5000000000000000000000E000", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.5000000000000000000000E000", CPL_TYPE_DOUBLE, -.5},
        {"+0000000000000.5000000000000000000000E-00", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.5000000000000000000000E-00", CPL_TYPE_DOUBLE, -.5},
        {"+0000000000000.5000000000000000000000E+00", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.5000000000000000000000E+00", CPL_TYPE_DOUBLE, -.5},
        ...
        /* Floating-point constants that are not C89 compliant, which do not
           follow the "strongly recommended" practice of putting a space
           before the comment separator, but which can be read by CFITSIO */
        {"+0000000000000.5000000000000000000000000/", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.5000000000000000000000000/", CPL_TYPE_DOUBLE, -.5},

        /* Floating-point constants that are FITS, but not C89 compliant */
        {"+0000000000000.5000000000000000000000D000", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.5000000000000000000000D000", CPL_TYPE_DOUBLE, -.5},
        {"+0000000000000.5000000000000000000000D-00", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.5000000000000000000000D-00", CPL_TYPE_DOUBLE, -.5},
        {"+0000000000000.5000000000000000000000D+00", CPL_TYPE_DOUBLE, .5},
        {"-0000000000000.5000000000000000000000D+00", CPL_TYPE_DOUBLE, -.5},
        ...
        {"+000000000000000000000000000000000000042e+1", CPL_TYPE_DOUBLE, 420},
        {"-000000000000000000000000000000000000042e+1", CPL_TYPE_DOUBLE, -420},
        {"+.42e+3", CPL_TYPE_DOUBLE, 420},
        {"-.42e+3", CPL_TYPE_DOUBLE, -420},
        {"00e00", CPL_TYPE_DOUBLE, 0},
        {"-0e+3", CPL_TYPE_DOUBLE, 0},
        {"0000000000000000000000000000000000.000042e+7", CPL_TYPE_DOUBLE, 420},
        {"+000000000000000000000000000000000.000042e+7", CPL_TYPE_DOUBLE, 420},
        {"0000000000000000000000000000000000.000042e07", CPL_TYPE_DOUBLE, 420},
        {"+000000000000000000000000000000000.000042e07", CPL_TYPE_DOUBLE, 420},
    ...
    syscode = system(RMHIERARCHVALQUOTES MYFILE);
    ...
    if (!syscode) {
        ...
        for (int i = 0; i < CPL_MIN(k, nokval); i++) {
...
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_numeric_load_ok2(void)
{
    struct numerical
    {
        const char *number;
    ...
    struct numerical okval[] = {
        ...
        {"9223372036854775807", CPL_TYPE_LONG_LONG},
        {"09223372036854775807", CPL_TYPE_LONG_LONG},
        {"+9223372036854775807", CPL_TYPE_LONG_LONG},
        /* 1/machine precision, 1<<52 - 1, cast from double with no roundoff */
        {"00004503599627370495", CPL_TYPE_LONG_LONG},
        {"+0004503599627370495", CPL_TYPE_LONG_LONG},
        {"-0004503599627370495", CPL_TYPE_LONG_LONG},
        /* 1/machine precision, (1<<52)  */
        {"00004503599627370496", CPL_TYPE_LONG_LONG},
        {"-0004503599627370496", CPL_TYPE_LONG_LONG},
        {"01000000000000000000", CPL_TYPE_LONG_LONG},
        {"+1000000000000000000", CPL_TYPE_LONG_LONG},
        {"-1000000000000000000", CPL_TYPE_LONG_LONG},
        /* ULLLONG_MAX: Would overflow a long long, so becomes double */
        {"18446744073709551615", CPL_TYPE_DOUBLE}};
    ...
    if (!syscode) {
        ...
        for (int i = 0; i < nokval; i++) {
            ...
            if (type == CPL_TYPE_DOUBLE) {
                const double dval = cpl_property_get_double(pp);
                double eval = 0.0;
                const int iparse = sscanf(okval[i].number, "%lg", &eval);

                cpl_test_error(CPL_ERROR_NONE);

                cpl_test_eq(iparse, 1);

                cpl_test_rel(dval, eval, DBL_EPSILON);
            }
            else {
                const long long lval = cpl_property_get_long_long(pp);
                long long eval = 0.0;
                const int iparse = sscanf(okval[i].number, "%lld", &eval);

                cpl_test_error(CPL_ERROR_NONE);

                cpl_test_eq(iparse, 1);

                cpl_test_eq(lval, eval);
            }
...
/**
  @internal
  @brief    Check loading of bad (numerical) values

 */
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_numeric_load_bad(void)
{
    const char *badval[] = {"1A", "-2#", "3,0", "-4.0.", "5E0$", "-6E0E",
                            "7D0)", "-8D0-", "NaN", "Inf", "+Inf", "-Inf",
                            "1e999", "-1e999", "0.F", "   0.F", "+0.F",
                            "+0000.F", "    -0.F", ".E0", "   .E000", "+.E0",
                            "       +.E0", "-.E0", "+-0.0", "  ++0.0",
                            "    --0.0", "+-0.0", "TF", "1EO", "1DO", "E",
                            "1EO", "1DO", "E", "E123", "1E1D1",
                            /* errno: ERANGE */
                            "1E399", "-1E399", "1E-399", "-1E-399", "1D399",
                            "-1D399", "1D-399", "-1D-399"};
    ...
    for (int idocom = 0; idocom < 2; idocom++) {
        ...
        for (int i = 0; i < nbadval; i++) {
            /* Prepend key with a test of the comment support */
            char *ckey = cpl_sprintf("'/' " KEYFORM, 1 + i + nbadval);
            const char *key = ckey + 4;
            char *sval = i % 2 ? cpl_sprintf("(%s, %d)", badval[i], 1 + i)
                               : cpl_sprintf("(%d, %s)", 1 + i, badval[i]);

            cpl_propertylist_append_string(plist, key, sval);
            if (idocom)
                cpl_propertylist_set_comment(plist, key, ckey);
            cpl_free(ckey);
            cpl_free(sval);
        ...
        if (!syscode) {
            ...
            for (int i = 0; i < 2 * nbadval; i++) {
                cpl_propertylist *nulllist;
                char *key = cpl_sprintf(KEYFORM, 1 + i);
                const cpl_size nfailed = cpl_test_get_failed();

                if (i < nbadval) {
                    cpl_msg_info(cpl_func, "Testing BAD-case %d/%d: %s", 1 + i,
                                 (int)nbadval, badval[i]);
                }
                else {
                    cpl_msg_info(cpl_func, "Testing Complex BAD-case %d/%d: %s",
                                 1 + i - nbadval, (int)nbadval,
                                 badval[i - nbadval]);
                }
...
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_numeric_load_ok3(void)
{
    struct numerical
    {
        const char *number;
...
/**
  @internal
  @brief    Check loading of bad keys

 */
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_load_bad_key(void)
{
    cpl_propertylist *plist = cpl_propertylist_new();
    const char *strval;
    cpl_error_code code;
    ...
    code = cpl_propertylist_append_string(plist, "ESO DELKEY", "DELVALUE");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code = cpl_propertylist_set_comment(plist, "ESO DELKEY",
                                        " VALUE INDICATOR = 'is missing'");
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    /* Create an invalid HIERARCH card */
    syscode = system("perl -pi -e 's/ESO DELKEY/          /' " MYFILE);
    cpl_test_zero(syscode);

    cpl_test_fits(MYFILE);

    plist = cpl_propertylist_load_regexp(MYFILE, 0, "HIERARCH", 0);
    cpl_test_error(CPL_ERROR_NONE);
    cpl_test_nonnull(plist);

    nsize = cpl_propertylist_get_size(plist);
    cpl_test_error(CPL_ERROR_NONE);
    cpl_test_eq(nsize, 1);

    ival = cpl_propertylist_has(plist, "HIERARCH");
    cpl_test_error(CPL_ERROR_NONE);

    cpl_test_eq(ival, 1);

    strval = cpl_propertylist_get_string(plist, "HIERARCH");
    cpl_test_nonnull(strval);
    cpl_test_eq_string(strval, "DELVALUE");
    ...
    /* Remove the value indicator from the invalid HIERARCH card */
    syscode = system("perl -pi -e 's/(HIERARCH +)=/$1 /' " MYFILE);
    cpl_test_zero(syscode);

    cpl_test_fits(MYFILE);

    /* The card is no longer loadable */
    plist = cpl_propertylist_load_regexp(MYFILE, 0, "HIERARCH", 0);
    cpl_test_error(CPL_ERROR_NONE);
    cpl_test_nonnull(plist);
...
/**
  @internal
  @brief    Check (OK) loading and (not) saving of card with too long key

 */
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_too_long_key(void)
{
    const char *longkey[NLONGTEST] = {LONGBASE, LONGBASE "HI", LONGBASE "HIJK"};
    const char *longstr[NLONGTEST] = {" " LONGBASE " = T", " " LONGBASE "HI= F",
                                      " " LONGBASE "HIJK="};

    for (int istr = 0; istr < NLONGTEST; istr++) {
        cpl_propertylist *plist = cpl_propertylist_new();
        cpl_error_code code;
        int syscode;
        int status = 0;
        ...
        /* Create a card with a too long HIERARCH key */
        code = cpl_propertylist_append_string(plist, "HISTORY", longstr[istr]);
        cpl_test_eq_error(code, CPL_ERROR_NONE);
        ...
        plist = cpl_propertylist_load_regexp(MYFILE, 0, "HISTORY", 0);
        cpl_test_error(CPL_ERROR_NONE);
        cpl_test_nonnull(plist);
        ...
        ival = cpl_propertylist_has(plist, "HISTORY");
        cpl_test_error(CPL_ERROR_NONE);

        cpl_test_eq(ival, 1);
        cpl_propertylist_delete(plist);

        /* Convert to a (too long) HIERARCH card */
        syscode = system("perl -pi -e 's/HISTORY /HIERARCH/g' " MYFILE);
        cpl_test_zero(syscode);
        ...
        ival = cpl_propertylist_has(plist, longkey[istr]);
        cpl_test_error(CPL_ERROR_NONE);
        cpl_test_eq(ival, 1);

        /* Cannot save these too long keys */
        cpl_test_leq(FLEN_KEYWORD, strlen("HIERARCH ") + strlen(longkey[istr]));
...
/**
  @internal
  @brief    Check support for full cards

 */
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_full_card(void)
{
    cpl_propertylist *plist = cpl_propertylist_new();
    cpl_propertylist *plist2;
    cpl_size nsize, msize;
    int status = 0;
    cpl_error_code code;
    ...
    code = cpl_propertylist_append_char(plist,
                                        "ESO "
                                        "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                        "ABCDEFGHIJ ABCDEFGHIJ AC",
                                        '\'');
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code = cpl_propertylist_append_string(plist,
                                          "ESO "
                                          "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                          "ABCDEFGHIJ ABCDEFGHIJ AD",
                                          "KEEPMENO");
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code = cpl_propertylist_append_string(plist,
                                          "ESO "
                                          "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                          "ABCDEFGHIJ ABCDEFGHIJ AJ",
                                          "O'H'O");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code = cpl_propertylist_append_int(plist,
                                       "ESO "
                                       "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                       "ABCDEFGHIJ ABCDEFGHIJ AG",
                                       12345678);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code = cpl_propertylist_append_int(plist,
                                       "ESO "
                                       "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                       "ABCDEFGHIJ ABCDEFGHIJ AH",
                                       -1234567);
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code = cpl_propertylist_append_double_complex(plist,
                                                  "ESO "
    ...
    code = cpl_propertylist_append_string(
        plist,
        "ESO "
        "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
        "ABCDEFGHIJ ABCDEFGHIJ AA1" CPL_KEY_EXTRA,
        "12");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code =
        cpl_propertylist_append_int(plist,
                                    "ESO "
                                    "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                    "ABCDEFGHIJ ABCDEFGHIJ AB1" CPL_KEY_EXTRA,
                                    1234);
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code =
        cpl_propertylist_append_char(plist,
                                     "ESO "
                                     "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                     "ABCDEFGHIJ ABCDEFGHIJ AC1" CPL_KEY_EXTRA,
                                     '.');
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code = cpl_propertylist_append_double(
        plist,
        "ESO "
        "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
        "ABCDEFGHIJ ABCDEFGHIJ AD1" CPL_KEY_EXTRA,
        -12);
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code =
        cpl_propertylist_append_bool(plist,
                                     "ESO "
                                     "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                     "ABCDEFGHIJ ABCDEFGHIJ AE1" CPL_KEY_EXTRA,
                                     CPL_TRUE);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code =
        cpl_propertylist_append_char(plist,
                                     "ESO "
                                     "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                     "ABCDEFGHIJ ABCDEFGHIJ AF1" CPL_KEY_EXTRA,
                                     '\'');
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    /* No space for the quote */
    code = cpl_propertylist_append_string(
        plist,
        "ESO "
        "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
        "ABCDEFGHIJ ABCDEFGHIJ AG1" CPL_KEY_EXTRA,
        ".\'");
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    plist2 = cpl_propertylist_load_regexp(MYFILE, 0, "ESO ", 0);
    cpl_test_error(CPL_ERROR_NONE);
    cpl_test_nonnull(plist2);

    msize = cpl_propertylist_get_size(plist2);
    cpl_test_error(CPL_ERROR_NONE);
    cpl_test_eq(nsize, msize);

    for (cpl_size j = 0; j < nsize; j++) {
        const cpl_size nfailed = cpl_test_get_failed();
        const cpl_property *p1 = cpl_propertylist_get_const(plist, j);
        const char *name = cpl_property_get_name(p1);
        const cpl_property *p2 =
            cpl_propertylist_get_property_const(plist2, name);
        const cpl_type t1 = cpl_property_get_type(p1);
        cpl_type t2 = CPL_TYPE_INVALID;
        ...
        if (p2 != NULL) {
            const cpl_type t1e = t1 == CPL_TYPE_CHAR ? CPL_TYPE_STRING : t1;

            t2 = cpl_property_get_type(p2);

            cpl_test_eq(t1e, t2);

            if (t1 == CPL_TYPE_STRING && t2 == CPL_TYPE_STRING) {
                const char *v1 = cpl_property_get_string(p1);
                const char *v2 = cpl_property_get_string(p2);
                const int l1 = strlen(v1);
                const int l2 = strlen(v2);

                if (strlen("HIERARCH ") + strlen(name) + l1 + 4 <= 80 &&
                    memchr(v1, '\'', l1) == NULL) {
                    cpl_test_eq(l1, l2);
                    cpl_test_eq_string(v1, v2);
                }
                    ...
                    cpl_test_lt(l2, l1);
                    cpl_test_zero(memcmp(v1, v2, l2));
                }
            }
            else if (t1 == CPL_TYPE_DOUBLE && t2 == CPL_TYPE_DOUBLE) {
                const double v1 = cpl_property_get_double(p1);
                const double v2 = cpl_property_get_double(p2);
                if (v1 < 0.0) {
                    cpl_test_rel(v1, v2, 5 * 1e-1);
                }
                else {
                    cpl_test_rel(v1, v2, 5 * 1e-2);
                }
            }
        }

        if (cpl_test_get_failed() != nfailed) {
            cpl_msg_warning(cpl_func, "FAILED(%d/%d<=>%d): %s (%s <=> %s)",
                            (int)j, (int)nsize, (int)msize, name,
                            cpl_type_get_name(t1), cpl_type_get_name(t2));
        }
    }
...
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_load_bad(void)
{
    /* Illegal card values. "Q" will be modified to a quote "'" */
    const char *badval[] = {"(1., 0.", "(2.. 0.)", "t",  "f",
                            "True",    "False",    "QA", "QBaDQE",
                            "QBaDQQF", "QbadQQ"};
    ...
    for (int idocom = 0; idocom < 2; idocom++) {
        ...
        for (int i = 0; i < nbadval; i++) {
            /* Prepend key with a test of the comment support, but w/o quote */
            char *key = cpl_sprintf(KEYFORM, 1 + i);

            cpl_propertylist_append_string(plist, key, badval[i]);
            if (idocom)
                cpl_propertylist_set_comment(plist, key, key);
            cpl_free(key);
        }
        ...
        /* For each card remove the value's outer quotes, invalidating it */
        syscode = system(RMHIERARCHVALQUOTES SETQQUOTE MYFILE);
...
/**
  @internal
  @brief    Check handling of too long cards

 */
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_too_long_card(void)
{
    cpl_propertylist *plist = cpl_propertylist_new();
    ...
    code = cpl_propertylist_append_float(plist,
                                         "ESO "
                                         "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                         "ABCDEFGHIJ ABCDEFGHIJ AE",
                                         1.236E-45);
    ...
    code = cpl_propertylist_append_long(plist,
                                        "ESO "
                                        "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                        "ABCDEFGHIJ ABCDEFGHIJ AK",
                                        987654321);
    ...
    code = cpl_propertylist_append_long_long(plist,
                                             "ESO "
                                             "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                             "ABCDEFGHIJ ABCDEFGHIJ AM",
                                             987654321);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code = cpl_propertylist_append_float_complex(plist,
                                                 "ESO "
                                                 "ABCDEFGHIJ ABCDEFGHIJ "
                                                 "ABCDEFGHIJ ABCDEFGHIJ "
                                                 "ABCDEFGHIJ AQ",
    ...
    code =
        cpl_propertylist_append_bool(plist,
                                     "ESO "
                                     "ABCDEFGHIJ ABCDEFGHIJ ABCDEFGHIJ "
                                     "ABCDEFGHIJ ABCDEFGHIJ AF15" CPL_KEY_EXTRA,
                                     CPL_FALSE);
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    for (cpl_size j = 0; j < nsize; j++) {
        const cpl_property *p = cpl_propertylist_get_const(plist, j);

        code = cpl_propertylist_append_property(plist2, p);
        cpl_test_eq_error(code, CPL_ERROR_NONE);

        code = cpl_propertylist_save(plist2, FAILFILE, CPL_IO_CREATE);
        cpl_test_eq_error(code, CPL_ERROR_ILLEGAL_INPUT);

        cpl_propertylist_empty(plist2);
    }
...
/**
  @internal
  @brief    Check handling of underflow in a value card

 */
/*----------------------------------------------------------------------------*/
static void
cpl_propertylist_test_underflow(void)
{
    const double dnormal = DBL_MIN * DBL_EPSILON;

    if (dnormal != 0.0) {
        cpl_propertylist *plist = cpl_propertylist_new();
        char *dvalcomment =
            cpl_sprintf("denormal: %g * %g", DBL_MIN, DBL_EPSILON);
        double dval;
        int retval;
        int status = 0, syscode;
        cpl_type type;
        cpl_error_code code;

        code = cpl_propertylist_append_double(plist, "DENORMAL", dnormal);
        cpl_test_eq_error(code, CPL_ERROR_NONE);
        code = cpl_propertylist_set_comment(plist, "DENORMAL", dvalcomment);
        cpl_test_eq_error(code, CPL_ERROR_NONE);

        code = cpl_propertylist_append_string(plist, "ESO DRS DENORMAL",
                                              "7.25D-324");
        cpl_test_eq_error(code, CPL_ERROR_NONE);
        code = cpl_propertylist_set_comment(plist, "ESO DRS DENORMAL",
                                            "Supported. "
                                            "Rounded down to the other");
        cpl_test_eq_error(code, CPL_ERROR_NONE);

        code =
            cpl_propertylist_append_string(plist, "ESO DRS TOZERO", "2.4E-324");
        cpl_test_eq_error(code, CPL_ERROR_NONE);
        code = cpl_propertylist_set_comment(plist, "ESO DRS TOZERO",
                                            "Will truncate to zero");
        cpl_test_eq_error(code, CPL_ERROR_NONE);

        code =
            cpl_propertylist_append_string(plist, "ESO DRS DOZERO", "2.4D-324");
        cpl_test_eq_error(code, CPL_ERROR_NONE);
        code = cpl_propertylist_set_comment(plist, "ESO DRS DOZERO",
                                            "Will truncate to zero");
        cpl_test_eq_error(code, CPL_ERROR_NONE);
        ...
        /* For each card remove the value's outer quotes */
        syscode = system(RMHIERARCHVALQUOTES SETQQUOTE MYFILE);

        plist = cpl_propertylist_load_regexp(MYFILE, 0, "DENORMAL", 0);
        cpl_test_error(CPL_ERROR_NONE);
        cpl_test_nonnull(plist);
        retval = cpl_propertylist_has(plist, "DENORMAL");
        cpl_test_error(CPL_ERROR_NONE);
        cpl_test(retval);
        type = cpl_propertylist_get_type(plist, "DENORMAL");
        cpl_test_error(CPL_ERROR_NONE);
        cpl_test_eq(type, CPL_TYPE_DOUBLE);
        dval = cpl_propertylist_get_double(plist, "DENORMAL");
        cpl_test_error(CPL_ERROR_NONE);
        cpl_test(dval);
        cpl_test_rel(dval, dnormal, FLT_EPSILON);

        if (!syscode) {
            retval = cpl_propertylist_has(plist, "ESO DRS DENORMAL");
            cpl_test_error(CPL_ERROR_NONE);
            cpl_test(retval);
            type = cpl_propertylist_get_type(plist, "ESO DRS DENORMAL");
            cpl_test_error(CPL_ERROR_NONE);
            cpl_test_eq(type, CPL_TYPE_DOUBLE);
            dval = cpl_propertylist_get_double(plist, "ESO DRS DENORMAL");
            cpl_test_error(CPL_ERROR_NONE);
            cpl_test(dval);
            cpl_test_rel(dval, dnormal, FLT_EPSILON);

            cpl_propertylist_delete(plist);
            plist = cpl_propertylist_load_regexp(MYFILE, 0, "TOZERO", 0);
            cpl_test_error(CPL_ERROR_BAD_FILE_FORMAT);
            cpl_test_null(plist);

            cpl_propertylist_delete(plist);
            plist = cpl_propertylist_load_regexp(MYFILE, 0, "DOZERO", 0);
            cpl_test_error(CPL_ERROR_BAD_FILE_FORMAT);
            cpl_test_null(plist);
        }

        cpl_propertylist_delete(plist);
        cpl_free(dvalcomment);
    }
    else {
        cpl_msg_warning(cpl_func, "No FITS support for tiny val: %g * %g = %g",
                        DBL_MIN, DBL_EPSILON, dnormal);
    }
}
...
/**
  @internal
  @brief    Test whether the names in a propertylist are valid FITS keys
  @param self Propertylist to process
  @return True for valid, false for not

 */
/*----------------------------------------------------------------------------*/
static cpl_boolean
cpl_propertylist_test_fits_names(const cpl_propertylist *self)
{
    const int sz = cpl_propertylist_get_size(self);

    for (int i = 0; i < sz; i++) {
        const cpl_property *p1 = cpl_propertylist_get_const(self, i);
        const char *name = cpl_property_get_name(p1);
        const int namelen = cpl_property_get_size_name(p1);
        int stat = 0; /* Uppercase expected */

        for (int j = 0; j < namelen; j++) {
            switch (stat) {
                case 2: /* Space or alpha-numeric expected */
                    if (name[j] == ' ') {
                        stat = 1;
                        break;
                    }
                    CPL_ATTR_FALLTRHU; /* fall through */
                case 1:                /* Alpha-numeric expected */
                    if (islower(name[j])) {
                        cpl_msg_warning(cpl_func,
                                        "Property %d/%d has lower case "
                                        "in name: %s",
                                        i + 1, sz, name);
                        break;
                    }
                    if (isdigit(name[j]) || name[j] == '-' || name[j] == '_')
                        break;

                    CPL_ATTR_FALLTRHU; /* fall through */
                case 0:                /* Uppercase expected */
                    if (!isupper(name[j])) {
                        cpl_msg_warning(cpl_func,
                                        "Property %d/%d has invalid "
                                        "name: %s",
                                        i + 1, sz, name);
                        return CPL_FALSE;
                    }
...
/**
  @internal
  @brief    Create an upper-case copy of a string
  @param self String to copy
  @return Copied, upper-case string

 */
/*----------------------------------------------------------------------------*/
static char *
cpl_toupper(const char *self)
{
    char *other = cpl_strdup(self);
    const int n = strlen(self);
