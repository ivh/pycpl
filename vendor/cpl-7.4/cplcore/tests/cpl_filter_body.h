...
static void
TYPE_ADD(filter_median_bf)(const PIXEL_TYPE *in,
                           PIXEL_TYPE *out,
                           unsigned Nx,
                           unsigned Ny,
                           unsigned Rx,
...
{
    PIXEL_TYPE *data =
        cpl_malloc((size_t)(2 * Rx + 1) * (size_t)(2 * Ry + 1) * sizeof(*data));
    if (mode == CPL_BORDER_FILTER) {
#ifdef CPL_MEDIAN_SAMPLE
        const unsigned Nx_larger = Nx + 2 * Rx;
        const unsigned Ny_larger = Ny + 2 * Ry;

        PIXEL_TYPE *in_larger = cpl_malloc(
            (size_t)Nx_larger * (size_t)Ny_larger * sizeof(*in_larger));
        assure(in_larger != NULL);

        TYPE_ADD(cpl_image_filter_fill_chess)
        (in_larger, in, Nx_larger, Ny_larger, Nx, Ny, Rx, Ry);

        TYPE_ADD(filter_median_bf)
        ...
        for (y = 0 + Ry; y < Ny - Ry; y++, out += Nx) {
            ...
            if (mode == CPL_BORDER_COPY) {
                if (y == Ny - Ry - 1) {
                    (void)memcpy(out + Nx - Rx, in + Nx - Rx + y * Nx,
                                 Rx * sizeof(*out));
                }
            }
        }
...
static void
TYPE_ADD(test_cpl_image_filter)(unsigned Nx,
                                unsigned Ny,
                                unsigned Rx,
                                unsigned Ry,
                                unsigned filter,
                                ...
                                unsigned Nreps2)
{
    ...
    const double myeps
        ...
        = filter == CPL_FILTER_MEDIAN
              ? 0.0
              : 1e1 * Nx * Ny *
                    (sizeof(PIXEL_TYPE) == 4 ? FLT_EPSILON : DBL_EPSILON);
#endif
    ...
    {
        const unsigned Nxc = border_mode == CPL_BORDER_CROP ? Nx - 2 * Rx : Nx;
        const unsigned Nyc = border_mode == CPL_BORDER_CROP ? Ny - 2 * Ry : Ny;
            ...
            if (1) {
                ...
                for (j = 0; j < Nreps1; j++) {
                    if (filter == CPL_FILTER_MEDIAN) {
                        TYPE_ADD(filter_median_bf)
                        (in, ref, Nx, Ny, Rx, Ry, border_mode);
                    }
                        ...
                        TYPE_ADD(image_filter_average_ref)
                        (ref, in, Nx, Ny, Rx, Ry, border_mode);
#elif 1
                        TYPE_ADD(image_filter_average_bf)
                        (ref, in, Nx, Ny, Rx, Ry, border_mode);
#else
                ...
                cpl_msg_info(cpl_func,
                             "Time to %u-filter %u X %u " CPL_STRINGIFY(PIXEL_TYPE) " with %u X %u [s]: %f %f",
...
/*----------------------------------------------------------------------------*/
static void
TYPE_ADD(image_filter_average_ref)(PIXEL_TYPE *out,
                                   const PIXEL_TYPE *in,
                                   int nx,
                                   int ny,
                                   int hsizex,
    ...
    for (i = 0; i < nx * ny; i++) {
        ...
        out[x + y * nx] = (PIXEL_TYPE)((
            (aux[(right + 1) + (upper + 1) * (nx + 1)] +
             aux[left + lower * (nx + 1)] - aux[left + (upper + 1) * (nx + 1)] -
...
/*----------------------------------------------------------------------------*/
static void
TYPE_ADD(image_filter_average_bf)(PIXEL_TYPE *out,
                                  const PIXEL_TYPE *in,
                                  int Nx,
                                  int Ny,
                                  int hsizex,
