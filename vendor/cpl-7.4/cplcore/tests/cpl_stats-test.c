...
#define CPL_STAT_CMP_IMAGE(OP)                                                 \
    do {                                                                       \
        ...
        cpl_test_error(CPL_ERROR_NULL_INPUT);                                  \
                                                                               \
        (void)CPL_CONCAT2X(cpl_image_get,                                      \
                           CPL_CONCAT2X(OP, window))(NULL, 1, 1, IMAGE_SIZE_X, \
                                                     IMAGE_SIZE_Y);            \
        cpl_test_error(CPL_ERROR_NULL_INPUT);                                  \
                                                                               \
        /* Test out-of-range windows parameters */                             \
        (void)CPL_CONCAT2X(cpl_image_get,                                      \
                           CPL_CONCAT2X(OP, window))(img, 0, 1, IMAGE_SIZE_X,  \
                                                     IMAGE_SIZE_Y);            \
        cpl_test_error(CPL_ERROR_ACCESS_OUT_OF_RANGE);                         \
                                                                               \
        (void)CPL_CONCAT2X(cpl_image_get,                                      \
                           CPL_CONCAT2X(OP, window))(img, 1, 0, IMAGE_SIZE_X,  \
                                                     IMAGE_SIZE_Y);            \
        cpl_test_error(CPL_ERROR_ACCESS_OUT_OF_RANGE);                         \
                                                                               \
        (void)CPL_CONCAT2X(cpl_image_get,                                      \
                           CPL_CONCAT2X(OP, window))(img, 1, 1,                \
                                                     IMAGE_SIZE_X + 1,         \
                                                     IMAGE_SIZE_Y);            \
        cpl_test_error(CPL_ERROR_ACCESS_OUT_OF_RANGE);                         \
                                                                               \
        (void)CPL_CONCAT2X(cpl_image_get,                                      \
                           CPL_CONCAT2X(OP, window))(img, 1, 1, IMAGE_SIZE_X,  \
                                                     IMAGE_SIZE_Y + 1);        \
        cpl_test_error(CPL_ERROR_ACCESS_OUT_OF_RANGE);                         \
                                                                               \
        ...
        cpl_test_abs(CPL_CONCAT2X(cpl_stats_get, OP)(statall),                 \
                     CPL_CONCAT2X(cpl_image_get, OP)(img), TOL);               \
    } while (0)


#define CPL_STAT_CMP_VECTOR(OP, TOL)                                         \
    do {                                                                     \
        cpl_vector *vtest = cpl_vector_wrap(IMAGE_SIZE_X * IMAGE_SIZE_Y,     \
...
main(void)
{
    const cpl_type img_types[] = {CPL_TYPE_DOUBLE, CPL_TYPE_FLOAT,
                                  CPL_TYPE_INT};
    const cpl_stats_mode istats[] = {CPL_STATS_MIN,    CPL_STATS_MAX,
                                     CPL_STATS_MEDIAN, CPL_STATS_MEDIAN_DEV,
                                     CPL_STATS_MAD,    CPL_STATS_STDEV,
                                     CPL_STATS_FLUX,   CPL_STATS_ABSFLUX,
                                     CPL_STATS_SQFLUX, CPL_STATS_MINPOS,
    ...
    /* Test 1: NULL image */
    nullstat = cpl_stats_new_from_image(NULL, CPL_STATS_ALL);
    cpl_test_error(CPL_ERROR_NULL_INPUT);
    ...
    for (itype = 0; itype < sizeof(img_types) / sizeof(img_types[0]); itype++) {
        ...
        cpl_msg_info(cpl_func, "Testing %d X %d-image (type %u)", IMAGE_SIZE_X,
                     IMAGE_SIZE_Y, (unsigned)img_type);
        ...
        nullstat =
            cpl_stats_new_from_image_window(img, CPL_STATS_ALL, 2, 2, 1, 1);
        cpl_test_error(CPL_ERROR_ILLEGAL_INPUT);
        cpl_test_null(nullstat);

        nullstat =
            cpl_stats_new_from_image_window(img, CPL_STATS_ALL, 1, 1,
        ...
        /* Test 8A: Compare average from total flux with actual mean
           - computed both specifically and via the "all" option */

        stattwo =
            cpl_stats_new_from_image(img, CPL_STATS_MEAN | CPL_STATS_FLUX);
        cpl_test_error(CPL_ERROR_NONE);
        cpl_test_nonnull(stattwo);

        for (int dostat = 0; dostat < 2; dostat++) {
            const double mean = cpl_stats_get_mean(stattwo);
            const double flux = cpl_stats_get_flux(stattwo);
            const cpl_size npix = cpl_stats_get_npix(stattwo);

            cpl_test_error(CPL_ERROR_NONE);

            /* With Apple LLVM 13.0.0 TOL * 100 is not enough */
            cpl_test_rel(mean * npix, flux, 200.0 * TOL);

            if (dostat == 0) {
                cpl_stats_delete(stattwo);
                stattwo = statall;
            }
        }
        stattwo = NULL;
        ...
        cpl_msg_info(cpl_func,
                     "MAD as std.dev. estimator: MAD * 1.4826 = %g "
                     "<=> %g = st.dev.",
                     sig1 * CPL_MATH_STD_MAD, cpl_stats_get_stdev(statall));
...
/*----------------------------------------------------------------------------*/
static void
cpl_stats_new_all_bench(const cpl_image *image, int n)
{
    double secs;
    int i;
    ...
    cpl_msg_info(cpl_func,
                 "Time spent computing %d %d X %d - sized image "
                 "statistics [s]: %g",
                 n, IMAGE_SIZE_X, IMAGE_SIZE_Y, secs);
}
...
/*----------------------------------------------------------------------------*/
static void
cpl_stats_new_std_bench(const cpl_image *image, int n)
{
    double secs;
    int i;
    ...
    for (i = 0; i < n; i++) {
        cpl_stats *self =
            cpl_stats_new_from_image_window(image,
                                            CPL_STATS_MEAN | CPL_STATS_STDEV, 2,
                                            2, IMAGE_SIZE_X - 1,
                                            IMAGE_SIZE_Y - 1);
...
/*----------------------------------------------------------------------------*/
static void
cpl_stats_new_median_bench(const cpl_image *image, int n)
{
    double secs;
    int i;
