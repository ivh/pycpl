...
#ifdef HAVE_FITS_HEADEND_FFPBYT
#include "fitsio2.h"
#endif
 ...
 * @see fits_read_subset(), ffgsv()
 */
int
cpl_fits_read_subset(fitsfile *fptr,
                     int datatype,
                     const long *blc,
                     const long *trc,
                     const long *inc,
                     ...
                     int *status)
{
    ...
    return fits_read_subset(fptr, datatype, (long *)blc, (long *)trc,
                            (long *)inc, (void *)nulval, array, anynul, status);
    CPL_DIAG_PRAGMA_POP;
}
 ...
 */
int
cpl_fits_write_pix(fitsfile *fptr,
                   int datatype,
                   const long *firstpix,
                   LONGLONG nelem,
                   const void *array,
 ...
 */
int
cpl_fits_write_pixll(fitsfile *fptr,
                     int datatype,
                     const LONGLONG *firstpix,
                     LONGLONG nelem,
                     const void *array,
                     int *status)
{
 ...
 */
int
cpl_fits_create_img(fitsfile *fptr,
                    int bitpix,
                    int naxis,
                    const long *naxes,
                    int *status)
{
    CPL_DIAG_PRAGMA_PUSH_IGN(-Wcast-qual);
    return fits_create_img(fptr, bitpix, naxis, (long *)naxes, status);
    CPL_DIAG_PRAGMA_POP;
}
 ...
 * @brief Wrap around the CFISTIO function dropping const modifiers that the
 *        CFITSIO function should have.
 * @see fits_read_subset(), ffcrimll(), ffphprll()
 */
int
cpl_fits_create_imgll(fitsfile *fptr,
                      int bitpix,
                      int naxis,
                      const LONGLONG *naxes,
                      int *status)
{
    CPL_DIAG_PRAGMA_PUSH_IGN(-Wcast-qual);
    return fits_create_imgll(fptr, bitpix, naxis, (LONGLONG *)naxes, status);
    CPL_DIAG_PRAGMA_POP;
}
#endif
 ...
 */
int
cpl_fits_read_pix(fitsfile *fptr,
                  int datatype,
                  const long *firstpix,
                  LONGLONG nelem,
                  const void *nulval,
 ...
 */
int
cpl_fits_read_pixll(fitsfile *fptr,
                    int datatype,
                    const LONGLONG *firstpix,
                    LONGLONG nelem,
                    const void *nulval,
...
/**
 * @internal
 * @brief Wrap around the CFISTIO function
 * @see fits_write_record(), ffprec()
 */
int
cpl_fits_write_record(fitsfile *fptr, const char *card, int *status)
{
    ...
    CPL_DIAG_PRAGMA_PUSH_IGN(-Wcast-qual);
    ffpbyt(fptr, 80, (char *)card, status); /* write the 80 byte card */
    CPL_DIAG_PRAGMA_POP;
