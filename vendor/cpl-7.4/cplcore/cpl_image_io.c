...
#define CPL_MSG                                           \
    "Image with %" CPL_SIZE_FORMAT " X %" CPL_SIZE_FORMAT \
...
static void
cpl_image_floodfill(cpl_image *, void *, cpl_size, cpl_size, cpl_size);

static cpl_image *cpl_image_load_one(const char *,
                                     cpl_type,
                                     cpl_size,
                                     cpl_size,
                                     cpl_boolean,
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_new(cpl_size nx, cpl_size ny, cpl_type type)
{
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_wrap(cpl_size nx, cpl_size ny, cpl_type type, void *pixels)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_wrap_double(cpl_size nx, cpl_size ny, double *pixels)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_wrap_float(cpl_size nx, cpl_size ny, float *pixels)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_wrap_double_complex(cpl_size nx, cpl_size ny, double complex *pixels)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_wrap_float_complex(cpl_size nx, cpl_size ny, float complex *pixels)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_wrap_int(cpl_size nx, cpl_size ny, int *pixels)
{
    cpl_image *self;
...
/**
  ...
  This function will seamlessly load an image from a tile compressed FITS file.
  Exactly because the loading works automatically whether the file is compressed
  or not, the dimension of a succesfully loaded image should be determined with
  calls to cpl_image_get_size_x() and cpl_image_get_size_y() and not by
  inspecting the values of the FITS file's NAXIS1 and NAXIS2, since these in the
  case of a compressed file will have values different from the dimension of the
  loaded image.
  ...
  - CPL_ERROR_BAD_FILE_FORMAT if the data cannot be loaded from the file
    including attempting to read beyond EOF
  - CPL_ERROR_INVALID_TYPE if the requested pixel type is not supported
 */
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_load(const char *filename,
               cpl_type im_type,
               cpl_size pnum,
               cpl_size xtnum)
{
...
/*----------------------------------------------------------------------------*/
static cpl_image *
cpl_image_load_one(const char *filename,
                   cpl_type im_type,
                   cpl_size pnum,
                   cpl_size xtnum,
                   ...
                   cpl_size ury)
{
    ...
    if (cpl_io_fits_open_diskfile(&fptr, filename, READONLY, &error)) {
        ...
        (void)cpl_error_set_fits(
            CPL_ERROR_FILE_IO, error, fits_open_diskfile,
            "filename='%s', im_type=%u, pnum=%" CPL_SIZE_FORMAT
...
/**
  ...
  Possible #_cpl_error_code_ set in this function:
  - CPL_ERROR_NULL_INPUT if an input pointer is NULL
  - CPL_ERROR_ILLEGAL_INPUT if the passed extension number is negative or
    the window position is invalid
  - CPL_ERROR_FILE_IO if the file cannot be opened or does not exist
  - CPL_ERROR_DATA_NOT_FOUND if the specified extension has no image data
  - CPL_ERROR_BAD_FILE_FORMAT if the data cannot be loaded from the file
    including attempting to read beyond EOF
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_load_window(const char *filename,
                      cpl_type im_type,
                      cpl_size pnum,
                      cpl_size xtnum,
                      cpl_size llx,
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_new_from_mask(const cpl_mask *mask)
{
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_labelise_mask_create(const cpl_mask *in, cpl_size *nbobjs)
{
    cpl_image *intimage;
    int *pio;
...
/*----------------------------------------------------------------------------*/
cpl_type
cpl_image_get_type(const cpl_image *img)
{
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_image_get_size_x(const cpl_image *img)
{
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_image_get_size_y(const cpl_image *img)
{
    cpl_ensure(img, CPL_ERROR_NULL_INPUT, -1);
    return img->ny;
}
...
/*----------------------------------------------------------------------------*/
inline double
cpl_image_get_(const cpl_image *image, cpl_size xpos, cpl_size ypos)
{
...
/**
  ...
  @return   The pixel value (cast to a double), zero if bad, undefined on error
...
/*----------------------------------------------------------------------------*/
double
cpl_image_get(const cpl_image *image,
              cpl_size xpos,
              cpl_size ypos,
              int *pis_rejected)
{
    ...
    *pis_rejected =
        image->bpm != NULL && image->bpm->data[pos] != CPL_BINARY_0 ? 1 : 0;
...
/**
  ...
  @param    ypos           Pixel y position (FITS convention, 1 for lowest)
  @param    pis_rejected   1 if the pixel is bad, 0 if good, negative on error
  @return   The pixel value (cast to a double complex), zero if bad,
     undefined on error
  @see cpl_image_get()
  @note This function is available iff the application includes complex.h
...
/*----------------------------------------------------------------------------*/
double complex
cpl_image_get_complex(const cpl_image *image,
                      cpl_size xpos,
                      cpl_size ypos,
                      int *pis_rejected)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_re_im(cpl_image *im_real,
                     cpl_image *im_imag,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_abs_arg(cpl_image *im_abs,
                       cpl_image *im_arg,
                       const cpl_image *self)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_real(cpl_image *self, const cpl_image *other)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_imag(cpl_image *self, const cpl_image *other)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_mod(cpl_image *self, const cpl_image *other)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_phase(cpl_image *self, const cpl_image *other)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_conjugate(cpl_image *self, const cpl_image *other)
{
    cpl_error_code error;
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_window(cpl_image *self,
                      cpl_size llx,
                      cpl_size lly,
                      cpl_size urx,
                      cpl_size ury,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_int(cpl_image *self, int value)
{
    cpl_ensure_code(self != NULL, CPL_ERROR_NULL_INPUT);
    cpl_ensure_code(self->type == CPL_TYPE_INT, CPL_ERROR_TYPE_MISMATCH);

    if (value == 0) {
        (void)memset(self->pixels, 0,
                     (size_t)self->nx * (size_t)self->ny * sizeof(int));
    }
    else {
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_extract_real(const cpl_image *image)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_extract_imag(const cpl_image *image)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_extract_mod(const cpl_image *image)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_extract_phase(const cpl_image *image)
{
    cpl_image *self;
...
/*----------------------------------------------------------------------------*/
inline void
cpl_image_set_(cpl_image *image, cpl_size xpos, cpl_size ypos, double value)
{
    const cpl_size pos = (xpos - 1) + image->nx * (ypos - 1);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_set(cpl_image *image, cpl_size xpos, cpl_size ypos, double value)
{
    /* Check entries */
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_set_complex(cpl_image *image,
                      cpl_size xpos,
                      cpl_size ypos,
                      double complex value)
{
...
/*----------------------------------------------------------------------------*/
void *
cpl_image_get_data(cpl_image *img)
{
...
/*----------------------------------------------------------------------------*/
const void *
cpl_image_get_data_const(const cpl_image *img)
{
    cpl_ensure(img, CPL_ERROR_NULL_INPUT, NULL);
    return img->pixels;
}
...
/*----------------------------------------------------------------------------*/
cpl_mask *
cpl_image_set_bpm(cpl_image *self, cpl_mask *bpm)
{
    cpl_mask *oldbpm;
...
/*----------------------------------------------------------------------------*/
cpl_mask *
cpl_image_unset_bpm(cpl_image *self)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, NULL);
...
/*----------------------------------------------------------------------------*/
cpl_mask *
cpl_image_get_bpm(cpl_image *img)
{
    cpl_ensure(img != NULL, CPL_ERROR_NULL_INPUT, NULL);

    if (img->bpm == NULL)
        img->bpm = cpl_mask_new(img->nx, img->ny);
...
/*----------------------------------------------------------------------------*/
const cpl_mask *
cpl_image_get_bpm_const(const cpl_image *img)
{
    cpl_ensure(img != NULL, CPL_ERROR_NULL_INPUT, NULL);
...
/*----------------------------------------------------------------------------*/
double *
cpl_image_get_data_double(cpl_image *img)
{
#define CPL_CLASS CPL_CLASS_DOUBLE
#include "cpl_image_io_body.h"
#undef CPL_CLASS
...
/*----------------------------------------------------------------------------*/
float *
cpl_image_get_data_float(cpl_image *img)
{
#define CPL_CLASS CPL_CLASS_FLOAT
#include "cpl_image_io_body.h"
#undef CPL_CLASS
...
/*----------------------------------------------------------------------------*/
double complex *
cpl_image_get_data_double_complex(cpl_image *img)
{
#define CPL_CLASS CPL_CLASS_DOUBLE_COMPLEX
#include "cpl_image_io_body.h"
#undef CPL_CLASS
...
/*----------------------------------------------------------------------------*/
float complex *
cpl_image_get_data_float_complex(cpl_image *img)
{
#define CPL_CLASS CPL_CLASS_FLOAT_COMPLEX
#include "cpl_image_io_body.h"
#undef CPL_CLASS
...
/*----------------------------------------------------------------------------*/
int *
cpl_image_get_data_int(cpl_image *img){
#define CPL_CLASS CPL_CLASS_INT
#include "cpl_image_io_body.h"
#undef CPL_CLASS
}
...
/*----------------------------------------------------------------------------*/
const float *
cpl_image_get_data_float_const(const cpl_image *img)
{
#define CPL_CLASS CPL_CLASS_FLOAT
#include "cpl_image_io_body.h"
#undef CPL_CLASS
...
/*----------------------------------------------------------------------------*/
const int *
cpl_image_get_data_int_const(const cpl_image *img)
{
#define CPL_CLASS CPL_CLASS_INT
#include "cpl_image_io_body.h"
#undef CPL_CLASS
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_duplicate(const cpl_image *src)
{
    cpl_image *self;
    ...
    (void)memcpy(self->pixels, src->pixels,
                 (size_t)src->nx * (size_t)src->ny *
                     cpl_type_get_sizeof(src->type));

    /* 2: Duplicate the bad pixel map, if any */
    if (src->bpm != NULL)
        self->bpm = cpl_mask_duplicate(src->bpm);
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_cast(const cpl_image *self, cpl_type type)
{
    cpl_image *new_im;
    void *ppixels;
    ...
    if (self->type == type)
        return cpl_image_duplicate(self);

    cpl_ensure(type != CPL_TYPE_INVALID, CPL_ERROR_ILLEGAL_INPUT, NULL);

    ppixels =
        cpl_malloc((size_t)(self->nx * self->ny) * cpl_type_get_sizeof(type));
            ...
            switch (self->type) {
                case CPL_TYPE_FLOAT_COMPLEX: {
                    const float complex *pfci =
                        (const float complex *)self->pixels;
                    for (cpl_size i = 0; i < self->nx * self->ny; i++)
    ...
    /* Bad pixel map */
    if (self->bpm != NULL)
        new_im->bpm = cpl_mask_duplicate(self->bpm);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_fill_rejected(cpl_image *im, double a)
{
    cpl_binary *pbpm;
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_save_(const cpl_image *const *self,
                cpl_size nz,
                cpl_boolean is_cube,
                const char *filename,
                cpl_type type,
                ...
                unsigned mode)
{
    ...
    fitsfile *fptr;
    cpl_size i;
    const cpl_boolean hupdate = (mode & CPL_IO_APPEND) ? CPL_TRUE : CPL_FALSE;
    ...
    if (mode & CPL_IO_APPEND) {
        ...
        if (fits_get_num_hdus(fptr, &next, &status)) {
            ...
            return cpl_io_fits_close_file(fptr, &error2)
                       ...
                       : cpl_error_set_message_(
                             ...
                             "!= %d. type=%d, mode=%d)",
                             next, filename, exttype, IMAGE_HDU, type, mode);
        }
            ...
            return cpl_io_fits_close_file(fptr, &error2)
                       ...
                       : cpl_error_set_message_(
                             ...
                             "!= %d). type=%d, mode=%d)",
                             next, filename, bitpix, ibpp, type, mode);
        }
            ...
            return cpl_io_fits_close_file(fptr, &error2)
                       ...
                       : cpl_error_set_message_(CPL_ERROR_ILLEGAL_INPUT,
                                                "Data-unit %d in file %s has "
                                                "wrong NAXIS1/2 (%ldX%ld != "
                                                "%ldX%ld). type=%d, mode=%d)",
                                                next, filename, (long)fnaxes[0],
                                                (long)fnaxes[1], (long)naxes[0],
                                                (long)naxes[1], type, mode);
        }
            ...
            plane1 = fnaxes[2];
            fnaxes[2] += naxes[2];
            if (CPL_FITSIO_RESIZE_IMG(fptr, bitpix, naxis, fnaxes, &status)) {
                int error2 = 0;
                cpl_io_fits_close_file(fptr, &error2);
                return cpl_error_set_fits(CPL_ERROR_FILE_IO, status,
        ...
        return cpl_error_set_fits(CPL_ERROR_FILE_IO, status,
                                  ...
                                  "filename='%s', type=%d, mode=%d", filename,
                                  type, mode);
    }
    else if
    ...
    if (cpl_fits_add_properties(fptr, plist, mode, hupdate)) {
        return cpl_io_fits_close_file(fptr, &status)
                   ? cpl_error_set_fits(CPL_ERROR_FILE_IO, status,
                                        fits_close_file,
                                        "filename='%s', "
    ...
    for (i = 0; i < nz; i++) {
        ...
        /* Write the pixels */
        if (CPL_FITSIO_WRITE_PIX(fptr, cfitsiotype, fpixel, (LONGLONG)(nx * ny),
                                 data, &status))
            break;
    }
    ...
    if (status) {
        ...
        cpl_io_fits_close_file(fptr, &error2);
        return cpl_error_set_fits(CPL_ERROR_FILE_IO, status,
                                  CPL_FITSIO_WRITE_PIX_E,
                                  "filename='%s', "
                                  "type=%d, mode=%d, plane=%u",
                                  filename, type, mode, (unsigned)i);
    }

    /* Close (and write to disk) */
    return cpl_io_fits_close_file(fptr, &status)
               ? cpl_error_set_fits(CPL_ERROR_FILE_IO, status, fits_close_file,
                                    "filename='%s', "
                                    "type=%d, mode=%d",
                                    filename, type, mode)
               : CPL_ERROR_NONE;
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_save(const cpl_image *self,
               const char *filename,
               cpl_type type,
               const cpl_propertylist *plist,
               unsigned mode)
{
    const cpl_error_code error =
        self == NULL
            ? cpl_propertylist_save(plist, filename, mode)
            : cpl_image_save_(&self, 1, CPL_FALSE, filename, type, plist, mode);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_dump_structure(const cpl_image *self, FILE *stream)
{
    ...
    cpl_ensure_code(fprintf(stream, CPL_MSG, self->nx, self->ny,
                            cpl_type_get_name(self->type),
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_dump_window(const cpl_image *self,
                      cpl_size llx,
                      cpl_size lly,
                      cpl_size urx,
                      cpl_size ury,
                      FILE *stream)
{
    ...
    for (j = lly; j <= ury; j++) {
        for (i = llx; i <= urx; i++) {
            const char *badtxt =
                has_bad && cpl_mask_get(self->bpm, i, j) ? " (rejected)" : "";
...
/*----------------------------------------------------------------------------*/
static void
cpl_image_floodfill(cpl_image *lab,
                    void *fftemp,
                    cpl_size x,
                    cpl_size y,
                    cpl_size label)
{
    ...
    ov = pii[x + y * lab->nx];
    if (ov == label || x < wx1 || x > wx2 || y < wy1 || y > wy2)
        return;
    ...
    while (sp > stack) {
        ...
        l = x + 1;
        if (l < x1)
            FFSTACK_PUSH(y, l, x1 - 1, -dy); /* leak on left? */
        x = x1 + 1;
        do {
            for (; x <= wx2 && pii[x + y * lab->nx] == ov; x++)
                pii[x + y * lab->nx] =
                    (int)label; /* FIXME: Need cpl_size pixel */
            FFSTACK_PUSH(y, l, x - 1, dy);
            if (x > x2 + 1)
                FFSTACK_PUSH(y, x2 + 1, x - 1, -dy); /* leak on right? */
        skip:
            for (x++; x <= x2 && pii[x + y * lab->nx] != ov; x++) {
                ;
            }
            l = x;
        } while (x <= x2);
                ...
                cpl_size ury)
{
    ...
    pixels =
        cpl_malloc((size_t)nx * (size_t)ny * cpl_type_get_sizeof(*ppix_type));
    ...
    if (nx == (cpl_size)naxes[0]) {
        ...
        if (fits_read_img(fptr, loadtype, firstelem, nelem, NULL, pixels, NULL,
                          &error)) {
            cpl_free(pixels);
            (void)cpl_error_set_fits(CPL_ERROR_BAD_FILE_FORMAT, error,
                                     fits_read_img,
    ...
    else if (cpl_fits_read_subset(fptr, loadtype, fpixel, lpixel, inc, NULL,
                                  pixels, NULL, &error)) {
        cpl_free(pixels);
        (void)cpl_error_set_fits(CPL_ERROR_BAD_FILE_FORMAT, error,
                                 fits_read_subset,
...
/*----------------------------------------------------------------------------*/
cpl_image *
cpl_image_wrap_(cpl_size nx, cpl_size ny, cpl_type type, void *pixels)
{
    cpl_image *self;
    ...
    self->bpm = NULL;
    self->pixels =
        pixels != NULL ? pixels : cpl_calloc(upix, cpl_type_get_sizeof(type));
