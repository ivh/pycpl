...
cpl_property_check_regexp(const cpl_property *, cxcptr)
CPL_ATTR_PURE CPL_ATTR_NONNULL;

inline static cxbool
cpl_property_check_memcmp(const cpl_property *, cxcptr)
CPL_ATTR_PURE CPL_ATTR_NONNULL;

inline static cxbool
cpl_cstr_check_regexp(const cpl_cstr *, cxcptr)
CPL_ATTR_PURE CPL_ATTR_NONNULL;

inline static cxbool
cpl_cstr_check_memcmp(const cpl_cstr *, cxcptr)
CPL_ATTR_PURE CPL_ATTR_NONNULL;
...
static cpl_error_code _cpl_propertylist_to_fitsfile(fitsfile *,
                                                    const cpl_propertylist *,
                                                    const cpl_regexp *,
                                                    cpl_boolean)
#ifdef CPL_HAVE_ATTR_NONNULL
 ...
 *
 * The function converts the specified FITS header referenced by the cfitsio
 * file pointer to a property list.
 */
                                 ...
                                 const cpl_regexp *regexp)
{
    ...
    for (int i = 1; i <= ncards; i++) {
        ...
        /* Parse one card */
        propi = cpl_property_new_from_string(cardi, names, regexp);
        if (CPL_UNLIKELY(!cpl_errorstate_is_equal(prestate))) {
            return cpl_error_set_message_(cpl_error_get_code(),
                                          "HDU#=%d: Bad FITS Card %d/%d",
                                          hdumov, i, ncards);
        }
        else if (propi != NULL) {
            (void)cpl_propertylist_set_property(self, propi);
        }
    }
                              ...
                              cpl_boolean update)
{
    ...
    cardi[FLEN_CARD - 1] = '\0';
    (void)memset(putkey, 0, FLEN_KEYWORD * sizeof(*putkey));
    ...
    for (; first != last;
        ...
        if (update) {
            ...
            if (is_unique > 0) {
                ...
                cpl_msg_warning(cpl_func,
                                "Non-unique FITS key(len=%d): %s (%d/%d)",
        ...
        if (is_unique < 1) {
            if (cpl_fits_fill_card(cardi, p)) {
                ...
                return cpl_error_set_fits(
                    CPL_ERROR_ILLEGAL_INPUT, error, fits_write_record,
                    "name='%s', type=%d ('%s'), comment='%s'", name, type,
        ...
        switch (type) {
            ...
            case CPL_TYPE_STRING: {
                ...
                if (fits_update_key_str(file, name, value, comment, &error)) {
                    cpl_fits_key_free_unique(putkey);
                    return cpl_error_set_fits(
                        CPL_ERROR_ILLEGAL_INPUT, error, fits_update_key_str,
                        "name='%s', value='%s', comment='%s'", name, value,
                        comment);
                }
                             ...
                             int invert)
{
    if (nstart > 0 || nexact > 0 || invert) {
        ...
        if (cpl_propertylist_erase_filter(self, &filter, NULL) < 0) {
            return cpl_error_set_where_();
        }
    }
                      ...
                      unsigned mode)
{
    ...
    if (cpl_fits_add_properties(fptr, self, mode, CPL_FALSE)) {
        code = cpl_error_set_message_(cpl_error_get_code(),
                                      "Could not add properties "
                                      "to FITS header of file: %s",
                                      filename);
    }
