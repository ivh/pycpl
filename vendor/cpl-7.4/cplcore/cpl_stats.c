...
#define CPL_STATS_DUMP_TWO(OPERATOR, MEMBER, LABEL, CONVERTER)          \
    if (mode & OPERATOR) {                                              \
        ...
        cpl_ensure_code(fprintf(stream, "\t\t%-13s%" CONVERTER "\n",    \
                                "X " LABEL ":",                         \
                                self->CONCAT2X(MEMBER, x)) > 0,         \
                        CPL_ERROR_FILE_IO);                             \
        cpl_ensure_code(fprintf(stream, "\t\t%-13s%" CONVERTER "\n",    \
                                "Y " LABEL ":",                         \
                                self->CONCAT2X(MEMBER, y)) > 0,         \
                        CPL_ERROR_FILE_IO);                             \
    }
...
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_min(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(in->mode & CPL_STATS_MIN, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    return in->min;
}
...
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_max(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(in->mode & CPL_STATS_MAX, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    return in->max;
}
...
/**
  ...
  @param    in  the cpl_stats object
  @return   the mean value, or undefined on error
  @see      cpl_stats_get_min()
 */
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_mean(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(in->mode & CPL_STATS_MEAN, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    return in->mean;
}
...
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_median(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(in->mode & CPL_STATS_MEDIAN, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    return in->med;
}
...
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_median_dev(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
...
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_mad(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
...
/**
  ...
  @param    in  the cpl_stats object
  @return   the standard deviation, or non-positive on error
  @see      cpl_stats_get_min()
 */
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_stdev(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(in->mode & CPL_STATS_STDEV, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    return in->stdev;
}
...
/**
  ...
  @param    in  the cpl_stats object
  @return   the flux, or undefined on error
  @see      cpl_stats_get_min()
 */
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_flux(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(in->mode & CPL_STATS_FLUX, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    return in->flux;
}
...
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_absflux(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(in->mode & CPL_STATS_ABSFLUX, CPL_ERROR_ILLEGAL_INPUT, -2.0);
    return in->absflux;
}
...
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_sqflux(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(in->mode & CPL_STATS_SQFLUX, CPL_ERROR_ILLEGAL_INPUT, -2.0);
    return in->sqflux;
}
...
/**
  ...
  @param    in  the cpl_stats object
  @return   the x centroid, or undefined on error
  @see      cpl_stats_get_min()
 */
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_centroid_x(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(in->mode & CPL_STATS_CENTROID, CPL_ERROR_ILLEGAL_INPUT, 0.0);
...
/**
  ...
  @param    in  the cpl_stats object
  @return   the y centroid, or undefined on error
  @see      cpl_stats_get_min()
 */
/*----------------------------------------------------------------------------*/
double
cpl_stats_get_centroid_y(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(in->mode & CPL_STATS_CENTROID, CPL_ERROR_ILLEGAL_INPUT, 0.0);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_stats_get_min_x(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(in->mode & CPL_STATS_MINPOS, CPL_ERROR_ILLEGAL_INPUT, 0);
    return in->min_x;
}
...
/**
  ...
  @see      cpl_stats_get_min_x()

 */
/*----------------------------------------------------------------------------*/
cpl_size
cpl_stats_get_min_y(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(in->mode & CPL_STATS_MINPOS, CPL_ERROR_ILLEGAL_INPUT, 0);
    return in->min_y;
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_stats_get_max_x(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(in->mode & CPL_STATS_MAXPOS, CPL_ERROR_ILLEGAL_INPUT, 0);
    return in->max_x;
}
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_stats_get_max_y(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(in->mode & CPL_STATS_MAXPOS, CPL_ERROR_ILLEGAL_INPUT, 0);
    return in->max_y;
...
/**
  ...
  @param    in  the cpl_stats object
  @return   the number of pixels, -1 on error.
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_stats_get_npix(const cpl_stats *in)
{
    cpl_ensure(in != NULL, CPL_ERROR_NULL_INPUT, -1);
    return in->npix;
}
...
/*----------------------------------------------------------------------------*/
void
cpl_stats_delete(cpl_stats *stats)
{
    if (stats != NULL)
        cpl_free(stats);
}
...
/**
  ...
  @return   1 newly allocated cpl_stats structure or NULL on error
...
/*----------------------------------------------------------------------------*/
cpl_stats *
cpl_stats_new_from_image_window(const cpl_image *image,
                                cpl_stats_mode mode,
                                cpl_size llx,
                                cpl_size lly,
                                cpl_size urx,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_stats_fill_from_image(cpl_stats *self,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_stats_fill_from_image_window(cpl_stats *self,
                                 const cpl_image *image,
                                 cpl_stats_mode mode,
                                 cpl_size llx,
                                 cpl_size lly,
                                 ...
                                 cpl_size ury)
{
    ...
    cpl_size npix = mpix;
    cpl_ifalloc goodbuf;
    void *goodbufg = NULL; /* Iff non-NULL, then it points to all good pixels
                               - to be permuted in median computation */
    /* A map of the bad pixels in the input */
    const cpl_binary *badmap = image == NULL || image->bpm == NULL
                                   ? NULL
                                   : cpl_mask_get_data_const(image->bpm);

    self->mode = 0; /* Assume failure */
    ...
    /* Need the max if its position is requested */
    if (mode & CPL_STATS_MAXPOS)
        mode |= CPL_STATS_MAX;

    /* Need the min if its position or the centroid is requested */
    if (mode & (CPL_STATS_MINPOS | CPL_STATS_CENTROID))
        mode |= CPL_STATS_MIN;
    ...
    if (mode & CPL_STATS_MEDIAN) {
        /* Duplicate the good pixels inside the window - to be reused below */
        /* Also: Get the first good pixel and the number of bad pixels */

        cpl_ifalloc_set(&goodbuf,
                        (size_t)mpix * cpl_type_get_sizeof(image->type));
...
#undef CPL_OPERATION
        ...
        cpl_ifalloc_set(&goodbuf, 0); /* Initialize for deallocation */

        if (badmap != NULL) {
            /* Count the number of bad pixels inside the subwindow */
            const cpl_size nbadpix =
                cpl_mask_count_window_(image->bpm, llx, lly, urx, ury);

            if (!nbadpix) {
                badmap = NULL;
            }
            else if (npix > nbadpix) { /* Verify that there are good pixels */
                npix -= nbadpix;
            }
            else {
                cpl_ifalloc_free(&goodbuf);
...
#undef CPL_OPERATION
        ...
        if (mode & (CPL_STATS_FLUX | CPL_STATS_ABSFLUX | CPL_STATS_SQFLUX)) {
            /* Switch on image type */
            switch (image->type) {
...
#undef CPL_OPERATION
        ...
        if (mode & (CPL_STATS_MEAN | CPL_STATS_STDEV)) {
            /* Switch on image type */
            switch (image->type) {
...
#undef CPL_OPERATION
        ...
        if (mode & CPL_STATS_MEDIAN) { /* Get median from the good buffer */
            switch (image->type) {
                case CPL_TYPE_DOUBLE:
                    self->med =
                        cpl_tools_get_median_double((double *)goodbufg, npix);
                    break;
                case CPL_TYPE_FLOAT:
                    self->med =
                        cpl_tools_get_median_float((float *)goodbufg, npix);
                    break;
                case CPL_TYPE_INT:
                    self->med = cpl_tools_get_median_int((int *)goodbufg, npix);
                    break;
                default:
                    /* See comment in previous switch() default: */
                    cpl_ifalloc_free(&goodbuf);
        ...
        if (mode & (CPL_STATS_MIN | CPL_STATS_MAX)) { /* Has to follow median */
            /* Switch on image type */
            switch (image->type) {
...
#undef CPL_OPERATION
    ...
    if (mode & CPL_STATS_MAD) { /* Last, modifies the median buffer */
        switch (image->type) {
...
#undef CPL_OPERATION
    ...
    cpl_ifalloc_free(&goodbuf);
#ifdef CPL_STATS_DEBUG
    cpl_msg_warning(cpl_func,
                    "%s(mode=%X:type=%X): %d X %d <=> %d X %d = %d >= %d",
                    llx == 1 && urx == image->nx ? "stats" : "STATS",
                    image->type, mode, (int)image->nx, (int)image->ny,
                    1 + (int)(urx - llx), 1 + (int)(ury - lly), (int)mpix,
                    (int)npix);
#endif
...
/*----------------------------------------------------------------------------*/
cpl_stats *
cpl_stats_new_from_image(const cpl_image *image, cpl_stats_mode mode)
{
    cpl_stats *self;
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_stats_dump(const cpl_stats *self, cpl_stats_mode mode, FILE *stream)
{
    ...
    CPL_STATS_DUMP_TWO(CPL_STATS_MINPOS, min, "min. pos.", CPL_SIZE_FORMAT);
    CPL_STATS_DUMP_TWO(CPL_STATS_MAXPOS, max, "max. pos.", CPL_SIZE_FORMAT);
