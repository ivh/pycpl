...
/**
  @internal
  @brief  Definition of the polynomial object
...
inline static cpl_polynomial_1d *cpl_polynomial_new_1d(cpl_size) CPL_ATTR_ALLOC;
...
inline static void
cpl_polynomial_grow_1d_reset(cpl_polynomial_1d *, cpl_size, cpl_size);
...
static cpl_boolean
cpl_polynomial_prune_(cpl_polynomial_1d *, cpl_size, cpl_size);
...
static void
cpl_polynomial_multiply_scalar_self(cpl_polynomial_1d *, cpl_size, double);

static cpl_error_code
cpl_polynomial_dump_coeff(const cpl_size *, cpl_size, FILE *, double)
...
cpl_polynomial_get_coeff_(const cpl_polynomial_1d *, const cpl_size *, cpl_size)
#ifdef CPL_HAVE_ATTR_NONNULL
    __attribute__((nonnull(2)));
#endif

inline static cpl_long_double
cpl_polynomial_eval_(const cpl_polynomial_1d *, cpl_size, const double *)
#ifdef CPL_HAVE_ATTR_NONNULL
    __attribute__((nonnull(3)));
#endif
...
static cpl_error_code cpl_polynomial_solve_1d__(const cpl_polynomial_1d *,
                                                ...
                                                double *,
                                                cpl_size,
                                                cpl_boolean) CPL_ATTR_NONNULL;

inline static cpl_long_double
cpl_polynomial_eval_1d_(const cpl_polynomial_1d *, double, double *)
#ifdef CPL_HAVE_ATTR_NONNULL
    __attribute__((nonnull(1)))
#endif
    ;
...
inline static cpl_long_double
cpl_polynomial_nested_horner(const cpl_polynomial_1d *,
                             double,
                             double,
                             cpl_long_double *) CPL_ATTR_NONNULL;
...
static cpl_boolean cpl_polynomial_delete_coeff_(cpl_polynomial_1d *,
                                                const cpl_size *,
                                                cpl_size) CPL_ATTR_NONNULL;
...
static void cpl_matrix_fill_normal_vandermonde(cpl_matrix *,
                                               cpl_matrix *,
                                               const cpl_vector *,
                                               cpl_boolean,
                                               cpl_size,
...
static cpl_error_code cpl_polynomial_fit_2d(cpl_polynomial *,
                                            const cpl_bivector *,
                                            const cpl_vector *,
                                            const cpl_vector *,
                                            cpl_boolean,
...
static cpl_error_code cpl_polynomial_fit_3d(cpl_polynomial *,
                                            const cpl_vector *,
                                            const cpl_vector *,
                                            const cpl_vector *,
                                            const cpl_vector *,
...
/*----------------------------------------------------------------------------*/
cpl_polynomial *
cpl_polynomial_new(cpl_size dim)
{
    cpl_polynomial *self;
...
/*----------------------------------------------------------------------------*/
void
cpl_polynomial_empty(cpl_polynomial *self)
{
    if (self != NULL) {
        assert(self->dim > 0);
...
/*----------------------------------------------------------------------------*/
void
cpl_polynomial_delete(cpl_polynomial *self)
{
    if (self != NULL) {
...
/*----------------------------------------------------------------------------*/
/**
  @brief    Dump a polynomial as ASCII to a stream
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_dump(const cpl_polynomial *self, FILE *stream)
{
    cpl_error_code code;
    int retval;
    cpl_size icoeff = 0;
    ...
    if (self->tree == NULL) {
        ...
        code = cpl_polynomial_dump_(self->tree, self->dim - 1, pows, self->dim,
                                    &icoeff, stream);
        cpl_free(pows);
    }
...
/*----------------------------------------------------------------------------*/
cpl_polynomial *
cpl_polynomial_duplicate(const cpl_polynomial *self)
{
    ...
    cpl_polynomial_copy_(other->tree, self->dim - 1, self->tree, self->dim - 1);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_copy(cpl_polynomial *self, const cpl_polynomial *other)
{
    cpl_ensure_code(self != NULL, CPL_ERROR_NULL_INPUT);
    ...
    if (other->tree == NULL) {
        ...
        cpl_polynomial_copy_(self->tree, self->dim - 1, other->tree,
                             other->dim - 1);
...
/*----------------------------------------------------------------------------*/
int
cpl_polynomial_compare(const cpl_polynomial *self,
                       ...
                       double tol)
{
    ...
    return cpl_polynomial_compare_(self->tree, other->tree, self->dim - 1, tol);
}
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_polynomial_get_degree(const cpl_polynomial *self)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_polynomial_get_dimension(const cpl_polynomial *self)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
...
/*----------------------------------------------------------------------------*/
double
cpl_polynomial_get_coeff(const cpl_polynomial *self, const cpl_size *pows)
{
    ...
    for (cpl_size mydim = 0; mydim < self->dim; mydim++) {
        if (pows[mydim] < 0) {
            (void)cpl_error_set_message_(
                ...
                "Dimension %" CPL_SIZE_FORMAT " of %" CPL_SIZE_FORMAT
...
/**
  ...
  When setting multiple coefficients there is a small efficiency gain when
  setting the highest powers first.
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_set_coeff(cpl_polynomial *self,
                         const cpl_size *pows,
                         double value)
{
    ...
    if (value != 0.0) {
        ...
        if (powdegree > self->degree)
            self->degree = powdegree;
    }
    else {
...
/**
  ...
  @par For multiple evaluations the input vector can be updated like this:
  @code
    double xy[2];
...
/*----------------------------------------------------------------------------*/
double
cpl_polynomial_eval(const cpl_polynomial *self, const cpl_vector *x)
{
...
/*----------------------------------------------------------------------------*/
cpl_polynomial *
cpl_polynomial_extract(const cpl_polynomial *self,
                       cpl_size dim,
                       const cpl_polynomial *other)
{
    ...
    if (self->tree != NULL) {
        ...
        if (cpl_polynomial_extract_(collapsed->tree, self->tree, self->dim - 1,
                                    dim, value)) {
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_derivative(cpl_polynomial *self, cpl_size dim)
{
    cpl_ensure_code(self != NULL, CPL_ERROR_NULL_INPUT);
    ...
    if (self->tree != NULL) {
        if (cpl_polynomial_derivative_(self->tree, self->dim - 1, dim)) {
            ...
            self->degree =
                cpl_polynomial_find_degree_(self->tree, self->dim - 1);
        }
    }
...
/*----------------------------------------------------------------------------*/
/**
  @brief    Evaluate a univariate (1D) polynomial using Horner's rule.
  @param    self  The 1D-polynomial
  @param    x     The point of evaluation
  @param    pd    Iff pd is non-NULL the derivative (dy/dx) evaluated at x
...
/*----------------------------------------------------------------------------*/
double
cpl_polynomial_eval_1d(const cpl_polynomial *self, double x, double *pd)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1.0);
...
/*----------------------------------------------------------------------------*/
/**
  @brief    Evaluate p(a) - p(b) using Horner's rule.
  @param    self  The 1D-polynomial
...
/*----------------------------------------------------------------------------*/
double
cpl_polynomial_eval_1d_diff(const cpl_polynomial *self,
                            double a,
                            double b,
                            double *ppa)
{
...
/*----------------------------------------------------------------------------*/
/**
  @brief    Evaluate a 1D-polynomial on equidistant points using Horner's rule
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_vector_fill_polynomial(cpl_vector *v,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_solve_1d(const cpl_polynomial *p,
                        double x0,
                        double *px,
                        cpl_size mul)
{
...
/**
  ...
  @note Currently only 1D (uni-variate), 2D (bi-variate) and 3D (tri-variate) 
        polynomials are supported. For all but uni-variate 
        polynomials mindeg must be zero.
  ...
  N * (N + 1) / 2 coefficients are fitted. For a 3D-polynomial,
  it is N * (N + 1) * (N + 2) / 6.
  ...
  passed via fitsigm. NULL means that all uncertainties equals one. This
  uncertainties can be applied to the least squares method by converting them
  to weights (w = 1.0 / sigma^2).
  ...
  1) If fitsigm is non-NULL. The factors are applied to the values.
  2) If mindeg is zero, the sampling positions are first transformed into
     Xhat_i = X_i - mean(X_i), i=1, .., dimension.
  3) The Vandermonde matrix is formed from Xhat. If fitsigm is non-NULL,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_fit(cpl_polynomial *self,
                   ...
                   const cpl_size *maxdeg)
{
    ...
    if (fitsigm != NULL && cpl_vector_get_size_(fitsigm) != np)
        return cpl_error_set_message_(CPL_ERROR_INCOMPATIBLE_INPUT,
                                      ...
                                      "rors",
                                      np, cpl_vector_get_size(fitsigm));

    CPL_DIAG_PRAGMA_PUSH_IGN(-Wcast-qual);
    if (mdim == 1) {
        cpl_vector *x_pos =
            cpl_vector_wrap(np, cpl_matrix_get_data_((cpl_matrix *)samppos));
        const cpl_boolean isampsym = sampsym ? sampsym[0] : CPL_FALSE;
        const int mindeg0 = mindeg ? mindeg[0] : 0;
        const cpl_error_code error =
            cpl_polynomial_fit_1d(self, x_pos, fitvals, fitsigm, mindeg0,
                                  maxdeg[0], isampsym, NULL);
        CPL_DIAG_PRAGMA_POP;
        ...
        cpl_vector *y_pos =
            cpl_vector_wrap(np,
                            np + cpl_matrix_get_data_((cpl_matrix *)samppos));
        cpl_bivector *xy_pos = cpl_bivector_wrap_vectors(x_pos, y_pos);
        /* If dimdeg is false, only first element is read/used */
        const cpl_size mindeg0[2] = {mindeg ? mindeg[0] : 0,
                                     mindeg ? mindeg[dimdeg ? 1 : 0] : 0};
        const cpl_error_code error =
            cpl_polynomial_fit_2d(self, xy_pos, fitvals, fitsigm, dimdeg,
                                  mindeg0, maxdeg);
        CPL_DIAG_PRAGMA_POP;
        (void)cpl_vector_unwrap(x_pos);
        ...
        cpl_vector *y_pos =
            cpl_vector_wrap(np,
                            np + cpl_matrix_get_data_((cpl_matrix *)samppos));
        cpl_vector *z_pos =
            cpl_vector_wrap(np, np * 2 + cpl_matrix_get_data_(
                                             (cpl_matrix *)samppos));
        /* If dimdeg is false, only first element is read/used */
        const cpl_size mindeg0[3] = {mindeg ? mindeg[0] : 0,
                                     mindeg ? mindeg[dimdeg ? 1 : 0] : 0,
                                     mindeg ? mindeg[dimdeg ? 2 : 0] : 0};

        const cpl_error_code error =
            cpl_polynomial_fit_3d(self, x_pos, y_pos, z_pos, fitvals, fitsigm,
                                  dimdeg, mindeg0, maxdeg);

        CPL_DIAG_PRAGMA_POP;
        (void)cpl_vector_unwrap(x_pos);
        (void)cpl_vector_unwrap(y_pos);
        (void)cpl_vector_unwrap(z_pos);
        ...
        return cpl_error_set_message_(CPL_ERROR_UNSUPPORTED_MODE,
                                      "The fitting "
                                      "dimension %d > 3 is not supported",
                                      (int)mdim);
    }
                                        ...
                                        double *rechisq)
{
    ...
    const double *dsamppos;
    const double *dfitvals;
    double *dself;
    cpl_size i;
    ...
    cpl_ensure_code(cpl_matrix_get_ncol_(samppos) == np,
                    CPL_ERROR_INCOMPATIBLE_INPUT);

    cpl_ensure_code(cpl_matrix_get_nrow_(samppos) == mdim,
                    CPL_ERROR_INCOMPATIBLE_INPUT);

    /* May not be under-determined */
    cpl_ensure_code(np >= nc, CPL_ERROR_DATA_NOT_FOUND);

    dsamppos = cpl_matrix_get_data_const_(samppos);
    dfitvals = cpl_vector_get_data_const_(fitvals);
    ...
    if (mdim == 1) {
        for (i = 0; i < np; i++) {
            dself[i] =
                dfitvals[i] - cpl_polynomial_eval_1d(fit, dsamppos[i], NULL);
        }
    }
    else {
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_shift_1d(cpl_polynomial *p, cpl_size i, double u)
{
    ...
    if (p->tree != NULL) {
        ...
        if (i > 0) {
            ...
            cpl_polynomial_transpose_(transtree, p->dim - 1, pows, p->dim,
                                      p->tree, i);
            self = transtree;
        }
        ...
        if (i > 0) {
            ...
            cpl_polynomial_transpose_(p->tree, p->dim - 1, pows, p->dim,
                                      transtree, i);
...
/*----------------------------------------------------------------------------*/
cpl_polynomial *
cpl_polynomial_fit_1d_create(const cpl_vector *x_pos,
                             ...
                             double *pmse)
{
    ...
    const cpl_error_code error =
        cpl_polynomial_fit_1d(self, x_pos, values, NULL, 0, degree, is_eqdist,
                              pmse);
...
/*----------------------------------------------------------------------------*/
cpl_polynomial *
cpl_polynomial_fit_2d_create(const cpl_bivector *xy_pos,
                             const cpl_vector *values,
                             cpl_size degree,
                             double *pmse)
{
    ...
    const cpl_error_code error =
        cpl_polynomial_fit_2d(self, xy_pos, values, NULL, CPL_FALSE, &mindeg0,
                              &degree);
    if (error != CPL_ERROR_NONE) {
        cpl_polynomial_delete(self);
        self = NULL;
        ...
        *pmse = 0.0;
        for (cpl_size i = 0; i < np; i++) {
            ...
            residual = dvalues[i] - cpl_polynomial_eval(self, x_val);
            *pmse += residual * residual;
        }
...
/**
  ...
  @param    self     The 2D-polynomial
  @param    x        The x component of the evaluation point
  @param    y        The y component of the evaluation point
  @param    gradient Iff gradient is non-NULL, the 2D gradient array containing
...
/*----------------------------------------------------------------------------*/
double
cpl_polynomial_eval_2d(const cpl_polynomial *self,
                       double x,
                       double y,
                       double *gradient)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(self->dim == 2, CPL_ERROR_INVALID_TYPE, -3.0);
    ...
    if (self->tree == NULL || self->tree[0].nc == 0) { /* zero polynomial */
        return 0.0;
    }
    ...
    if (gradient != NULL) {
        ...
        while (j--) {
            double sumj = 0.0;
            double sumpdj = 0.0;
            const cpl_polynomial_1d *itree = self->tree[0].coef[j].next;
            if (itree) {
                i = itree->nc;
                while (i--) {
                    sumj = sumj * x + itree->coef[i].val;
                    if (i > 0) {
                        sumpdj = sumpdj * x + i * itree->coef[i].val;
                    }
                }
            }
            result = result * y + sumj;
            gradient[0] = gradient[0] * y + sumpdj;
            if (j > 0) {
                gradient[1] = gradient[1] * y + j * sumj;
            }
        }
    }
    else {
...
/**
  @brief    Evaluate a 3D polynomial using Horner's rule and compute the
            the derivatives if needed.
  @param    self     The 3D-polynomial
  @param    x        The x component of the evaluation point
  @param    y        The y component of the evaluation point
  @param    z        The z component of the evaluation point
  @param    gradient Iff gradient is non-NULL, the 3D gradient array containing
...
/*----------------------------------------------------------------------------*/
double
cpl_polynomial_eval_3d(const cpl_polynomial *self,
                       double x,
                       double y,
                       double z,
                       double *gradient)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(self->dim == 3, CPL_ERROR_INVALID_TYPE, -3.0);
    ...
    if (gradient != NULL) {
        ...
        while (k--) {
            ...
            double sumpdkx = 0.0;
            double sumpdky = 0.0;
            const cpl_polynomial_1d *jtree = self->tree[0].coef[k].next;
            if (jtree) {
                j = jtree->nc;
                while (j--) {
                    double sumpdjx = 0.0;
                    double sumj = 0.0;
                    const cpl_polynomial_1d *itree = jtree->coef[j].next;
                    if (itree) {
                        i = itree->nc;
                        while (i--) {
                            sumj = sumj * x + itree->coef[i].val;
                            if (i > 0) {
                                sumpdjx = sumpdjx * x + i * itree->coef[i].val;
                            }
                        }
                    }
                    sumk = sumk * y + sumj;
                    sumpdkx = sumpdkx * y + sumpdjx;
                    if (j > 0) {
                        sumpdky = sumpdky * y + j * sumj;
                    }
                }
            }
            result = result * z + sumk;
            gradient[0] = gradient[0] * z + sumpdkx;
            gradient[1] = gradient[1] * z + sumpdky;
            if (k > 0) {
                gradient[2] = gradient[2] * z + k * sumk;
            }
        }
    }
    else {
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_add(cpl_polynomial *self,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_subtract(cpl_polynomial *self,
                        const cpl_polynomial *first,
                        const cpl_polynomial *second)
{
    cpl_ensure_code(self != NULL, CPL_ERROR_NULL_INPUT);
    ...
    if (first->tree == NULL && second->tree == NULL) {
        ...
        if (cpl_polynomial_subtract_(self->tree, first->tree, second->tree,
                                     self->dim - 1)) {
            cpl_polynomial_empty(self);
        }
        else {
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_multiply(cpl_polynomial *self,
                        const cpl_polynomial *first,
                        const cpl_polynomial *second)
{
    ...
    if (first->tree != NULL && second->tree != NULL) {
        ...
        if (self->tree == NULL) {
            ...
            self->tree =
                cpl_polynomial_new_1d(first->tree->nc + second->tree->nc);
        }
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_multiply_scalar(cpl_polynomial *self,
                               const cpl_polynomial *other,
                               double factor)
{
    cpl_ensure_code(self != NULL, CPL_ERROR_NULL_INPUT);
    ...
    if (factor != 0.0 && other->tree != NULL) {
        if (self == other) {
            cpl_polynomial_multiply_scalar_self(self->tree, self->dim - 1,
                                                factor);
        }
        else {
            if (self->tree == NULL)
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_polynomial_get_1d_size(const cpl_polynomial *self)
{
    return (char *)&(self->tree->coef[self->tree->nc].val) -
...
/**
  ...
  @param  x_pos   Vector of positions of the signal to fit.
  @param  values  Vector of values of the signal to fit.
  @param  sigmas  Vector of uncertainties of the sampled values, or NULL for all ones.
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_fit_1d(cpl_polynomial *self,
                      ...
                      double *pmse)
{
    ...
    if (sigmas != NULL) { /* Check sigma errors */
        const double *sval = cpl_vector_get_data_const_(sigmas);
        for (i = 0; i < np; ++i) {
            cpl_ensure_code(sval[i] != 0.0, CPL_ERROR_DIVISION_BY_ZERO);
        }
    }
    ...
    if (degree == mindeg) {
        ...
        if (degree == 0) {
            ...
            if (sigmas != NULL) {
                ...
                const double *sval = cpl_vector_get_data_const_(sigmas);
                double sum = 0.0;  /* sum of squared weighted values */
                double wsum = 0.0; /* sum of the squared weights */

                /* Compute the squared weighted mean */
                for (i = 0; i < np; ++i) {
                    const double w = 1.0 / (sval[i] * sval[i]);
                    wsum += w * w;
                    sum += dval[i] * w * w;
                }
                mean = sum / wsum;
                ...
                /* In the case of a weighted fit, the default definition
                   of the MSE still applied. 
                   MSE=sum((x - p(x))^2)/N => MSE=sum((x - wmean)^2)/N */
                if (pmse != NULL) {
                    *pmse = 0.0;
                    for (i = 0; i < np; i++) {
                        /* Subtract from the true value, square, accumulate */
                        double residual = dval[i] - mean;
                        *pmse += residual * residual;
                    }
                    /* Average the error term */
                    *pmse /= (double)np;
                ...
                /* If requested, compute mean squared error */
                if (pmse != NULL)
                    *pmse = cpl_tools_get_variance_double(
                        cpl_vector_get_data_const_(values), np, &mean);
                else
            ...
            const double *dval = cpl_vector_get_data_const_(values);
            double h =
                0.0; /* Hankel = Transpose(W * Vandermonde) * (W * Vandermonde) */
            double vtv = 0.0; /* Transpose(W * Vandermonde) * (W * values) */
            ...
            if (sigmas != NULL) {
                ...
                /* Sigma vector is filled with one (no need to apply weights) */
                for (i = 0; i < np; i++) {
                    const double xn = cpl_tools_ipow(xpos[i], (int)mindeg);
                    vtv += xn * dval[i];
                    h += xn * xn;
    ...
    */
    ...
    if (mindeg == 0) {
        if (sigmas != NULL) {
            /* Weights must be taken in account */
            /* Transform: xhat = x - wmean(x) */
            xhat = cpl_vector_duplicate(x_pos);
            const double *dval = cpl_vector_get_data_const_(xhat);
            const double *sval = cpl_vector_get_data_const_(sigmas);
            double sum = 0.0;
            double wsum = 0.0;
            for (i = 0; i < np; i++) {
                double w = 1.0 / (sval[i] * sval[i]);
                sum += dval[i] * w * w;
                wsum += w * w;
            }
            mean = sum / wsum;
            cpl_vector_subtract_scalar(xhat, mean);
            xuse = xhat;
        }
        else {
    ...
    /* If we apply the weights we have H = (W * V') * (W * V) */
    ...
    mh = symsamp
             ? cpl_matrix_new(nc, nc)
             : cpl_matrix_wrap(nc, nc,
                               cpl_malloc((size_t)(nc * nc) * sizeof(double)));
    ...
    cpl_msg_warning(cpl_func,
                    "MINDEG=%" CPL_SIZE_FORMAT ". degree=%" CPL_SIZE_FORMAT
                    ". nc=%" CPL_SIZE_FORMAT ". np=%" CPL_SIZE_FORMAT
                    ". mean=%g",
                    mindeg, degree, nc, np, mean);
    ...
    if (error) {
        ...
        cpl_polynomial_empty(self);
        return cpl_error_set_message_(error,
                                      "mindeg=%" CPL_SIZE_FORMAT
                                      ". degree=%" CPL_SIZE_FORMAT
                                      ". nc=%" CPL_SIZE_FORMAT
                                      ". np=%" CPL_SIZE_FORMAT ". mean=%g",
                                      mindeg, degree, nc, np, mean);
    }
        ...
        *pmse = 0.0;
        for (i = 0; i < np; i++) {
            ...
            const double residual =
                dval[i] - cpl_polynomial_eval_1d(self, xpos[i], NULL);
            *pmse += residual * residual;
        }
        /* Average the error term */
        *pmse /= (double)np;
...
/**
  ...
  @param  sigmas  Vector of uncertainties of the sampled values, or NULL for all ones.
  @param  dimdeg  True iff there is a fitting degree per dimension
  @param  mindeg  Pointer to 1 or d minimum fitting degree(s), at least 0
  @param  maxdeg  Pointer to 1 or d maximum fitting degree(s), at least mindeg
  @return The fitted polynomial or NULL on error
  @see cpl_polynomial_fit()
 */
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_polynomial_fit_2d(cpl_polynomial *self,
                      ...
                      const cpl_size *maxdeg)
{
    ...
    cpl_matrix *mh; /* Block-Hankel matrix, (W * V') * (W * V) */
    cpl_matrix *mb;
    cpl_matrix *mx;
    ...
    if (sigmas != NULL) { /* Check sigma errors */
        sval = cpl_vector_get_data_const_(sigmas);
        for (i = 0; i < np; ++i) {
            cpl_ensure_code(sval[i] != 0.0, CPL_ERROR_DIVISION_BY_ZERO);
        }
    }
    ...
    if (np < nc) {
        return cpl_error_set_message_(CPL_ERROR_DATA_NOT_FOUND,
                                      ...
                                      "(dimdeg=%d, mindeg=%d)",
                                      (int)np, (int)nc, (int)degree,
                                      (int)dimdeg, (int)mindeg[0]);
    }
    ...
    /* If the fit use sigmas, the solved matrix equation is
       (W*V)t*(W*V)*x = (W*V)t*(W*y) where W is the diagonal weighted matrix. We can see
       that the (W*V)t is common in both member. The weights can therefore be multiplied
       direclty in the construction of the transposed Vandermonde matrix. We also must
       multiply the values (y) by the weights.
       https://www.cs.auckland.ac.nz/courses/compsci369s1c/lectures/GG-notes/CS369-LeastSquares.pdf (slide 36)
       https://en.wikipedia.org/wiki/Weighted_least_squares */

    /* Apply the weights to the values (W*y) */
    if (sigmas != NULL) {
        wvalues = cpl_vector_duplicate(values);
        double *dval = cpl_vector_get_data_(wvalues);
        for (i = 0; i < np; i++) {
            const double w = 1.0 / (sval[i] * sval[i]);
            dval[i] *= w;
        }
    ...
    if (is_mindeg) {
        ...
        if (sigmas != NULL) {
            /* Weights must be taken in account */
            /* Transform: xhat = x - wmean(x) */
            /* Transform: yhat = y - wmean(y) */
            double xsum, ysum;
            xhat = cpl_vector_duplicate(cpl_bivector_get_x_const(xy_pos));
            yhat = cpl_vector_duplicate(cpl_bivector_get_y_const(xy_pos));
            const double *xdval = cpl_vector_get_data_const(xhat);
            const double *ydval = cpl_vector_get_data_const(yhat);
            xsum = ysum = wsum = 0.0;
            for (i = 0; i < np; i++) {
                double w = 1.0 / (sval[i] * sval[i]);
                xsum += xdval[i] * w * w;
            ...
            xmean = xsum / wsum;
            ymean = ysum / wsum;
            cpl_vector_subtract_scalar(xhat, xmean);
            cpl_vector_subtract_scalar(yhat, ymean);
            xuse = xhat;
            yuse = yhat;
        }
        else {
    ...
    /* If weights are used, the factors are applied to the values */
    for (i = 0; i < np; i++) {
        ...
        for (degy = 0; degy < ncy; degy++) {
            ...
            for (degx = 0; degx < ncx; degx++) {
                if (dimdeg ? (degx >= mindeg[0] && degy >= mindeg[1])
                           : degx + degy >= mindeg[0]) {
                    /* multiplied weights if needed */
                    dmv[np * j++ + i] =
                        sval ? (xvalue * yvalue) / (sval[i] * sval[i])
                             : xvalue * yvalue;
                }
                xvalue *= x;
            }
    ...
    /* mb contains the values, it is weighted */
    CPL_DIAG_PRAGMA_PUSH_IGN(-Wcast-qual);
    mb = cpl_matrix_wrap(np, 1, cpl_vector_get_data_(wvalues));
    CPL_DIAG_PRAGMA_POP;
    ...
    coeffs1d = cpl_matrix_get_data_const_(mx);
    ...
    for (degy = 0; degy < ncy; degy++) {
        ...
        for (degx = 0; degx < ncx; degx++) {
            if (dimdeg ? (degx >= mindeg[0] && degy >= mindeg[1])
                       : degx + degy >= mindeg[0]) {
                const double coeff = cpl_matrix_get_(mx, j, 0);
                if (coeff != 0.0) {
                    ...
                    if (coeffs1d[j] != coeff)
                        break;
                    if (cpl_polynomial_set_coeff(self, powers, coeff))
                        break;
                }
                j++;
            }
...
/**
  @internal
  @brief  Fit a 3D-polynomial to a 3D-volume in a least squares sense
  @param  x_pos   X positions of the points volume to fit.
  @param  y_pos   Y positions of the points volume to fit.
  @param  z_pos   Z positions of the points volume to fit.
  @param  values  Vector of values of the volume to fit.
  @param  sigmas  Vector of uncertainties of the sampled values, or NULL for all ones.
  @param  dimdeg  True iff there is a fitting degree per dimension
  @param  mindeg  Pointer to 1 or d minimum fitting degree(s), at least 0
  @param  maxdeg  Pointer to 1 or d maximum fitting degree(s), at least mindeg
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_polynomial_fit_3d(cpl_polynomial *self,
                      const cpl_vector *x_pos,
                      const cpl_vector *y_pos,
                      const cpl_vector *z_pos,
                      const cpl_vector *values,
                      const cpl_vector *sigmas,
                      cpl_boolean dimdeg,
                      const cpl_size *mindeg,
                      const cpl_size *maxdeg)
{
    const cpl_size np = cpl_vector_get_size_(values);
    cpl_size degree;                   /* The degree of the fitted polynomial */
    cpl_boolean is_mindeg = CPL_FALSE; /* Whether mindeg is non-zero */
    cpl_size nc;                       /* Number of unknowns to determine */
    cpl_size ncz;   /* Number of Z-coefficients (incl. mindeg ones) */
    cpl_matrix *mv; /* The transpose of the Vandermonde matrix */
    cpl_matrix *mh; /* Block-Hankel matrix, (W*V)'*(W*V) */
    cpl_matrix *mb;
    cpl_matrix *mx;
    ...
    cpl_ensure_code(self != NULL, CPL_ERROR_NULL_INPUT);
    cpl_ensure_code(cpl_polynomial_get_dimension(self) == 3,
                    CPL_ERROR_INVALID_TYPE);
    cpl_ensure_code(np > 0, cpl_error_get_code());
    cpl_ensure_code(values != NULL, CPL_ERROR_NULL_INPUT);
    cpl_ensure_code(maxdeg != NULL, CPL_ERROR_NULL_INPUT);

    cpl_ensure_code(cpl_vector_get_size_(x_pos) == np &&
                        cpl_vector_get_size_(y_pos) == np &&
                        cpl_vector_get_size_(z_pos) == np,
                    CPL_ERROR_INCOMPATIBLE_INPUT);

    if (sigmas != NULL) { /* Check sigmas errors */
        sval = cpl_vector_get_data_const_(sigmas);
        for (i = 0; i < np; ++i) {
            cpl_ensure_code(sval[i] != 0.0, CPL_ERROR_DIVISION_BY_ZERO);
        }
    ...
    if (dimdeg) {
        ...
        cpl_ensure_code(mindeg[2] >= 0, CPL_ERROR_ILLEGAL_INPUT);
        cpl_ensure_code(maxdeg[2] >= mindeg[2], CPL_ERROR_ILLEGAL_INPUT);

        if (mindeg[0] > 0 || mindeg[1] > 0 || mindeg[2] > 0)
            is_mindeg = CPL_TRUE;

        degree = maxdeg[0] + maxdeg[1] + maxdeg[2];
        ncz = maxdeg[2] + 1;
        nc = (maxdeg[0] + 1 - mindeg[0]) * (maxdeg[1] + 1 - mindeg[1]) *
             (ncz - mindeg[2]);
    }
    else {
        /* 
          In this case, we want to calculate the number of entries in the              
          Vandermonde matrix which is the number of combination for 
          x^i * y^j * z^k where i + j + k <= degree.                                                         
          This number can be calculated with the following formula for 3D case:        
            nc(n) = n * (n + 1) * (n + 2) / 6                                          
          Proof:                                                                       
            nc(n) = sum(j=1->n, sum(k=1->j, k))                                          
            nc(n) = sum(j=1->n, j * (j + 1) / 2)                                        
            nc(n) = 1/2 * (sum(j=1->n, j * j) + sum(j=1->n, j))                          
            nc(n) = 1/2 * n * (n + 1) * (2 * n + 1) / 6 + n * (n + 1) / 2                                           
            nc(n) = n * (n + 1) * (n + 2) / 6    
          This proof can be generalized for a polynomial of dimension n and degree d:
            nc(d, n) = (d + n - 1)! / (d! * (n - 1)!)                                   
        */

        /* Compute the number of 0 coefficients using the previous formula */
        const cpl_size nc0 =
            (mindeg[0] * (mindeg[0] + 1) * (mindeg[0] + 2)) / 6;

        degree = maxdeg[0];
        ncz = degree + 1;

        /* Compute the number of entries which is nc(degree + 1) - nc0 */
        nc = (((degree + 1) * (degree + 2) * (degree + 3)) / 6) - nc0;
    ...
    if (np < nc) {
        return cpl_error_set_message_(CPL_ERROR_DATA_NOT_FOUND,
                                      "Too few data "
                                      "points %d < %d for %d-degree 3D-fit "
                                      "(dimdeg=%d, mindeg=%d)",
                                      (int)np, (int)nc, (int)degree,
                                      (int)dimdeg, (int)mindeg[0]);
    }
    ...
    if (is_mindeg) {
        ...
        if (sigmas != NULL) {
            /* Weights must be taken in account */
            /* Transform: xhat = x - weightedmean(x) */
            /* Transform: yhat = y - weightedmean(y) */
            /* Transform: zhat = z - weightedmean(z) */
            double xsum, ysum, zsum;
            xhat = cpl_vector_duplicate(x_pos);
            yhat = cpl_vector_duplicate(y_pos);
            zhat = cpl_vector_duplicate(z_pos);
            const double *xdval = cpl_vector_get_data_const(xhat);
            const double *ydval = cpl_vector_get_data_const(yhat);
            const double *zdval = cpl_vector_get_data_const(zhat);
            xsum = ysum = zsum = wsum = 0.0;
            for (i = 0; i < np; i++) {
                double w = 1.0 / (sval[i] * sval[i]);
                xsum += xdval[i] * w * w;
                ysum += ydval[i] * w * w;
            ...
            cpl_vector_subtract_scalar(xhat, xmean);
            cpl_vector_subtract_scalar(yhat, ymean);
            cpl_vector_subtract_scalar(zhat, zmean);
            xuse = xhat;
            yuse = yhat;
            zuse = zhat;
        }
            ...
            /* Transform: xhat = x - mean(x) */
            xhat = cpl_vector_transform_mean(x_pos, &xmean);
            assert(xhat != NULL);
            xuse = xhat;

            /* Transform: yhat = y - mean(y) */
            yhat = cpl_vector_transform_mean(y_pos, &ymean);
            assert(yhat != NULL);
            yuse = yhat;

            /* Transform: zhat = z - mean(z) */
            zhat = cpl_vector_transform_mean(z_pos, &zmean);
            assert(zhat != NULL);
            zuse = zhat;
        }
    }
    ...
    dxuse = cpl_vector_get_data_const_(xuse);
    dyuse = cpl_vector_get_data_const_(yuse);
    dzuse = cpl_vector_get_data_const_(zuse);
    ...
    for (i = 0; i < np; i++) {
        const double x = dxuse[i];
        const double y = dyuse[i];
        const double z = dzuse[i];
        j = 0;
        double zvalue = 1.0;
        for (degz = 0; degz < ncz; ++degz) {
            const cpl_size ny = 1 + (dimdeg ? maxdeg[1] : degree - degz);
            double yvalue = 1.0;
            for (degy = 0; degy < ny; ++degy) {
                const cpl_size nx =
                    1 + (dimdeg ? maxdeg[0] : degree - degz - degy);
                double xvalue = 1.0;
                for (degx = 0; degx < nx; ++degx) {
                    if (dimdeg ? (degx >= mindeg[0] && degy >= mindeg[1] &&
                                  degz >= mindeg[2])
                               : degx + degy + degz >= mindeg[0]) {
                        dmv[np * j++ + i] =
                            sval
                                ? (xvalue * yvalue * zvalue) /
                                      (sval[i] * sval[i])
                                : xvalue * yvalue *
                                      zvalue; /* multiplied weights if needed */
                    }
                    xvalue *= x;
                }
                yvalue *= y;
    ...
    cpl_tools_add_flops(np *
                        (nc * 3)); /* TODO: find the good flops estimation */
    ...
    /* Check coefficients for anomalies */
    /* This check was added due to its presence in the 2D case */
    ...
    /* - make sure there is enough upper dim space, then fill up from below */
    if (self->tree == NULL) {
        self->tree = cpl_polynomial_new_1d(ncz);
    }
    else {
        cpl_polynomial_grow_1d(self->tree, ncz);
    }
    ...
    for (degz = 0; degz < ncz; ++degz) {
        ...
        powers[2] = degz;
        for (degy = 0; degy < ny; ++degy) {
            const cpl_size nx = 1 + (dimdeg ? maxdeg[0] : degree - degz - degy);
            powers[1] = degy;
            for (degx = 0; degx < nx; ++degx) {
                if (dimdeg ? (degx >= mindeg[0] && degy >= mindeg[1] &&
                              degz >= mindeg[2])
    ...
    if (do_prune && cpl_polynomial_prune_(self->tree, self->dim - 1, ncz))
        cpl_polynomial_empty(self);
    ...
    if (!is_mindeg) {
        ...
        cpl_polynomial_shift_1d(self, 0, -xmean);
        cpl_polynomial_shift_1d(self, 1, -ymean);
        cpl_polynomial_shift_1d(self, 2, -zmean);
    }
...
/*----------------------------------------------------------------------------*/
static void
cpl_polynomial_delete_coeff(cpl_polynomial *self,
                            const cpl_size *pows,
                            cpl_size deldeg)
{
    if (self->tree == NULL)
...
/**
  ...
  @brief    Fill the Hankel Matrix H=V'*V, where V is a 1D-Vandermonde matrix
            and apply the weights (1.0 / sigma^2) if needed (H = (W*V)'*(W*V)).
  @param    self      The matrix H
  @param    mx        A right multiplication with V', mx = V' * values 
                      or mx = (W*V)'*W*values with weights.
  ...
  @param    sigmas    The uncertainties of the sampled values
  @return   void
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_matrix_fill_normal_vandermonde(cpl_matrix *self,
                                   cpl_matrix *mx,
                                   ...
                                   const cpl_vector *sigmas)
{
    ...
    double *ehat = NULL;
    const cpl_size nc = cpl_matrix_get_ncol_(self);
    ...
    if (sigmas != NULL) {
        assert(np == cpl_vector_get_size_(sigmas));
        shat = (cpl_vector *)sigmas; /* required to compute the sum */
    }
    else {
        shat = cpl_vector_new(np);
        cpl_vector_fill(shat, 1.0);
    }

    sval = cpl_vector_get_data_const_(shat);

    assert(nc == cpl_matrix_get_nrow_(self));
    assert(nc == cpl_matrix_get_nrow_(mx));
    assert(1 == cpl_matrix_get_ncol_(mx));
    ...
    if (mindeg > 0) {
        ...
        for (k = 0; k < np; k++) {
            ...
            const double w = 1.0 / (sval[k] * sval[k]);

            if (mindeg > 1)
                ehat[k] = cpl_tools_ipow(x, (int)mindeg);
            dhat[k] *= ehat[k];

            hsum += ehat[k] * ehat[k] * w * w;
        }
        cpl_matrix_set_(self, 0, 0, hsum);
    }
    else {
        /* First coefficient is the sum of weights */
        /* NULL sigms means sum(wi)=np */
        double sum = 0.0;
        for (i = 0; i < np; ++i) {
            const double w = 1.0 / (sval[i] * sval[i]);
            sum += w * w;
        }
        cpl_matrix_set_(self, 0, 0, sum);
    }
    ...
    for (j = 1; j < 2; j++) {
        ...
        for (k = 0; k < np; k++) {
            ...
            hsum += mindeg > 0 ? ehat[k] * dhat[k] * w * w : dhat[k] * w * w;
            vsum += y * dhat[k] * w * w;
            vsum0 += mindeg > 0 ? ehat[k] * y * w * w : y * w * w;
        }
        cpl_matrix_set_(mx, 0, 0, vsum0);
        cpl_matrix_set_(mx, j, 0, vsum);
        if (is_eqdist)
            continue;
        k = j;
        for (i = 0; i <= k; i++, k--) {
            cpl_matrix_set_(self, i, k, hsum);
        }
    }
    for (; j < nc; j++) {
        double hsum = 0.0;
...
/*----------------------------------------------------------------------------*/
static int
cpl_vector_is_eqdist(const cpl_vector *self)
{
    double xval, delta;
    ...
    if (cpl_vector_get_size_(self) == 1)
        return 1;
...
/*----------------------------------------------------------------------------*/
inline void
cpl_polynomial_shift_double(double *coeffs, cpl_size n, double u)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_polynomial_solve_1d_(const cpl_polynomial *self,
                         double x0,
                         double *px,
                         cpl_size mul,
                         cpl_boolean bpos)
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_polynomial_solve_1d__(const cpl_polynomial_1d *p,
                          double x0,
                          double *px,
                          cpl_size mul,
                          cpl_boolean bpos)
{
    ...
    for (i = 0; i < mite; i++) {
        ...
        if (bpos && d <= 0.0)
            break;
        if (d * dprev >= 0.0 && fabs(r * dprev) >= fabs(rprev * d))
            break;
        ...
        if (fabs(x - xprev) < fabs(x) * DBL_EPSILON)
            break;
    ...
    *px = x;
    ...
    if (i == mite)
        return cpl_error_set_message_(CPL_ERROR_CONTINUE,
                                      "x0=%g, mul=%d, degree=%d, "
                                      "i=%d, p(x=%g)=%g",
                                      x0, (int)mul, (int)p->nc - 1, (int)i, x,
                                      (double)cpl_polynomial_eval_1d_(p, x,
                                                                      NULL));
    ...
    if (fabs(r) > fabs(d)) {
        ...
        double max = 0.0;
        while (n)
            if (fabs(p->coef[--n].val) > max)
                max = fabs(p->coef[n].val);

        if (fabs(r) > max * DBL_EPSILON)
            return cpl_error_set_message_(
                CPL_ERROR_DIVISION_BY_ZERO,
                "x0=%g, mul=%d, "
                "degree=%d, i=%d, p(x=%g)=%g, p'(x)=%g",
                x0, (int)mul, (int)p->nc - 1, (int)i, x, r, d);
    }
 ...
 */
/*----------------------------------------------------------------------------*/
inline static cpl_polynomial_1d *
cpl_polynomial_new_1d(cpl_size nc)
{
    const cpl_size nz =
        nc > CPL_POLYNOMIAL_DEFAULT_COEFFS ? nc : CPL_POLYNOMIAL_DEFAULT_COEFFS;
    ...
    self = cpl_malloc(sizeof(*self) + (size_t)nz * sizeof(*(self->coef)));
    self->coef = (void *)(self + 1);
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_polynomial_grow_1d(cpl_polynomial_1d *self, cpl_size newsize)
{
    if (newsize == 0 || newsize > self->nz) {
        ...
        if (self->nc == 0) {
            ...
            self->coef =
                memcpy(cpl_malloc((size_t)self->nz * sizeof(*(self->coef))),
                       self->coef, (size_t)self->nc * sizeof(*(self->coef)));
        }
            ...
            self->coef = cpl_realloc(self->coef,
                                     (size_t)self->nz * sizeof(*(self->coef)));
        }
    }
}
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_polynomial_grow_1d_reset(cpl_polynomial_1d *self,
                             cpl_size newsize,
                             cpl_size zeroto)
{
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_polynomial_delete_(cpl_polynomial_1d *self, cpl_size mydim)
{
    if (self != NULL) {
        if (mydim > 0) {
            if (self->nc > 0) { /* Could be a zero-polynomial from a collapse */
                ...
                do {
                    self->nc--;
                    cpl_polynomial_delete_(self->coef[self->nc].next,
                                           mydim - 1);
                } while (self->nc);
            }
            assert(self->nc == 0);
...
/*----------------------------------------------------------------------------*/
static void
cpl_polynomial_reset_(cpl_polynomial_1d *self, cpl_size mydim)
{
    if (self != NULL) {
        if (mydim > 0) {
...
/*----------------------------------------------------------------------------*/
static cpl_boolean
cpl_polynomial_prune_(cpl_polynomial_1d *self, cpl_size mydim, cpl_size newsize)
{
    ...
    if (self != NULL) {
        ...
        if (mydim == 0) {
            for (; i > 0; i--) {
                if (self->coef[i - 1].val != 0.0)
                    break;
            }
        }
        else {
            for (; i > 0; i--) {
                if (self->coef[i - 1].next == NULL)
                    continue;

                if (self->coef[i - 1].next->nc == 0 ||
                    cpl_polynomial_prune_(self->coef[i - 1].next, mydim - 1,
...
/*----------------------------------------------------------------------------*/
static cpl_boolean
cpl_polynomial_delete_coeff_(cpl_polynomial_1d *self,
                             const cpl_size *pows,
                             cpl_size mydim)
{
    cpl_boolean do_del = CPL_FALSE;
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_polynomial_set_coeff_(cpl_polynomial_1d *self,
                          const cpl_size *pows,
                          cpl_size mydim,
                          double value)
{
    ...
    if (mydim == 0) {
        ...
        cpl_polynomial_set_coeff_(self->coef[pows[mydim]].next, pows, mydim - 1,
                                  value);
    }
}
...
/*----------------------------------------------------------------------------*/
static cpl_size
cpl_polynomial_find_degree_(const cpl_polynomial_1d *self, cpl_size mydim)
{
...
/*----------------------------------------------------------------------------*/
static void
cpl_polynomial_copy_(cpl_polynomial_1d *self,
                     cpl_size selfdim,
                     ...
                     cpl_size otherdim)
{
    if (otherdim == 0) {
        ...
        selfdim > 0 ? assert(self->nc <= other->nc) : assert(self->nc == 0);
...
/*----------------------------------------------------------------------------*/
static int
cpl_polynomial_compare_(const cpl_polynomial_1d *self,
                        const cpl_polynomial_1d *other,
                        cpl_size mydim,
                        double tol)
{
    if (self != NULL || other != NULL) {
        ...
        if (mydim == 0) {
            for (; i < nmin; i++)
                if (fabs(self->coef[i].val - other->coef[i].val) > tol)
                    return i + 1;

            for (; i < nother; i++)
                if (fabs(other->coef[i].val) > tol)
                    return i + 1;

            for (; i < nself; i++)
                if (fabs(self->coef[i].val) > tol)
                    return i + 1;
        }
        else {
            int cmp = 0;
            ...
            for (; i < nother; i++) {
                cmp = cpl_polynomial_compare_(NULL, other->coef[i].next,
                                              mydim - 1, tol);
                if (cmp)
                    return i + cmp;
            }
            for (; i < nself; i++) {
                cmp = cpl_polynomial_compare_(self->coef[i].next, NULL,
                                              mydim - 1, tol);
                if (cmp)
                    return i + cmp;
            }
...
/*----------------------------------------------------------------------------*/
static cpl_boolean
cpl_polynomial_derivative_(cpl_polynomial_1d *self,
                           cpl_size mydim,
                           cpl_size dodim)
{
    if (mydim > dodim) {
        ...
        for (cpl_size i = 0; i < self->nc; i++) {
            ...
            cpl_polynomial_multiply_scalar_self(self->coef[i].next, mydim - 1,
                                                (double)(i + 1));
        }
    }
    else {
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_polynomial_dump_coeff(const cpl_size *pows,
                          cpl_size dim,
                          FILE *stream,
                          double value)
{
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_polynomial_transpose_(cpl_polynomial_1d *self,
                          cpl_size mydim,
                          cpl_size *pows,
                          cpl_size dim,
                          const cpl_polynomial_1d *other,
                          cpl_size dodim)
{
    if (mydim == 0) {
        for (cpl_size i = 0; i < other->nc; i++) {
            ...
            if (value != 0.0) {
                /* Write the zero'th dimension into the dimension to swap */
                pows[dodim] = i;
        ...
        for (cpl_size i = 0; i < other->nc; i++) {
            ...
            pows[mydim == dodim ? 0 : mydim] = i;

            cpl_polynomial_transpose_(self, mydim - 1, pows, dim,
                                      other->coef[i].next, dodim);
        }
    }
}
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_polynomial_dump_(const cpl_polynomial_1d *self,
                     cpl_size mydim,
                     cpl_size *pows,
                     cpl_size dim,
                     cpl_size *ncoeff,
    ...
    if (mydim == 0) {
        for (cpl_size i = 0; i < self->nc; i++) {
            ...
            if (value != 0.0) {
                ...
                if (cpl_polynomial_dump_coeff(pows, dim, stream, value)) {
                    return cpl_error_set_where_();
                }
            }
        }
        ...
        for (cpl_size i = 0; i < self->nc; i++) {
            if (self->coef[i].next != NULL) {
                ...
                if (cpl_polynomial_dump_(self->coef[i].next, mydim - 1, pows,
                                         dim, ncoeff, stream)) {
                    return cpl_error_set_where_();
                }
            }
...
/*----------------------------------------------------------------------------*/
inline static double
cpl_polynomial_get_coeff_(const cpl_polynomial_1d *self,
                          const cpl_size *pows,
                          cpl_size mydim)
{
    double value = 0.0;
...
/*----------------------------------------------------------------------------*/
inline static cpl_long_double
cpl_polynomial_nested_horner(const cpl_polynomial_1d *self,
                             double a,
                             double b,
                             cpl_long_double *ppa)
{
...
/*----------------------------------------------------------------------------*/
inline static cpl_long_double
cpl_polynomial_eval_1d_(const cpl_polynomial_1d *self, double x, double *pd)
{
    ...
    if (pd == NULL) {
        ...
        while (i)
            result = x * result + self->coef[--i].val;
    }
    else if (self->nc > 1) {
...
/*----------------------------------------------------------------------------*/
inline static cpl_long_double
cpl_polynomial_eval_(const cpl_polynomial_1d *self,
                     cpl_size mydim,
                     const double *px)
{
...
/*----------------------------------------------------------------------------*/
double
cpl_polynomial_eval_monomial(const cpl_polynomial *self, const cpl_vector *x)
{
    cpl_long_double result = 0.0;
    ...
    if (self->tree != NULL) {
        ...
        cpl_polynomial_eval_monomial_(self->tree, self->dim - 1, pows,
                                      self->dim, cpl_vector_get_data_const_(x),
                                      &result);
        cpl_free(pows);
    }
...
/*----------------------------------------------------------------------------*/
static void
cpl_polynomial_eval_monomial_(const cpl_polynomial_1d *self,
                              cpl_size mydim,
                              cpl_size *pows,
                              cpl_size dim,
                              const double *px,
    ...
    if (self != NULL && self->nc > 0) {
        if (mydim == 0) {
            ...
            for (cpl_size i = 0; i < self->nc; i++) {
                ...
                cpl_polynomial_eval_monomial_(self->coef[i].next, mydim - 1,
                                              pows, dim, px, presult);
            }
        }
    }
...
/*----------------------------------------------------------------------------*/
static cpl_boolean
cpl_polynomial_add_(cpl_polynomial_1d *self,
                    ...
                    cpl_size mydim)
{
    ...
    if (first != NULL || second != NULL) {
        ...
        if (mydim == 0) {
            ...
            for (; i < nmin; i++) {
                if (self->coef[i].next == NULL)
                    self->coef[i].next = cpl_polynomial_new_1d(
                        CPL_MAX(first->coef[i].next->nc,
                                second->coef[i].next->nc));

                if (cpl_polynomial_add_(self->coef[i].next, first->coef[i].next,
                                        second->coef[i].next, mydim - 1)) {
                    cpl_polynomial_delete_(self->coef[i].next, mydim - 1);
                    self->coef[i].next = NULL;
                }
            }
            ...
            for (; i < nsecond; i++) {
                ...
                cpl_polynomial_copy_(self->coef[i].next, mydim - 1,
                                     second->coef[i].next, mydim - 1);
            }
            for (; i < nfirst; i++) {
                if (self->coef[i].next == NULL)
...
/*----------------------------------------------------------------------------*/
static cpl_boolean
cpl_polynomial_subtract_(cpl_polynomial_1d *self,
                         const cpl_polynomial_1d *first,
                         const cpl_polynomial_1d *second,
                         cpl_size mydim)
{
    ...
    if (first != NULL || second != NULL) {
        ...
        if (mydim == 0) {
            ...
            for (; i < nsecond; i++) {
                ...
                if (cpl_polynomial_subtract_(self->coef[i].next, NULL,
                                             second->coef[i].next, mydim - 1)) {
                    cpl_polynomial_delete_(self->coef[i].next, mydim - 1);
                    self->coef[i].next = NULL;
                }
...
/*----------------------------------------------------------------------------*/
static void
cpl_polynomial_multiply_scalar_(cpl_polynomial_1d *self,
                                const cpl_polynomial_1d *other,
                                cpl_size mydim,
                                double factor)
{
...
/*----------------------------------------------------------------------------*/
static void
cpl_polynomial_multiply_scalar_self(cpl_polynomial_1d *self,
                                    cpl_size mydim,
                                    double factor)
{
    if (self != NULL) {
...
/*----------------------------------------------------------------------------*/
static cpl_boolean
cpl_polynomial_scale_add_(cpl_polynomial_1d *self,
                          const cpl_polynomial_1d *second,
                          cpl_size mydim,
                          double factor,
                          const cpl_size *pows)
        ...
        if (mydim == 0) {
            ...
            for (i = 0; i < second->nc; i++) {
                if (self->coef[i + pows[mydim]].next == NULL)
                    self->coef[i + pows[mydim]].next = cpl_polynomial_new_1d(
                        second->coef[i].next->nc + pows[mydim - 1]);
...
/*----------------------------------------------------------------------------*/
static void
cpl_polynomial_multiply_(cpl_polynomial_1d *self,
                         const cpl_polynomial_1d *first,
                         const cpl_polynomial_1d *second,
                         cpl_size mydim,
                         cpl_size *pows,
    ...
    if (mydim == 0) {
        for (cpl_size i = 0; i < second->nc; i++) {
            ...
            if (value != 0.0) {
                ...
                if (cpl_polynomial_scale_add_(self, first, dim - 1, value,
                                              pows)) {
...
/*----------------------------------------------------------------------------*/
inline static void
cpl_polynomial_shift_1d_(cpl_polynomial_1d *self, cpl_size mydim, double u)
{
    if (mydim > 0) {
        for (cpl_size i = 0; i < self->nc; i++) {
            if (self->coef[i].next == NULL)
...
/*----------------------------------------------------------------------------*/
static cpl_boolean
cpl_polynomial_extract_(cpl_polynomial_1d *self,
                        const cpl_polynomial_1d *other,
                        cpl_size mydim,
                        cpl_size dodim,
                        double value)
{
    ...
    if (mydim == dodim) {
        ...
        cpl_polynomial_copy_(self, mydim - 1, other->coef[icopy].next,
                             mydim - 1);

        for (cpl_size i = icopy; i > 0; i--) {
            cpl_polynomial_multiply_scalar_self(self, mydim - 1, value);

            if (other->coef[i - 1].next == NULL)
                continue;

            cpl_polynomial_add_(self, self, other->coef[i - 1].next, mydim - 1);
        }
        ...
        for (; i > 0; i--) {
            self->coef[i - 1].val =
                cpl_polynomial_eval_(other->coef[i - 1].next, mydim - 1,
                                     &value);
        }
        do_del = self->nc == 0;
    }
