...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_reject_value(cpl_image *self, cpl_value mode)
{
    ...
    switch (cpl_image_get_type(self)) {
        ...
        default:
            ...
            error =
                self != NULL ? CPL_ERROR_INVALID_TYPE : CPL_ERROR_NULL_INPUT;
            break;
    }
...
/*----------------------------------------------------------------------------*/
int
cpl_image_is_rejected(const cpl_image *im, cpl_size x, cpl_size y)
{
    /* Test entries */
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_image_count_rejected(const cpl_image *im)
{
    cpl_ensure(im != NULL, CPL_ERROR_NULL_INPUT, -1);
...
/*----------------------------------------------------------------------------*/
void
cpl_image_reject_(cpl_image *im, cpl_size x, cpl_size y)
{
    cpl_binary *pbpm;

    /* Create the bad pixels mask if empty */
    if (im->bpm == NULL)
        im->bpm = cpl_mask_new(im->nx, im->ny);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_reject(cpl_image *im, cpl_size x, cpl_size y)
{
    /* Test entries */
...
/*----------------------------------------------------------------------------*/
void
cpl_image_accept_(cpl_image *im, cpl_size x, cpl_size y)
{
    if (im->bpm != NULL) {
        /* Get the access to the data */
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_accept(cpl_image *im, cpl_size x, cpl_size y)
{
    /* Test entries */
    cpl_ensure_code(im != NULL, CPL_ERROR_NULL_INPUT);
    cpl_ensure_code(x >= 1, CPL_ERROR_ACCESS_OUT_OF_RANGE);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_accept_all(cpl_image *self)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_image_reject_from_mask(cpl_image *im, const cpl_mask *map)
{
    /* Test entries */
    cpl_ensure_code(im != NULL, CPL_ERROR_NULL_INPUT);
