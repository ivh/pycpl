...
/*---------------------------------------------------------------------------*/
cpl_error_code
cpl_fft_image_(cpl_image *self, const cpl_image *other, cpl_fft_mode mode)
{
    /*
...
/*---------------------------------------------------------------------------*/
cpl_error_code
cpl_fft_image__(cpl_image *self,
                ...
                cpl_boolean is_last)
{
...
#ifdef CPL_FFTWF_INSTALLED
        error =
            cpl_fft_image_float(self, other, mode, rigor, (fftwf_plan *)pplan,
...
#ifdef CPL_FFTW_INSTALLED
        error =
            cpl_fft_image_double(self, other, mode, rigor, (fftw_plan *)pplan,
                                 (fftw_complex **)pbufin,
    ...
    return cpl_error_set_message_(error,
                                  "mode=%d (%d). %d X %d (%s) => %d X"
                                  " %d (%s)",
                                  (int)mode, (int)is_last, nxin, nyin,
                                  cpl_type_get_name(typin), nxout, nyout,
                                  cpl_type_get_name(typout));
}
...
/*---------------------------------------------------------------------------*/
cpl_error_code
cpl_fft_image_cycle(cpl_image *self,
                    const cpl_image *other,
                    double xshift,
                    double yshift)
{
    ...
    if (nx < 1) {
        ...
        for (size_t j = 0; j < (size_t)ny; j++) {
            const double yscale =
                (j < (size_t)(ny + 1) / 2 ? -(double)j : (double)(ny - j)) *
                yshift / (double)ny;
            for (size_t i = 0; i < (size_t)nh; i++) {
                const double kx =
                    i < (size_t)nh ? -(double)i : (double)(nx - i);
...
/*----------------------------------------------------------------------------*/
static void *
cpl_fft_aligned(void *self, void *other, size_t align)
{
