...
/* gcc 3.3 or higher */
#define CPL_ATTR_ALWAYSINLINE __attribute__((always_inline))
#else
#define CPL_ATTR_ALWAYSINLINE /* __attribute__ */
#endif
...
/* Swap macros */
#define CPL_INT_SWAP(a, b)               \
    {                                    \
        register const cpl_size t = (a); \
        (a) = (b);                       \
        (b) = t;                         \
    }
...
/*----------------------------------------------------------------------------*/
inline void
cpl_ifalloc_set(cpl_ifalloc *self, size_t size)
{
...
/*----------------------------------------------------------------------------*/
inline void *
cpl_ifalloc_get(cpl_ifalloc *self)
{
    return self->data;
}
...
/*----------------------------------------------------------------------------*/
inline void
cpl_ifalloc_free(cpl_ifalloc *self)
{
    if (self->heap != NULL)
        cpl_free(self->heap); /* Typically NULL */
}
...
/*----------------------------------------------------------------------------*/
int
cpl_tools_get_bpp(cpl_type type)
{
    int bpp;
    ...
    switch (type & ~CPL_TYPE_POINTER & ~CPL_TYPE_FLAG_ARRAY) {
        ...
        default:
            bpp = 0;
            (void)cpl_error_set_message_(CPL_ERROR_INVALID_TYPE, "type=0x%x",
                                         (int)type);
    }
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_tools_is_power_of_2(cpl_size p)
{
    cpl_size ipow = 0;
    cpl_size done;
...
/*----------------------------------------------------------------------------*/
inline void
cpl_tools_add_flops_(cpl_flops flops)
{
#ifdef _OPENMP
#pragma omp atomic
#endif
...
/*----------------------------------------------------------------------------*/
cpl_vector *
cpl_vector_transform_mean(const cpl_vector *x, double *pm)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_vector_count_distinct(cpl_vector *self, cpl_size *pndistinct)
{
    if (pndistinct == NULL) {
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_vector_ensure_distinct(const cpl_vector *self, cpl_size ndistinct)
{
    if (ndistinct > 1) {
        const cpl_size n = cpl_vector_get_size(self);
        ...
        if (n < 1) {
            ...
            return cpl_error_set_message_(
                ...
                "vector cannot have at least %" CPL_SIZE_FORMAT
                " distinct values",
                n, ndistinct);
        }
        else {
            ...
            if (idistinct < ndistinct) {
                return cpl_error_set_message_(
                    ...
                    "must have at least %" CPL_SIZE_FORMAT
                    " (not %" CPL_SIZE_FORMAT ") distinct "
                    " values",
                    n, ndistinct, idistinct);
            }
        }
    }
    else if (ndistinct < 1) {
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_tools_copy_window(void *self,
                      const void *src,
                      size_t size,
                      cpl_size nx,
                      cpl_size ny,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_tools_shift_window(void *self,
                       size_t size,
                       cpl_size nx,
                       cpl_size ny,
                       int null,
                       ...
                       cpl_size dy)
{
    ...
    if (dx != 0) {
        ...
        if (dy <= 0) {
            for (j = -dy; j < ny;
                 j++, prdest += rsize, prsrc += rsize, prset += rsize) {
