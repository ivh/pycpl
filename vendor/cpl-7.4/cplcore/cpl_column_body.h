...
static void
ADDTYPE(cpl_column_pow)(cpl_column *column, double exponent)
{
    ...
    if (exponent == -0.5) {
        ...
        if (exponent != ceil(exponent)) {
            ...
            for (size_t i = 0; i < (size_t)length; i++) {
                if (np == NULL || np[i] == 0) {
                    ...
                    if (fp[i] != 0.0) {
                        value =
                            ELEM_CAST(cpl_tools_ipow((double)fp[i], exponent));
                    }
                    else {
                        errno = EDOM; /* Use errno to reuse code */
                    }
        ...
        for (size_t i = 0; i < (size_t)length; i++) {
            if (np == NULL || np[i] == 0) {
                fp[i] = ELEM_CAST(cpl_tools_ipow((double)fp[i], exponent));
            }
        }
    }
