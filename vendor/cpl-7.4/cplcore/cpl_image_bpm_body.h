...
ADDTYPE(cpl_image_reject_value)(cpl_image *self, cpl_value mode)
{
    CPL_TYPE *data = (CPL_TYPE *)cpl_image_get_data(self);
    cpl_binary *bpm = NULL;
    const size_t npix =
        (size_t)cpl_image_get_size_x(self) * (size_t)cpl_image_get_size_y(self);
    size_t i;
    ...
    for (i = 0; i < npix; i++) {
        ...
        if ((fptype == FP_ZERO && (mode & CPL_VALUE_ZERO)) ||
            ...
            (fptype == FP_INFINITE &&
             (((mode & CPL_VALUE_PLUSINF) &&
               ((mode & CPL_VALUE_MINUSINF) || data[i] > 0.0)) ||
              ((mode & CPL_VALUE_MINUSINF) && data[i] < 0.0)))) {
            if (bpm == NULL) {
                bpm = cpl_mask_get_data(cpl_image_get_bpm(self));
            }
