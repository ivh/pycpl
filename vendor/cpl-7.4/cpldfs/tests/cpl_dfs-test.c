...
static void cpl_dfs_product_test_one(const char *, const char *);
...
static void
cpl_dfs_propertylist_append_dicb(cpl_propertylist *) CPL_ATTR_NONNULL;
...
main(int argc, char *argv[])
{
    ...
    if (argc == 3) {
        cpl_dfs_product_test_one(argv[1], argv[2]);
    }
...
static void
cpl_dfs_product_tests(const char *rawname)
{
    cpl_frame *rawframe;
    cpl_frame *inhframe;
    cpl_frame *proframe;
    ...
    error = cpl_dfs_setup_product_header(NULL, NULL, NULL, NULL, NULL, NULL,
                                         NULL, NULL);
    ...
    error =
        cpl_propertylist_append_string(plist, "CHECKSUM", "BADAKIZUEUSKARAZ");
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    error = cpl_propertylist_set_comment(plist, "CHECKSUM", "HDU checksum");
    cpl_test_eq_error(error, CPL_ERROR_NONE);

    error = cpl_propertylist_append_string(plist, "DATASUM", "3141592653");
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    error =
        cpl_propertylist_set_comment(plist, "DATASUM", "data unit checksum");
    cpl_test_eq_error(error, CPL_ERROR_NONE);

    cpl_dfs_parameterlist_fill(parlist);

    error =
        cpl_dfs_setup_product_header(plist, proframe, frameset, parlist,
                                     "Recipe", "Pipeline", "PRO-1.16", NULL);
    ...
    /* Test with pre-existing CPL error - and key to be removed */
    error = cpl_propertylist_append_string(plist, "ORIGFILE", "/dev/null");
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    ...
    /* The function returns CPL_ERROR_NONE & the CPL error state is unchanged */
    cpl_test_eq_string(cpl_func, cpl_error_get_function());
    ...
    error =
        cpl_dfs_save_paf("IIINSTRUME", "RRRECIPE", plist, "cpl_dfs-test.paf");
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    ...
    cpl_test_zero(cpl_image_save(NULL, CPL_DFS_FITSFILE, CPL_TYPE_UCHAR, plist,
                                 CPL_IO_CREATE));
    ...
    */
    ...
    cpl_test_zero(cpl_image_save(image, CPL_DFS_FITSFILE, CPL_TYPE_UCHAR, plist,
                                 CPL_IO_CREATE));
    cpl_test_fits(CPL_DFS_FITSFILE);
    ...
    */
    ...
    for (i = 0; i < next; i++) {
        ...
        error = cpl_image_save(image,
                               i % 2 ? CPL_DFS_FITSFILE : "./" CPL_DFS_FITSFILE,
                               i >= 255 ? CPL_TYPE_INT : CPL_TYPE_UCHAR, plist,
                               CPL_IO_EXTEND);
        cpl_test_eq_error(error, CPL_ERROR_NONE);
...
static void
cpl_dfs_save_tests(const char *rawname)
{
    cpl_error_code error;
    ...
    cpl_test_zero(
        cpl_propertylist_append_string(qclist, CPL_DFS_PRO_CATG, "procat"));
    ...
    error =
        cpl_dfs_save_paf("INSTRUME", "recipe", qclist, "recipe" CPL_DFS_PAF);
    ...
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    cpl_test_zero(
        cpl_propertylist_append_double(qclist, "ESO QC DOUBLE", 1.0 / 3.0));
    error = cpl_propertylist_append_string(qclist, "ESO QC DROP", "image");
    cpl_test_eq_error(error, CPL_ERROR_NONE);

    cpl_test_zero(
        cpl_propertylist_set_comment(qclist, "ESO QC STRING", "string"));
    error = cpl_propertylist_set_comment(qclist, "ESO QC INT", "int");
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    cpl_test_zero(
        cpl_propertylist_set_comment(qclist, "ESO QC FLOAT", "float"));
    cpl_test_zero(
        cpl_propertylist_set_comment(qclist, "ESO QC DOUBLE", "double"));

    /* Make sure file does not exist */
    (void)remove("image" CPL_DFS_FITS);
    image = cpl_image_new(1, 41, CPL_TYPE_FLOAT);
    error = cpl_dfs_save_image(frames, NULL, parlist, frames, NULL, image,
                               CPL_TYPE_UCHAR, "recipe", qclist, remregexp,
                               "pipe_id", "image" CPL_DFS_FITS);
    ...
    error = cpl_dfs_save_image(frames, NULL, parlist, frames, NULL, NULL,
                               CPL_TYPE_UCHAR, "recipe", qclist, remregexp,
                               "pipe_id", "nullimage" CPL_DFS_FITS);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    cpl_test_eq(cpl_frameset_get_size(frames), ++framesetsize);
    ...
    if (cpl_test_get_failed() == 0 && cpl_msg_get_level() > CPL_MSG_INFO) {
...
static void
cpl_dfs_save_txt(const char *self)
{
    FILE *stream = fopen(self, "w");

    cpl_test_nonnull(stream);

    cpl_test_leq(1, fprintf(stream, "SIMPLE ASCII file - not FITS\n"));
...
/*----------------------------------------------------------------------------*/
static void
cpl_dfs_parameterlist_fill(cpl_parameterlist *self)
{
    cpl_error_code error;
    ...
    error =
        cpl_parameterlist_append(self,
                                 cpl_parameter_new_value("my_int", CPL_TYPE_INT,
                                                         "test integer",
                                                         "int context", 42));
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    ...
    /* Use the value 1/3 to see the accuracy in the FITS header */
    error =
        cpl_parameterlist_append(self, cpl_parameter_new_value("my_double",
                                                               CPL_TYPE_DOUBLE,
                                                               "test double",
                                                               "double context",
                                                               1.0 / 3.0));
    ...
    error =
        cpl_parameterlist_append(self, cpl_parameter_new_value("my_string",
                                                               CPL_TYPE_STRING,
                                                               "test string",
                                                               "string context",
                                                               "Hello, World"));
...
/*----------------------------------------------------------------------------*/
static void
cpl_dfs_propertylist_append_dicb(cpl_propertylist *self)
{
    cpl_error_code code;
    ...
    code =
        cpl_propertylist_append_string(self, "NAXISKEYS", "Nine letter card");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code = cpl_propertylist_append_string(self, "COMMENT", "Comment card");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code = cpl_propertylist_append_string(self, "HISTORY", "History card");
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code = cpl_propertylist_append_string(self, "ESO INS DATE", "2018-10-25");
    cpl_test_eq_error(code, CPL_ERROR_NONE);

    code =
        cpl_propertylist_append_string(self, "ESO TEL DID", "ESO-VLT-DIC.TCS");
    cpl_test_eq_error(code, CPL_ERROR_NONE);
    ...
    code = cpl_propertylist_append_string(self, "ESO DPR CATG", "CALIB");
    cpl_test_eq_error(code, CPL_ERROR_NONE);
}

static void
cpl_dfs_product_test_one(const char *infilename, const char *outfilename)
{
    const cpl_size next = cpl_fits_count_extensions(infilename);
    cpl_error_code code;

    cpl_assert(infilename != NULL);
    cpl_assert(outfilename != NULL);

    for (cpl_size i = 0; i <= next; i++) {
        cpl_propertylist *plist =
            i == 0 ? cpl_propertylist_load_regexp(infilename, i,
                                                  "^ESO |"
                                                  "^COMMENT|"
                                                  "^DATAMD5|"
                                                  "^PIPEFILE|"
                                                  ...
                                                  "^DATASUM",
                                                  1)
                   : cpl_propertylist_load(infilename, i);


        cpl_msg_info(cpl_func, "Reading %d", (int)i);

        if (i == 0) {
            code = cpl_propertylist_save(plist, outfilename, CPL_IO_CREATE);
            cpl_test_eq_error(code, CPL_ERROR_NONE);
        }
        else {
            cpl_table *mytab = cpl_table_load(infilename, i, 0);
            const double mymax = cpl_table_get_column_max(mytab, "Efficiency");

            /* Unitless */
            code = cpl_table_set_column_unit(mytab, "Efficiency", "");
            cpl_test_eq_error(code, CPL_ERROR_NONE);

            code = cpl_table_name_column(mytab, "Efficiency",
                                         "Quantum_Efficiency");
            cpl_test_eq_error(code, CPL_ERROR_NONE);

            code = cpl_table_duplicate_column(mytab, "Grating_Efficiency",
                                              mytab, "Quantum_Efficiency");
            cpl_test_eq_error(code, CPL_ERROR_NONE);

            cpl_msg_info(cpl_func, "Scaling efficiency from: %g", mymax);

            code = cpl_table_multiply_scalar(mytab, "Quantum_Efficiency",
                                             0.80 / mymax);
            cpl_test_eq_error(code, CPL_ERROR_NONE);

            code = cpl_table_multiply_scalar(mytab, "Grating_Efficiency",
                                             0.95 / mymax);
            cpl_test_eq_error(code, CPL_ERROR_NONE);

            code =
                cpl_table_save(mytab, NULL, plist, outfilename, CPL_IO_EXTEND);
            cpl_test_eq_error(code, CPL_ERROR_NONE);
