...
#define CPL_DFS_PRO_DID "PRO-1.17"
...
static cpl_error_code cpl_dfs_find_md5sum(char *, fitsfile *) CPL_ATTR_NONNULL;
...
cpl_dfs_paf_dump_string(const char *, const char *, const char *, FILE *);

static cpl_error_code
cpl_dfs_paf_dump_double(const char *, double, const char *, FILE *);

static cpl_error_code
cpl_dfs_paf_dump_int(const char *, cpl_size, const char *, FILE *);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_dfs_save_image(cpl_frameset *allframes,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_dfs_save_propertylist(cpl_frameset *allframes,
                          cpl_propertylist *header,
                          const cpl_parameterlist *parlist,
                          const cpl_frameset *usedframes,
                          const cpl_frame *inherit,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_dfs_save_imagelist(cpl_frameset *allframes,
                       cpl_propertylist *header,
                       const cpl_parameterlist *parlist,
                       const cpl_frameset *usedframes,
                       const cpl_frame *inherit,
                       ...
                       const char *filename)
{
    ...
    return cpl_dfs_product_save(allframes, header, parlist, usedframes, inherit,
                                imagelist, NULL, type, NULL, NULL, recipe,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_dfs_save_table(cpl_frameset *allframes,
                   cpl_propertylist *header,
                   const cpl_parameterlist *parlist,
                   const cpl_frameset *usedframes,
                   const cpl_frame *inherit,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_dfs_save_paf(const char *instrume,
                 const char *recipe,
...
cpl_error_code
cpl_dfs_setup_product_header(cpl_propertylist *header,
                             ...
                             const cpl_frame *inherit_frame)
{
    ...
    klist mandatory[] = {{CXSTR("ORIGIN", 6), CPL_TYPE_STRING},
                         ...
                         {CXSTR("RADESYS", 7), CPL_TYPE_STRING},
    ...
    /*
     * These are the mandatory keys to be loaded from the reference frames.
     * Included are ARCFILE and RADECSYS to make sure they are loaded and
     * available for internal processing here.
     */

    const cpl_cstr *exactkeys[] = {CXSTR("ORIGIN", 6),   CXSTR("TELESCOP", 8),
                                   CXSTR("INSTRUME", 8), CXSTR("OBJECT", 6),
                                   CXSTR("RA", 2),       CXSTR("DEC", 3),
                                   CXSTR("EPOCH", 5),    CXSTR("EQUINOX", 7),
                                   CXSTR("RADESYS", 7),  CXSTR("DATE-OBS", 8),
                                   CXSTR("MJD-OBS", 7),  CXSTR("UTC", 3),
                                   CXSTR("LST", 3),      CXSTR("PI-COI", 6),
                                   CXSTR("OBSERVER", 8), CXSTR("RADECSYS", 8),
                                   CXSTR("ARCFILE", 7)};

    const cpl_cstr *copykeys[] = {CXSTR("ESO PRO REC", 11)};
    const cpl_cstr *avoidkeys[] = {CXSTR("ESO DPR ", 8), CXSTR("ESO DRS ", 8),
                                   CXSTR("ESO PRO ", 8), CXSTR("ESO QC ", 7)};

    const cpl_cstr *erasekeys1[] = {CXSTR("ESO DPR ", 8)};
    const cpl_cstr *erasekeys2[] = {CXSTR("ARCFILE", 7), CXSTR("ORIGFILE", 8),
                                    CXSTR("CHECKSUM", 8), CXSTR("DATASUM", 7),
                                    CXSTR("HDRVER", 6)};

    /* The same list, except the last two elements are not used */
    const cpl_cstr **mandatorykeys = exactkeys;

    /* ESO HIERARCH keys are also to be loaded */
    const cpl_cstr *esokeys[] = {CXSTR("ESO ", 4)};

    const int ncopykeys = (int)CX_N_ELEMENTS(copykeys);
    const int navoidkeys = (int)CX_N_ELEMENTS(avoidkeys);
    const int nesokeys = (int)CX_N_ELEMENTS(esokeys);
    const int nexactkeys = (int)CX_N_ELEMENTS(exactkeys);
    const int nerasekeys1 = (int)CX_N_ELEMENTS(erasekeys1);
    const int nerasekeys2 = (int)CX_N_ELEMENTS(erasekeys2);
    const int nmandatorykeys = nexactkeys - 2;

    assert(count_mandatory == nmandatorykeys);
    assert(nexactkeys == nmandatorykeys + 2);
    assert(nesokeys == 1);
    ...
    cpl_propertylist_erase_name_(header, nerasekeys1, erasekeys1, nerasekeys2,
                                 erasekeys2, 0);
    ...
    if (inherit_frame) {
        ...
        if (frame != NULL) {
            ...
            return cpl_error_set_message_(CPL_ERROR_ILLEGAL_INPUT,
                                          "The reference frame does not belong "
                                          "to the input set-of-frames");
        }
    }
    else {
    ...
    switch (cpl_is_fits(cpl_frame_get_filename(first_frame))) {
        case 0:
            ...
            break;
        case 1:
            /* Load only the mandatory keywords and the HIERARCH ESO keywords */
            if (first_frame == inherit_frame) {
                plist = cpl_propertylist_load_name_(cpl_frame_get_filename(
                                                        first_frame),
                                                    0, nesokeys, esokeys,
                ...
                plist = cpl_propertylist_load_name_(cpl_frame_get_filename(
                                                        first_frame),
                                                    0, nmandatorykeys,
                                                    mandatorykeys, 0, NULL, 0);
                cpl_ensure_code(plist != NULL, cpl_error_get_code());
                plist2 = cpl_propertylist_load_name_(cpl_frame_get_filename(
                                                         inherit_frame),
                                                     0, nesokeys, esokeys,
                                                     nkeys, exactkey, 0);
                cpl_ensure_code(plist2 != NULL, cpl_error_get_code());
                code = cpl_propertylist_append(plist, plist2);
                cpl_propertylist_delete(plist2);
                cpl_ensure_code(!code, code);
            }
            break;
        default:
    ...
    /*
     * Handle the deprecated RADECSYS keyword. RADECSYS has been replaced
     * by RADESYS but in order to be backwards compatible the following
     * scheme is adopted:
     *
     * Both keyword names are loaded when the list of mandatory keywords is
     * loaded in the previous code section.
     *
     * If RADECSYS is present and RADESYS is not, RADECSYS is renamed to
     * RADESYS. If RADECSYS is not present but RADESYS is already there
     * nothing needs to be done.
     *
     * If both keywords are there it is assumed that RADESYS contains the
     * correct information. In this case, following the DICB policy, the
     * keyword RADECSYS is dropped and only RADESYS is copied to the product
     * header.
     */

    if (cpl_propertylist_has(plist, "RADECSYS")) {
        if (!cpl_propertylist_has(plist, "RADESYS")) {
            const cpl_property *_property =
                cpl_propertylist_get_property_const(plist, "RADECSYS");
            cpl_property *property = cpl_property_duplicate(_property);
            cpl_property_set_name(property, "RADESYS");
            cpl_propertylist_append_property(plist, property);
    ...
    for (i = 0; i < count_mandatory; i++) {
        const cpl_property *propin =
            cpl_propertylist_get_const_cx(plist, mandatory[i].key);
        if (propin != NULL) {
            const cpl_property *propout =
                cpl_propertylist_get_const_cx(header, mandatory[i].key);
            const cpl_type intype = cpl_property_get_type(propin);

            if (intype != mandatory[i].type) {
                cpl_msg_warning(cpl_func,
                                "Unexpected type for keyword %s in "
                                "input header: %s",
                                mandatory[i].key->data,
                                cpl_type_get_name(intype));
            }
            if (propout != NULL) {
                const cpl_type outtype = cpl_property_get_type(propout);

                if (outtype != mandatory[i].type) {
                    cpl_msg_warning(cpl_func,
                                    "Unexpected type for keyword %s "
                                    "in output header: %s",
                                    mandatory[i].key->data,
                                    cpl_type_get_name(outtype));
                }
                if (intype != outtype) {
    ...
    do {
        level++;
        keylen = snprintf(ckey, FLEN_KEYWORD, PRO_RECi_ID, level);
    } while (cpl_propertylist_has_cx(plist, CXSTR(ckey, keylen)));
    ...
    cpl_propertylist_copy_name_(header, plist, navoidkeys, avoidkeys, 0, NULL,
                                1);
    /* Copy the processing history */

    cpl_propertylist_copy_name_(header, plist, ncopykeys, copykeys, 0, NULL, 0);

    /* Copy or create the product's ancestor */

    if (!cpl_propertylist_has_cx(header, CXSTR(PRO_ANCESTOR, 16))) {
        const cpl_property *propin =
            cpl_propertylist_get_const_cx(plist, CXSTR(PRO_ANCESTOR, 16));

        if (propin != NULL) {
            cpl_propertylist_append_property(header, propin);
        }
        else {
            propin = cpl_propertylist_get_const_cx(plist, CXSTR(ARCFILE, 7));
            if (propin != NULL) {
                propupd = cpl_property_new_cx(CXSTR(PRO_ANCESTOR, 16),
                                              CPL_TYPE_STRING);
                cpl_property_set_string_cx(
                    propupd, CXSTR(cpl_property_get_string(propin),
                                   cpl_property_get_size(propin) - 1));
                cpl_propertylist_set_property(header, propupd);
            }
        }
    }
     ...
     */
    propupd = cpl_propertylist_update_string_(header, CXSTR(DATAMD5, 7),
                                              CXSTR("Not computed", 12));
    cpl_ensure_code(propupd != NULL, cpl_error_get_code());
    cpl_property_set_comment_cx(propupd, CXSTR("MD5 checksum", 12));
    ...
    propupd = cpl_propertylist_update_string_(header, CXSTR(PIPEFILE, 8),
                                              CXSTR(kname, strlen(kname)));
    cpl_ensure_code(propupd != NULL, cpl_error_get_code());
    cpl_property_set_comment_cx(propupd, CXSTR("Filename of data product", 24));

    /*
     * PRO DID
     */

    propupd = cpl_propertylist_update_string_(header, CXSTR(PRO_DID, 11),
                                              CXSTR(dictionary_id,
                                                    strlen(dictionary_id)));
    cpl_ensure_code(propupd != NULL, cpl_error_get_code());
    cpl_property_set_comment_cx(propupd, CXSTR("Data dictionary for PRO", 23));
    ...
    propupd = cpl_propertylist_update_string_(header, CXSTR(PRO_CATG, 12),
                                              CXSTR(kname, strlen(kname)));
    cpl_ensure_code(propupd != NULL, cpl_error_get_code());
    cpl_property_set_comment_cx(propupd,
    ...
    if (!cpl_propertylist_has_cx(header, CXSTR(PRO_TYPE, 12))) {
        propupd = cpl_property_new_cx(CXSTR(PRO_TYPE, 12), CPL_TYPE_STRING);
        cpl_property_set_string_cx(propupd, CXSTR("REDUCED", 7));
        cpl_property_set_comment_cx(propupd, CXSTR("Product type", 12));
        cpl_propertylist_set_property(header, propupd);
    }
    ...
    if (!cpl_propertylist_has_cx(header, CXSTR(PRO_TECH, 12))) {
        const cpl_property *propin = NULL;

        if (plist != NULL) {
            propin = cpl_propertylist_get_const_cx(plist, CXSTR(DPR_TECH, 12));
#ifdef CPL_FIX_PIPE_8268
            if (propin == NULL) {
                propin =
                    cpl_propertylist_get_const_cx(plist, CXSTR(PRO_TECH, 12));
            }
#endif
        }
        propupd = cpl_property_new_cx(CXSTR(PRO_TECH, 12), CPL_TYPE_STRING);
        cpl_property_set_string_cx(
            propupd, propin == NULL ? CXSTR("UNDEFINED", 9)
                                    : CXSTR(cpl_property_get_string(propin),
                                            cpl_property_get_size(propin) - 1));
        cpl_property_set_comment_cx(propupd,
    ...
    if (!cpl_propertylist_has_cx(header, CXSTR(PRO_SCIENCE, 15))) {
        ...
        if (plist != NULL) {
            const cpl_property *propin =
                cpl_propertylist_get_const_cx(plist, CXSTR(DPR_CATG, 12));
            if (propin != NULL &&
                cpl_property_get_type(propin) == CPL_TYPE_STRING &&
                cpl_property_get_size(propin) - 1 == 7 &&
                !memcmp(cpl_property_get_string(propin), "SCIENCE", 7)) {
                pro_science = !0;
#ifdef CPL_FIX_PIPE_8268
            }
            else {
                propin = cpl_propertylist_get_const_cx(plist,
                                                       CXSTR(PRO_SCIENCE, 15));
                if (propin != NULL &&
                    cpl_property_get_type(propin) == CPL_TYPE_BOOL) {
                    pro_science = cpl_property_get_bool(propin);
                }
#endif
            }
        }
        propupd = cpl_property_new_cx(CXSTR(PRO_SCIENCE, 15), CPL_TYPE_BOOL);
        cpl_property_set_bool(propupd, pro_science);
        cpl_property_set_comment_cx(propupd,
                                    CXSTR("Scientific product if T", 23));
        cpl_propertylist_set_property(header, propupd);
    }
    ...
    keylen = snprintf(ckey, FLEN_KEYWORD, PRO_RECi_ID, level);
    propupd = cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                              CXSTR(recid, strlen(recid)));
    cpl_ensure_code(propupd != NULL, cpl_error_get_code());
    cpl_property_set_comment_cx(propupd,
    ...
    keylen = snprintf(ckey, FLEN_KEYWORD, PRO_RECi_DRS_ID, level);
    propupd =
        cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                        CXSTR(PACKAGE "-" PACKAGE_VERSION,
                                              strlen(PACKAGE
                                                     "-" PACKAGE_VERSION)));
    cpl_ensure_code(propupd != NULL, cpl_error_get_code());
    cpl_property_set_comment_cx(propupd,
                                CXSTR("Data Reduction System identifier", 32));

    /*
     * PRO REC1 PIPE ID
     */

    keylen = snprintf(ckey, FLEN_KEYWORD, PRO_RECi_PIPE_ID, level);
    propupd = cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                              CXSTR(pipeline_id,
                                                    strlen(pipeline_id)));
    cpl_ensure_code(propupd != NULL, cpl_error_get_code());
    ...
    while (frame != NULL) {
        ...
        if (cpl_frame_get_group(frame) == CPL_FRAME_GROUP_RAW) {
            ...
            const char *basename = cpl_get_base_name(filename);
            const char *tagname = cpl_frame_get_tag(frame);

            ++nraw;

            keylen =
                snprintf(ckey, FLEN_KEYWORD, PRO_RECi_RAWi_NAME, level, nraw);
            propupd =
                cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                                CXSTR(basename,
                                                      strlen(basename)));
            ...
            keylen =
                snprintf(ckey, FLEN_KEYWORD, PRO_RECi_RAWi_CATG, level, nraw);
            propupd =
                cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                                CXSTR(tagname,
                                                      strlen(tagname)));
    ...
    if (!cpl_propertylist_has_cx(header, CXSTR(PRO_DATANCOM, 16))) {
        propupd = cpl_property_new_cx(CXSTR(PRO_DATANCOM, 16), CPL_TYPE_INT);
        cpl_property_set_int(propupd, nraw);
        cpl_property_set_comment_cx(propupd,
    ...
    while (frame != NULL) {
        ...
        if (cpl_frame_get_group(frame) == CPL_FRAME_GROUP_CALIB) {
            ...
            keylen =
                snprintf(ckey, FLEN_KEYWORD, PRO_RECi_CALi_NAME, level, ncal);
            propupd =
                cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                                CXSTR(basename,
                                                      strlen(basename)));
            ...
            cpl_property_set_comment_cx(propupd,
                                        CXSTR("File name of calibration frame",
                                              30));

            keylen =
                snprintf(ckey, FLEN_KEYWORD, PRO_RECi_CALi_CATG, level, ncal);
            propupd =
                cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                                CXSTR(tagname,
                                                      strlen(tagname)));
            ...
            cpl_property_set_comment_cx(propupd,
                                        CXSTR("Category of calibration frame",
                                              29));

            switch (cpl_is_fits(filename)) {
                case 0:
                    break;
                case 1:
                    plist = cpl_propertylist_load_name_(filename, 0, 0, NULL,
                                                        nkeys, exactkey, 0);
                    if (plist == NULL) {
                        cpl_frameset_iterator_delete(it);
                        return cpl_error_set_where_();
                        ...
                        const cpl_property *propin =
                            cpl_propertylist_get_const_cx(plist,
                                                          CXSTR(DATAMD5, 7));

                        if (propin != NULL) {
                            if (cpl_property_get_type(propin) !=
                                CPL_TYPE_STRING) {
                                cpl_msg_warning(cpl_func,
                                                "Card " DATAMD5 " in %s "
                                                "has non-string type: %s",
                                                filename,
                                                cpl_type_get_name(
                                ...
                                keylen = snprintf(ckey, FLEN_KEYWORD,
                                                  PRO_RECi_CALi_DATAMD5, level,
                                                  ncal);
                                propupd = cpl_propertylist_update_string_(
                                    header, CXSTR(ckey, keylen),
                                    CXSTR(cpl_property_get_string(propin),
                                          cpl_property_get_size(propin) - 1));
                                ...
                                cpl_property_set_comment_cx(
                                    propupd, CXSTR("MD5 signature of "
                                                   "calib frame",
                                                   28));
                            }
                        }
    ...
    while (param != NULL) {
        ...
        switch (cpl_parameter_get_type(param)) {
            case CPL_TYPE_BOOL:
                pval = cpl_strdup(cpl_parameter_get_bool(param) == 1 ? "true"
                                                                     : "false");
            ...
            case CPL_TYPE_DOUBLE:
                ...
                dval = cpl_sprintf("Default: %g%n",
                                   ...
                                   &dvallen);
                if (!comment)
                    comment = "Floating point recipe parameter";
                break;
            case CPL_TYPE_STRING:
                ...
                dval = cpl_sprintf("Default: '%s'%n",
                                   ...
                                   &dvallen);
                if (!comment)
                    comment = "String recipe parameter";
                break;
            default:
        ...
        keylen =
            snprintf(ckey, FLEN_KEYWORD, PRO_RECi_PARAMi_NAME, level, npar);
        propupd = cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                                  CXSTR(kname, strlen(kname)));
        cpl_ensure_code(propupd != NULL, cpl_error_get_code());
        cpl_property_set_comment(propupd, comment);

        keylen =
            snprintf(ckey, FLEN_KEYWORD, PRO_RECi_PARAMi_VALUE, level, npar);
        propupd = cpl_propertylist_update_string_(header, CXSTR(ckey, keylen),
                                                  CXSTR(pval, strlen(pval)));
        cpl_ensure_code(propupd != NULL, cpl_error_get_code());
        cpl_property_set_comment_cx(propupd, CXSTR(dval, dvallen));
    ...
    /*
     * Final consistency checks of the created, finalized product header.
     */

    if (cpl_propertylist_has(header, "RADECSYS")) {
        cpl_msg_warning(cpl_func, "Product header contains deprecated "
                                  "keyword 'RADECSYS'!");
    }
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_dfs_update_product_header(cpl_frameset *self)
{
    return cpl_dfs_update_product_header_(self) |
                   cpl_io_fits_close_tid(
                       CPL_IO_FITS_ALL) /* FIXME: Close only writers */
               ? cpl_error_set_where_()
               : CPL_ERROR_NONE;
}
...
_cpl_dfs_sign_product(const cpl_frame *frame, cxuint flags)
{
    ...
    if (cpl_is_fits(filename)) {
        ...
        if (cpl_io_fits_open_diskfile(&fptr, filename, READWRITE, &_status)) {
            return cpl_error_set_fits(CPL_ERROR_BAD_FILE_FORMAT, _status,
        ...
        if (_status) {
            return cpl_error_set_fits(CPL_ERROR_FILE_IO, _status,
        ...
        if (flags & CPL_DFS_SIGNATURE_DATAMD5) {
            ...
            fits_update_key_str(fptr, DATAMD5, md5sum, "MD5 checksum",
                                &_status);

            if (_status) {
                return cpl_error_set_fits(CPL_ERROR_FILE_IO, _status,
                                          fits_update_key_str,
                                          "filename='%s', key='" DATAMD5
                                          "', value='%s'",
                                          filename, md5sum);
            }
        }
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_dfs_update_product_header_(cpl_frameset *self)
{
    const cpl_frame *frame;
                ...
                if (fits_read_card(fproduct, DATAMD5, card, &error)) {
                    ...
                    if (cpl_dfs_find_md5sum(md5sum, fproduct)) {
                        ...
                        if (fits_update_key_str(fproduct, DATAMD5, md5sum,
                                                "MD5 checksum", &error)) {
                            code = cpl_error_set_fits(
                                CPL_ERROR_FILE_IO, error, fits_update_key_str,
                                "filename='%s', key='" DATAMD5 "', value='%s'",
                                filename, md5sum);
                            error = 0;
                        }
                            ...
                            code = cpl_error_set_fits(
                                CPL_ERROR_FILE_IO, error, fits_update_key_str,
                                "filename='%s', key='" PIPEFILE "'", filename);
                            error = 0;
                        }
                        ...
                        if (!fits_find_nextkey(fproduct, &inclist, 1, NULL, 0,
                                               card, &error)) {
                            if (fits_write_chksum(fproduct, &error)) {
                                code = cpl_error_set_fits(
                                    CPL_ERROR_FILE_IO, error,
                                    fits_update_key_str,
                                    "filename='%s', key='" CHECKSUM "'",
                                    filename);
                                error = 0;
                            }
                        }
                            ...
                            code = cpl_error_set_fits(
                                CPL_ERROR_FILE_IO, error, fits_update_key_str,
                                "filename='%s', key='" CHECKSUM "'", filename);
                            error = 0;
                        }
                    }
                }
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_dfs_find_md5sum(char *md5sum, fitsfile *fproduct)
{
    ...
    */
    ...
    do {
        ...
        while (datapos < dataend) {
            ...
            MD5Update(&ctx, (const unsigned char *)buffer, (unsigned)blocksize);
        ...
        if (dataend != datapos) {
            ...
            return cpl_error_set_message_(CPL_ERROR_BAD_FILE_FORMAT,
                                          "HDU#=%d, datapos=%ld != "
                                          "dataend=%ld, sizeof(" CPL_STRINGIFY(CPL_OFF_TYPE) ")=%zu", next,
                                              (long)datapos, (long)dataend,
                                              sizeof(CPL_OFF_TYPE));
        }
        next++;
    ...
    if (buffer != NULL) {
        ...
        (void)memcpy(md5sum, "d41d8cd98f00b204e9800998ecf8427e", 1 + MD5HASHSZ);
    }
    ...
    if (fits_movabs_hdu(fproduct, 1, NULL, &error)) {
        return cpl_error_set_fits(CPL_ERROR_FILE_IO, error, fits_movabs_hdu,
                                  "HDU#=%d", next);
    }
...
/*----------------------------------------------------------------------------*/
static const char *
cpl_get_base_name(const char *self)
{
...
/*----------------------------------------------------------------------------*/
static FILE *
cpl_dfs_paf_init(const char *instrume, const char *recipe, const char *filename)
{
    FILE *paf = NULL;
    char *paf_id = NULL;
    ...
    if (error) {
        ...
        (void)cpl_error_set_message_(error,
                                     "Could not write PAF, instrume=%s, "
                                     "recipe=%s, filename=%s",
                                     instrume, recipe, filename);
    }
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_dfs_paf_dump(const cpl_propertylist *self, FILE *paf)
{
    ...
    for (i = 0; i < nproperties; i++) {
        ...
        switch (cpl_property_get_type(prop)) {
            ...
            case CPL_TYPE_LONG_LONG:
                err = cpl_dfs_paf_dump_int(usekey,
                                           cpl_property_get_long_long(prop),
                                           usecom, paf);
                break;
            case CPL_TYPE_FLOAT:
                err = cpl_dfs_paf_dump_double(usekey,
            ...
            case CPL_TYPE_DOUBLE:
                ...
                break;
            case CPL_TYPE_STRING:
                err = cpl_dfs_paf_dump_string(usekey,
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_dfs_paf_dump_string(const char *key,
                        const char *value,
                        const char *comment,
                        FILE *paf)
{
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_dfs_paf_dump_double(const char *key,
                        double value,
                        const char *comment,
                        FILE *paf)
{
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_dfs_paf_dump_int(const char *key,
                     cpl_size value,
                     const char *comment,
                     FILE *paf)
{
    ...
    if (comment == NULL)
        ...
        cpl_ensure_code(fprintf(paf,
                                PAF_KEY_FORMAT "%" CPL_SIZE_FORMAT "; # %s\n",
...
static cpl_error_code
cpl_dfs_product_save(cpl_frameset *allframes,
                     cpl_propertylist *header,
                     const cpl_parameterlist *parlist,
                     const cpl_frameset *usedframes,
                     const cpl_frame *inherit,
                     ...
                     const char *filename)
{
    ...
    /* Inside this function the product-types are numbered:
       ...
       3: propertylist only
    */
    const unsigned pronum = imagelist != NULL ? 0
                            : table != NULL   ? 1
                                              : (image != NULL ? 2 : 3);
    const char *proname[] = {"imagelist", "table", "image", "propertylist"};
    ...
    if (!error) {
        switch (pronum) {
            ...
            case 2:
                error =
                    cpl_image_save(image, filename, type, plist, CPL_IO_CREATE);
                break;
            default:
                /* case 3: */
...
static char *
cpl_dfs_extract_printable(const char *self)
{
    char *card = NULL;
