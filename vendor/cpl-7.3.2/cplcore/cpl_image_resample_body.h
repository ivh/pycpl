...
case CPL_TYPE_T: {
    ...
    for (j = ystart - 1; j < ny; j++) {
        for (i = xstart - 1; i < nx; i++) {
            po[(i - xstart + 1) / xstep +
               ((j - ystart + 1) / ystep) * new_nx] += pi[i + j * old_nx];
        }
    }
} break;
...
case CPL_TYPE_T:
    ...
    for (cpl_size j = 0; j < out->ny; j++) {
        for (cpl_size i = 0; i < out->nx; i++) {
            ...
            const double value =
                CPL_TYPE_INTERPOLATE((const CPL_TYPE *)in->pixels, inbpm,
                                     in->nx, in->ny, xtabsperpix, ytabsperpix,
                                     x, y, yweight, pxprof, xradius, pyprof,
                                     yradius, sqyradius, sqyxratio,
                                     &confidence);
...
case CPL_TYPE_T:
    ...
    for (j = 0; j < out->ny; j++) {
        ...
        for (i = 0; i < out->nx; i++) {
            ...
            value = CPL_TYPE_INTERPOLATE((const CPL_TYPE *)in->pixels, inbpm,
                                         in->nx, in->ny, xtabsperpix,
                                         ytabsperpix, x, y, yweight, pxprof,
                                         xradius, pyprof, yradius, sqyradius,
                                         sqyxratio, &confidence);
                                    ...
                                    double *pconfid)
{
    ...
    for (intptr_t y = yfirst; y <= ylast; y++)
        yweight[(size_t)(y - yfirst)] =
            pyprof[(size_t)(fabs((double)y - ypos) * ytabsperpix + 0.5)];
...
#ifdef CPL_ADD_FLOPS
    cpl_tools_add_flops(3 * (ylast - yfirst + 1) + 6 * (xlast - xfirst + 1) +
                        11 * (ylast - yfirst + 1) * (xlast - xfirst + 1) -
                        skipped);
#endif
