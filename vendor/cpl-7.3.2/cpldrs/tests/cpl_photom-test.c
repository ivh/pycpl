...
static void cpl_photom_fill_blackbody_test(cpl_vector *,
                                           cpl_unit,
                                           const cpl_vector *,
                                           cpl_unit,
                                           double,
...
main(void)
{
    ...
    i++;
    error =
        cpl_polynomial_set_coeff(poly1, &i, (WL_MAX - WL_MIN) / FUNCTION_SIZE);
    cpl_test_eq_error(error, CPL_ERROR_NONE);

    fun = cpl_bivector_new(FUNCTION_SIZE);
    cpl_test_nonnull(fun);

    error = cpl_vector_fill_polynomial(cpl_bivector_get_x(fun), poly1, 0, 1);
    cpl_test_eq_error(error, CPL_ERROR_NONE);

    cpl_photom_fill_blackbody_test(cpl_bivector_get_y(fun),
                                   CPL_UNIT_PHOTONRADIANCE,
                                   cpl_bivector_get_x(fun), CPL_UNIT_LENGTH,
                                   temp, __LINE__);
    ...
    cpl_photom_fill_blackbody_test(cpl_bivector_get_y(fun),
                                   CPL_UNIT_PHOTONRADIANCE,
                                   cpl_bivector_get_x(fun), CPL_UNIT_FREQUENCY,
                                   temp, __LINE__);
    ...
    error = cpl_photom_fill_blackbody(vxc, CPL_UNIT_LESS,
                                      cpl_bivector_get_x_const(trans),
                                      CPL_UNIT_LENGTH, temp);
    ...
    error =
        cpl_photom_fill_blackbody(cpl_bivector_get_y(trans), CPL_UNIT_LENGTH,
                                  cpl_bivector_get_x_const(trans),
...
/*----------------------------------------------------------------------------*/
static void
cpl_photom_units_test(void)
{
    const cpl_unit units[] = {
        CPL_UNIT_LESS,   CPL_UNIT_RADIAN,         CPL_UNIT_LENGTH,
        CPL_UNIT_TIME,   CPL_UNIT_PERLENGTH,      CPL_UNIT_FREQUENCY,
        CPL_UNIT_MASS,   CPL_UNIT_ACCELERATION,   CPL_UNIT_FORCE,
        CPL_UNIT_ENERGY, CPL_UNIT_PHOTONRADIANCE, CPL_UNIT_ENERGYRADIANCE
    };
...
/*----------------------------------------------------------------------------*/
static void
cpl_photom_fill_blackbody_test(cpl_vector *spectrum,
                               cpl_unit out_unit,
                               ...
                               unsigned line)
{
    ...
    const cpl_error_code error =
        cpl_photom_fill_blackbody(spectrum, out_unit, evalpoints, in_unit,
                                  temp);
    cpl_test_eq_error(error, CPL_ERROR_NONE);
    ...
    if (error != CPL_ERROR_NONE) {
        ...
        if (in_unit == CPL_UNIT_LENGTH) {
            ...
            cpl_test_lt(0.0, wl[0]);
            for (cpl_size i = 1; i < fsize; i++) {
            ...
            cpl_test_lt(0.0, wl[fsize - 1]);
        }
    }
    else if (out_unit == CPL_UNIT_PHOTONRADIANCE) {
        const double *sp = cpl_vector_get_data_const(spectrum);
        const double *wl = cpl_vector_get_data_const(evalpoints);

        cpl_boolean has_peak = CPL_FALSE; /* FIXME: Find & use known peak */
