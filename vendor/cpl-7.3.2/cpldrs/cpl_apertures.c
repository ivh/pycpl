...
/*----------------------------------------------------------------------------*/
void
cpl_apertures_delete(cpl_apertures *self)
{
    if (self != NULL) {
        cpl_free(self->x);
        cpl_free(self->y);
...
/*----------------------------------------------------------------------------*/
void
cpl_apertures_dump(const cpl_apertures *self, FILE *fp)
{
    if (self != NULL && fp != NULL && self->naperts > 0) {
        ...
        for (i = 0; i < self->naperts; i++) {
            fprintf(fp, "% 3" CPL_SIZE_FORMAT " %6.1f %6.1f", i + 1, self->x[i],
                    self->y[i]);
            fprintf(fp, "    %6.1f    %6.1f", self->xcentroid[i],
                    self->ycentroid[i]);
...
/**
  ...
  @param    in_image Reference image
...
/*----------------------------------------------------------------------------*/
cpl_apertures *
cpl_apertures_new_from_image(const cpl_image *in_image, const cpl_image *lab)
{
    cpl_apertures *aperts =
        cpl_apertures_new_from_image_(in_image, lab, 0, CPL_TRUE);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_size(const cpl_apertures *self)
{
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_pos_x(const cpl_apertures *self, cpl_size ind)
{
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_pos_y(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2.0);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3.0);
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_max_x(const cpl_apertures *self, cpl_size ind)
{
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_max_y(const cpl_apertures *self, cpl_size ind)
{
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_centroid_x(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2.0);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3.0);
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_centroid_y(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2.0);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3.0);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_maxpos_x(const cpl_apertures *self, cpl_size ind)
{
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_maxpos_y(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_minpos_x(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_minpos_y(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_npix(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_left(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_left_y(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_right(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_right_y(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_bottom_x(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_bottom(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_top_x(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
cpl_size
cpl_apertures_get_top(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3);
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_max(const cpl_apertures *self, cpl_size ind)
{
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_min(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, 0.0);
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_mean(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, 0.0);
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_median(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, 0.0);
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_stdev(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, -1.0);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, -2.0);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, -3.0);
...
/*----------------------------------------------------------------------------*/
double
cpl_apertures_get_flux(const cpl_apertures *self, cpl_size ind)
{
    cpl_ensure(self != NULL, CPL_ERROR_NULL_INPUT, 0.0);
    cpl_ensure(ind > 0, CPL_ERROR_ILLEGAL_INPUT, 0.0);
    cpl_ensure(ind <= self->naperts, CPL_ERROR_ACCESS_OUT_OF_RANGE, 0.0);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_apertures_sort_by_npix(cpl_apertures *self)
{
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_apertures_sort_by_max(cpl_apertures *self)
{
    cpl_apertures *sorted_apert;
    void *swap;
    cpl_size naperts;
    ...
    for (j = 0; j < naperts; j++) {
        ...
        for (i = 0; i < naperts; i++) {
            if ((computed[i] == 0) &&
                (max_ind < 0 || max_max < self->max_val[i])) {
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_apertures_sort_by_flux(cpl_apertures *self)
{
    cpl_apertures *sorted_apert;
    void *swap;
    cpl_size naperts;
    ...
    for (j = 0; j < naperts; j++) {
        ...
        for (i = 0; i < naperts; i++) {
            if ((computed[i] == 0) &&
                (max_ind < 0 || max_flux < self->flux[i])) {
...
/**
  ...
  @param    in_image    The image to process
...
/*----------------------------------------------------------------------------*/
cpl_apertures *
cpl_apertures_extract(const cpl_image *in_image,
                      ...
                      cpl_size *pisigma)
{
    ...
    cpl_ensure(in_image != NULL, CPL_ERROR_NULL_INPUT, NULL);
    ...
    for (i = 0; i < n; i++) {
        ...
        /* Apply the detection  */
        aperts = cpl_apertures_extract_sigma(in_image, sigma);
        if (aperts != NULL)
            break;
    }
...
/*----------------------------------------------------------------------------*/
cpl_apertures *
cpl_apertures_extract_window(const cpl_image *in_image,
                             const cpl_vector *sigmas,
                             cpl_size llx,
                             cpl_size lly,
                             cpl_size urx,
                             ...
                             cpl_size *pisigma)
{
    ...
    extracted = cpl_image_extract(in_image, llx, lly, urx, ury);
...
/*----------------------------------------------------------------------------*/
cpl_apertures *
cpl_apertures_extract_mask(const cpl_image *in_image, const cpl_mask *selection)
{
    cpl_errorstate prestate = cpl_errorstate_get();
    cpl_apertures *aperts = NULL;
    cpl_size nlabels;
    ...
    if (labels != NULL && nlabels > 0) {
        ...
        aperts =
            cpl_apertures_new_from_image_(in_image, labels, nlabels, CPL_FALSE);
    }
...
/*----------------------------------------------------------------------------*/
cpl_apertures *
cpl_apertures_extract_sigma(const cpl_image *in_image, double sigma)
{
    ...
    median = cpl_image_get_median_dev(in_image, &med_dist);
    threshold = median + sigma * med_dist;

    /* Binarise the image */
    selection = cpl_mask_threshold_image_create(in_image, threshold, DBL_MAX);
    if (selection != NULL) {
        ...
        const int error = cpl_mask_not(kernel) ||
                          cpl_mask_filter(selection, selection, kernel,
                                          CPL_FILTER_OPENING, CPL_BORDER_ZERO);
        cpl_mask_delete(kernel);

        if (!error) {
            /* Create the detected apertures list */
            aperts = cpl_apertures_extract_mask(in_image, selection);
        }
...
/*----------------------------------------------------------------------------*/
cpl_bivector *
cpl_apertures_get_fwhm(const cpl_image *in_image, const cpl_apertures *aperts)
{
...
cpl_apertures_new(cpl_size naperts)
{
    cpl_apertures *aperts;
...
/*----------------------------------------------------------------------------*/
cpl_apertures *
cpl_apertures_new_from_image_(const cpl_image *in_image,
                              const cpl_image *lab,
                              cpl_size maperts,
                              cpl_boolean bpm_check)
{
    ...
    double *medians;
    cpl_size *medianc;
    const cpl_size nx = cpl_image_get_size_x(in_image);
    const cpl_size ny = cpl_image_get_size_y(in_image);
    ...
    cpl_ensure(cpl_image_get_type(lab) == CPL_TYPE_INT, CPL_ERROR_TYPE_MISMATCH,
               NULL);
    cpl_ensure(nx == cpl_image_get_size_x(lab), CPL_ERROR_INCOMPATIBLE_INPUT,
               NULL);
    cpl_ensure(ny == cpl_image_get_size_y(lab), CPL_ERROR_INCOMPATIBLE_INPUT,
               NULL);

    if (bpm_check) {
        const cpl_mask *bpm = cpl_image_get_bpm_const(in_image);
    ...
    dimage = cpl_image_get_type(in_image) == CPL_TYPE_DOUBLE
                 ? in_image
                 : (cimage = cpl_image_cast(in_image, CPL_TYPE_DOUBLE));
    ...
    for (k = 0; k < aperts->naperts; k++) {
        ...
        if (npix <= 0) {
            ...
            (void)cpl_error_set_message_(CPL_ERROR_DATA_NOT_FOUND,
                                         ...
                                         "has no pixel labeled %d",
                                         (int)nx, (int)ny, (int)naperts,
                                         (int)k);
            return NULL;
        }
        ...
        aperts->stdev[k] =
            npix > 1 ? sqrt(aperts->varsum[k] / ((double)npix - 1.0)) : -1.0;
    ...
    medians =
        cpl_malloc((size_t)medianc[aperts->naperts - 1] * sizeof(*medians));
