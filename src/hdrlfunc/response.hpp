...
public:
    ...
    std::string repr() const {
        try {
            return "ResponseResult(best_telluric_model_idx=" + std::to_string(get_best_telluric_model_idx()) +
...
private:
    std::unique_ptr<hdrl_response_result, void(*)(hdrl_response_result*)> result_;
...
public:
    ...
    static hdrl::core::Parameter velocity_parameter_create(
        double wguess,
        double range_wmin,
        double range_wmax,
        double fit_wmin,
    ...
    static hdrl::core::Parameter calc_parameter_create(
        double Ap,
        double Am,
        double G,
        double Tex);

    static hdrl::core::Parameter telluric_evaluation_parameter_create(
    ...
    static hdrl_parameter* fit_parameter_create(
        size_t radius,
        const cpl_array* fit_points,
        double wrange,
        const cpl_bivector* high_abs_regions);

    static hdrl::core::Parameter fit_parameter_create(
        size_t radius,
        py::array fit_points,
        double wrange,
    ...
    static void validate_input_spectra(
        const hdrl::core::Spectrum1D& obs_s,
        const hdrl::core::Spectrum1D& ref_s,
        const hdrl::core::Spectrum1D& E_x) {
        ...
        if (obs_s.size() == 0 || ref_s.size() == 0 || E_x.size() == 0) {
