...
namespace core
{
// The following 2 declarations/definitions are
 ...
 *        generate hdrl::core::Errors from the input error (using
 * cpl_error_getters).
 */
static void
dumper_function(unsigned current, unsigned first, unsigned last)
{
...
{
  switch (code) {
#define HDRL_MAKE_ERROR_CALLBACK(CODE, SUPER_EXC, CPPNAME, ERROR_DESCRIPTION) \
  case CODE: return new CPPNAME(function_name, file_name, line, error_message);

    HDRL_EXCEPTION_ENUMERATOR(HDRL_MAKE_ERROR_CALLBACK)
...
Error::make_trace(std::vector<ErrorFrame>&& chronological_errors)
{
  ...
  switch (code) {
#define HDRL_MAKE_TRACE_CALLBACK(CODE, SUPER_EXC, CPPNAME, ERROR_DESCRIPTION) \
  case CODE: return new CPPNAME(std::move(chronological_errors));

    HDRL_EXCEPTION_ENUMERATOR(HDRL_MAKE_TRACE_CALLBACK)
...
#define HDRL_MAKE_COPY_CALLBACK(CODE, SUPER_EXC, CPPNAME, ERROR_DESCRIPTION) \
  case CODE: return new CPPNAME(reinterpret_cast<const CPPNAME&>(other));

    HDRL_EXCEPTION_ENUMERATOR(HDRL_MAKE_COPY_CALLBACK)
    ...
    : m_errors(chronological_errors)
{
  ...
  // Like python
  error_message << "Most recent error last:\n";
...
#define HDRL_EXCEPTION_EXPLICIT_INSTANTIATION_DEFINITION_CALLBACK( \
    CODE, SUPER_EXC, CPPNAME, ERROR_DESCRIPTION)                   \
  template class ErrorInstance<CODE, SUPER_EXC>;


HDRL_EXCEPTION_ENUMERATOR(
    HDRL_EXCEPTION_EXPLICIT_INSTANTIATION_DEFINITION_CALLBACK)
