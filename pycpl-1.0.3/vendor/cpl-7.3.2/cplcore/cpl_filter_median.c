...
typedef struct ADDTYPE(dheap)
{
    ...
    */
    unsigned
        *B; /* Size Nx x Ny map from image positions to double heap positions.
                 ...
                 */
    unsigned *
        SCA; /* Array Nx*(2Ry+1) of sorted column arrays. Values are image pixel
 ...
 */
static void
ADDTYPE(qsort_int)(unsigned *SCA, unsigned n, const PIX_TYPE *image)
{
    int i, ir, j, k, l;
    int i_stack[BITS];
    int j_stack;
    ...
    while (1) {
        if (ir - l < 7) {
            ...
            for (;;) {
                do
                    i++;
                while (image[SCA[i - 1]] < ia);
                do
                    j--;
                while (image[SCA[j - 1]] > ia);
                if (j < i)
                    break;
                S_SWAP(SCA, i - 1, j - 1);
            }
...
ADDTYPE(SCA_init)(ADDTYPE(dheap) * dh, unsigned x)
{
    ...
    ADDTYPE(qsort_int)(SCAx, dh->Ry, dh->image);
...
*/
static void
ADDTYPE(SCA_replace)(ADDTYPE(dheap) * dh,
                     unsigned x,
                     unsigned yold,
                     unsigned ynew)
{
    ...
    */
    if (i + 1 < n && im[SCAx[i]] > im[SCAx[i + 1]]) {
        S_SWAP(SCAx, i, i + 1);
        i++;

        while (i + 1 < n && im[SCAx[i]] > im[SCAx[i + 1]]) {
            S_SWAP(SCAx, i, i + 1);
            i++;
        }
    }
    else {
        while (i >= 1 && im[SCAx[i]] < im[SCAx[i - 1]]) {
            S_SWAP(SCAx, i, i - 1);
            i--;
        }
    }
...
*/
static void
ADDTYPE(HEAP_SWAP)(unsigned i, unsigned j, unsigned int *B, unsigned int *heaps)
{
    unsigned ai = heaps[i];
    unsigned aj = heaps[j];
    heaps[i] = aj;
...
ADDTYPE(median)(ADDTYPE(dheap) * dh)
{
    ...
    while (lo < hi) {
        ...
        do {
            while (dh->image[dh->heaps[i]] < pivot)
                i++;
            while (dh->image[dh->heaps[j]] > pivot)
                j--;
            if (i <= j) {
                if (i != j)
                    ADDTYPE(HEAP_SWAP)(i, j, dh->B, dh->heaps);
                i++;
                j--;
            }
        } while (i <= j);
                        ...
                        unsigned root)
{
    ...
    while (root * 2 + 1 <= end) {
        ...
        if (child < end &&
            image[heaps[m1 + child]] > image[heaps[m1 + child + 1]]) {
            child = child + 1;
        }
        if (image[heaps[m1 + root]] > image[heaps[m1 + child]]) {
                      ...
                      unsigned child)
{
    while (child > 0) {
        ...
        if (image[heaps[m1 + parent]] > image[heaps[m1 + child]]) {
...
ADDTYPE(heapify_gt)(ADDTYPE(dheap) * dh)
{
    ...
    while (start >= 0) {
        ADDTYPE(bubble_down_gt)
        (dh->image, dh->B, dh->heaps, dh->m, dh->m + 1, start);
        start = start - 1;
    }
    return;
}
                        ...
                        unsigned root)
{
    while (root * 2 + 1 <= end) {
        ...
        if (child < end && image[heaps[child]] < image[heaps[child + 1]]) {
            child = child + 1;
        }
        if (image[heaps[root]] < image[heaps[child]]) {
                      ...
                      unsigned child)
{
    while (child > 0) {
        ...
        if (image[heaps[parent]] < image[heaps[child]]) {
...
ADDTYPE(heapify_lt)(ADDTYPE(dheap) * dh)
{
    ...
    while (start >= 0) {
        ADDTYPE(bubble_down_lt)(dh->image, dh->B, dh->heaps, dh->m - 1, start);
        start = start - 1;
    }
    return;
}
...
*/
static ADDTYPE(dheap) * ADDTYPE(dheap_new)(const PIX_TYPE *image,
                                           unsigned Nx,
                                           unsigned Ny,
                                           unsigned rx,
                                           unsigned ry)
                       ...
                       unsigned aold)
{
    ...
    if (ci < m) {
        ...
        if (*pprev_was_smaller) {
            pos = ADDTYPE(bubble_down_lt)(image, B, heaps, m - 1, ci);
            if (pos == ci)
        ...
        if (pos == 0 && image[heaps[0]] > image[heaps[m]]) {
            ADDTYPE(HEAP_SWAP)(0, m, B, heaps);

            if (image[heaps[m1]] < image[heaps[m]]) {
        ...
        *pprev_was_smaller = (pos > ci);
        ...
        if (*pprev_was_larger) {
            ...
            if (pos == ci - (m1))
                pos =
                    ADDTYPE(bubble_down_gt)(image, B, heaps, m, m1, ci - (m1));
        }

        if (pos == 0 && image[heaps[m1]] < image[heaps[m]]) {
            ADDTYPE(HEAP_SWAP)(m1, m, B, heaps);
        ...
        *pprev_was_larger = (pos > ci - (m1));
        ...
        if (*pprev_was_smaller) {
            ...
            else if (image[heaps[m1]] < image[heaps[m]]) {
                ADDTYPE(HEAP_SWAP)(m1, m, B, heaps);
                ADDTYPE(bubble_down_gt)(image, B, heaps, m, m1, 0);
...
ADDTYPE(max3)(PIX_TYPE p0, PIX_TYPE p1, PIX_TYPE p2)
{
    return (p0 > p1) ? ((p0 > p2) ? p0 : p2) : ((p1 > p2) ? p1 : p2);
}
...
*/
static inline PIX_TYPE
ADDTYPE(min3)(PIX_TYPE p0, PIX_TYPE p1, PIX_TYPE p2)
{
    return (p0 < p1) ? ((p0 < p2) ? p0 : p2) : ((p1 < p2) ? p1 : p2);
}
...
static inline PIX_TYPE
ADDTYPE(
    median5)(PIX_TYPE p0, PIX_TYPE p1, PIX_TYPE p2, PIX_TYPE p3, PIX_TYPE p4)
{
    /*
    ...
    */
    ...
    return (p3 < p1) ? ((p4 >= p1) ? p1 : ADDTYPE(max3)(p3, p4, p0))
...
ADDTYPE(median9_2)(PIX_TYPE p0,
                   ...
                   PIX_TYPE p8)
{
    return (p4 <= p7) ? ADDTYPE(median5)(p3, p4, p5, p2, p6)
...
*/
static void
ADDTYPE(filter_median_1)(const PIX_TYPE *in,
                         PIX_TYPE *out,
                         unsigned Nx,
                         unsigned Ny,
                         unsigned border_mode)
{
    ...
    switch (border_mode) {
        ...
        case CPL_BORDER_COPY:
            ...
            out +=
                Nx - 1; /* Could also fall through since dimensions are same */
            break;
        case CPL_BORDER_NOP:
    ...
    for (; i2 < istop; i0 += Nx, i1 += Nx, i2 += Nx, out += Nx) {
        ...
        for (; j < Nx; j++) {
            ...
            out[j] = (PIX_TYPE)ADDTYPE(median9_1)(p0, p1, p2, p3, p4, p5, p6,
                                                  p7, p8);
...
*/
void
ADDTYPE(cpl_image_filter_fill_chess)(PIX_TYPE *in_larger,
                                     const PIX_TYPE *in,
                                     unsigned Nx_larger,
                                     unsigned Ny_larger,
                                     unsigned Nx,
                                     ...
                                     unsigned ry)
{
    ...
    for (y = 0; y < ry; y++) {
        ADDTYPE(fill_row)
        (in_larger, Nx_larger, y, 0, Nx_larger,
         (y & 1) == 0 ? PIX_MAX : PIX_MIN, (y & 1) == 0 ? PIX_MIN : PIX_MAX);
    }

    for (y = ry; y < ry + Ny; y++) {
        ADDTYPE(fill_row)
        (in_larger, Nx_larger, y, 0, rx, (y & 1) == 0 ? PIX_MAX : PIX_MIN,
         (y & 1) == 0 ? PIX_MIN : PIX_MAX);

        (void)memcpy(in_larger + rx + y * Nx_larger, in + (y - ry) * Nx,
                     Nx * sizeof(*in));

        ADDTYPE(fill_row)
        (in_larger, Nx_larger, y, rx + Nx, Nx_larger,
 ...
 */
static void
ADDTYPE(filter_median)(const PIX_TYPE *in,
                       PIX_TYPE *out,
                       unsigned Nx,
                       unsigned Ny,
                       unsigned rx,
                       ...
                       unsigned border_mode)
{
    ...
    assure(border_mode == CPL_BORDER_NOP || border_mode == CPL_BORDER_CROP ||
           border_mode == CPL_BORDER_COPY || border_mode == CPL_BORDER_FILTER);
    ...
    if (border_mode == CPL_BORDER_FILTER) {
        ...
        */
        ...
        PIX_TYPE *in_larger =
            malloc(Nx_larger * Ny_larger * sizeof(*in_larger));
        assure(in_larger != NULL);

        ADDTYPE(cpl_image_filter_fill_chess)
        (in_larger, in, Nx_larger, Ny_larger, Nx, Ny, rx, ry);

        ADDTYPE(filter_median)
        (in_larger, out, Nx_larger, Ny_larger, rx, ry, CPL_BORDER_CROP);

        free(in_larger);
        return;
    }

    assert(border_mode == CPL_BORDER_NOP || border_mode == CPL_BORDER_CROP ||
    ...
    */
    for (y = 0 + dh->ry, dx = 1; y < dh->Ny - dh->ry; y++, dx = -dx) {
        ...
        for (x = xfirst; x != xlast + dx; x += dx) {
            if (y == dh->ry) {
                ...
                ADDTYPE(SCA_replace)
                (dh, x + dx * dh->rx, y - dh->ry - 1, y + dh->ry);
            }
            ...
            if (x == xfirst) {
                if (y != dh->ry) {
                    ...
                    unsigned i;
                    for (i = x - dx * dh->rx; i != x + dx * dh->rx; i += dx) {
                        ADDTYPE(SCA_replace)(dh, i, y - dh->ry - 1, y + dh->ry);

                        ADDTYPE(dheap_replace)
                        (dh->image, dh->B, dh->heaps, &prev_was_larger,
                         &prev_was_smaller, dh->m, dh->m + 1,
                         i + ((y + dh->ry) * dh->Nx),
                         i + ((y - dh->ry - 1) * dh->Nx));
                    }
                ...
                */
                for (i = 0; i < dh->Ry; i++) {
                    ...
                    ADDTYPE(dheap_replace)
                    (image, B, heaps, &prev_was_larger, &prev_was_smaller, m,
                     m1, anew, aold);
                }
            }
...
static void
ADDTYPE(cpl_filter_median_fast)(void *out,
                                const void *in,
                                cpl_size Nx,
                                cpl_size Ny,
                                cpl_size hsizex,
                                ...
                                cpl_border_mode mode)
{
    ...
    if ((cpl_size)Nxu == Nx && (cpl_size)Nyu == Ny &&
        ...
        ADDTYPE(filter_median)
        (in, out, (unsigned)Nx, (unsigned)Ny, (unsigned)hsizex,
         (unsigned)hsizey, (unsigned)mode);
    }
    else {
