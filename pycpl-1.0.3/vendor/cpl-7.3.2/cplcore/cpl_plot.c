...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_plot_vector(const char *pre,
                const char *options,
                const char *post,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_plot_bivector(const char *pre,
                  const char *options,
                  const char *post,
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_plot_vectors(const char *pre,
                 const char *options,
                 const char *post,
                 ...
                 cpl_size nvec)
{
    ...
    for (i = 1; i < nvec; i++) {
        ...
        if (i == 1)
            vec_size = cpl_vector_get_size(vectors[i]);
        else
            cpl_ensure_code(vec_size == cpl_vector_get_size(vectors[i]),
                            CPL_ERROR_ILLEGAL_INPUT);
    }
    if (vectors[0] != NULL) {
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_plot_bivectors(const char *pre,
                   const char **options,
                   const char *post,
              ...
              const cpl_mask *mask)
{
    ...
    if (options == NULL || strlen(options) < 1) {
        dvi_options =
            cpl_sprintf("%" CPL_SIZE_FORMAT "X%" CPL_SIZE_FORMAT "-mask (%p)",
                        nx, ny, (const void *)mask);
    ...
    if (dvi_options != options)
        cpl_free(dvi_options);
               ...
               const cpl_image *image)
{
    ...
    if (options == NULL || strlen(options) < 1) {
        dvi_options = cpl_sprintf("%" CPL_SIZE_FORMAT "X%" CPL_SIZE_FORMAT
                                  "-image-(%d) (%p)",
                                  nx, ny, (int)cpl_image_get_type(image),
                                  (const void *)image);
        assert(dvi_options != NULL);
    }
    ...
    if (strstr(plot->exe, "gnuplot")) {
        ...
        if (tmax > 0.0)
            error |=
                cpl_image_multiply_scalar(temp,
                                          256.0 * (1.0 - DBL_EPSILON) / tmax);
        ...
        myoptions =
            cpl_sprintf("# True Value Range: [%g;%g]\n", tmin, tmin + tmax);
        assert(myoptions != NULL);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_plot_image_row(const char *pre,
                   const char *options,
                   const char *post,
                   const cpl_image *image,
                   cpl_size firstrow,
    ...
    do {
        ...
        do {
            ...
            for (j = firstrow - 1; j < lastrow; j += rowstep) {
                ...
                if (cpl_mplot_puts(plot, useoptions))
                    break;
            }
            if (cpl_mplot_puts(plot, ";\n      "))
                break;
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_plot_image_col(const char *pre,
                   const char *options,
                   const char *post,
                   const cpl_image *image,
                   cpl_size firstcol,
    ...
    do {
        ...
        do {
            ...
            for (i = firstcol - 1; i < lastcol; i += colstep) {
                const char *useoptions = options;
                if (i > firstcol - 1 && cpl_mplot_puts(plot, ","))
                    break;
                if (cpl_mplot_puts(plot, " '-' "))
                    break;
                if (useoptions == NULL || strlen(useoptions) < 1) {
                    ...
                    myoptions =
                        (i == firstcol - 1)
                            ? cpl_sprintf("t 'Column %" CPL_SIZE_FORMAT " of a "
                                          ...
                                          (int)cpl_image_get_type(image),
                                          (const void *)image)
                            : cpl_sprintf("t 'Column %" CPL_SIZE_FORMAT
                                          " of the "
                                          "same image'",
                                          i);
                    assert(myoptions != NULL);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_plot_column(const char *pre,
                const char *options,
                const char *post,
                const cpl_table *tab,
                const char *xlab,
                const char *ylab)
{
    ...
    cpl_ensure_code(type_x == CPL_TYPE_INT || type_x == CPL_TYPE_FLOAT ||
                        type_x == CPL_TYPE_DOUBLE,
                    CPL_ERROR_INVALID_TYPE);
    cpl_ensure_code(type_y == CPL_TYPE_INT || type_y == CPL_TYPE_FLOAT ||
    ...
    if (type_y != CPL_TYPE_DOUBLE || invalid_y > 0) {
        ...
        yvec =
            cpl_vector_wrap(n, (double *)cpl_table_get_data_double_const(tab,
                                                                         ylab));
        CPL_DIAG_PRAGMA_POP;
    }
    /* Cast columns to CPL_TYPE_DOUBLE and remove invalid entries */
    if (type_x != CPL_TYPE_DOUBLE || invalid_x > 0) {
        ...
        if (xlab == NULL) {
            ...
            CPL_DIAG_PRAGMA_PUSH_IGN(-Wcast-qual);
            xvec = cpl_vector_wrap(
                n, (double *)cpl_table_get_data_double_const(tab, xlab));
            CPL_DIAG_PRAGMA_POP;
        }
    }
    ...
    if (xlab == NULL)
        cpl_vector_delete(xvec);
    else
        (void)cpl_vector_unwrap(xvec);
...
/*----------------------------------------------------------------------------*/
cpl_error_code
cpl_plot_columns(const char *pre,
                 const char *options,
                 const char *post,
                 const cpl_table *tab,
                 const char **labels,
...
/*----------------------------------------------------------------------------*/
static const char *
cpl_mplot_plotter(void)
{
    return getenv("CPL_PLOTTER") ? getenv("CPL_PLOTTER") : "gnuplot -persist";
}
...
/*----------------------------------------------------------------------------*/
static const char *
cpl_mplot_imager(void)
{
    return getenv("CPL_IMAGER") ? getenv("CPL_IMAGER") : cpl_mplot_plotter();
}
...
/*----------------------------------------------------------------------------*/
static cpl_plot *
cpl_mplot_open(const char *options)
{
    cpl_plot *plot;
    ...
    if (strstr(exe, "gnuplot") == NULL) {
        (void)cpl_error_set_message_(CPL_ERROR_UNSUPPORTED_MODE, "%s", exe);
        return NULL;
    }
    ...
    if (data == NULL) {
        (void)cpl_error_set_message_(CPL_ERROR_FILE_IO, "%s", tmp);
        return NULL;
    }
...
/*----------------------------------------------------------------------------*/
static cpl_plot *
cpl_image_open(const char *options)
{
    cpl_plot *plot;
...
cpl_mplot_close(cpl_plot *plot, const char *options)
{
    ...
    if (fclose(plot->data) != 0)
        error = cpl_error_set_(CPL_ERROR_FILE_IO);
    ...
    if (!error) {
        ...
        if (nstat != 0)
            error = cpl_error_set_message_(CPL_ERROR_FILE_IO,
                                           "system('%s') returned "
                                           "%d",
                                           command ? command : "<NULL>", nstat);
        cpl_free(command);
    }
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_mplot_puts(cpl_plot *plot, const char *cmd)
{
...
/*----------------------------------------------------------------------------*/
static cpl_error_code
cpl_mplot_write(cpl_plot *plot, const char *buffer, size_t length)
{
    cpl_ensure_code(plot != NULL, CPL_ERROR_NULL_INPUT);
