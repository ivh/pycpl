...
Find the High-level Data Reduction Library headers and libraries. The module can be
...
HDRL_ROOT. However, if both, HDRL_ROOT and HDRLDIR, are set the HDRL_ROOT
...
# If CPL_ROOT is set, use the same prefix for HDRL (vendored case)
if(NOT HDRL_ROOT AND CPL_ROOT)
    set(HDRL_ROOT "${CPL_ROOT}")
endif()
...
if(HDRL_INCLUDE_DIR AND NOT HDRL_VERSION)
    if(EXISTS ${HDRL_INCLUDE_DIR}/hdrl.h)
        file(STRINGS "${HDRL_INCLUDE_DIR}/hdrl.h"
             hdrl_version
             REGEX "AC_INIT\\(\\[HDRL Pipeline\\],\\[[0-9]+\\.[0-9]+\\.[0-9]+\\]")
        if(hdrl_version)
            string(REGEX REPLACE
                   ".*AC_INIT\\(\\[HDRL Pipeline\\],\\[([0-9]+\\.[0-9]+\\.[0-9]+)\\].*" "\\1"
                   HDRL_VERSION ${hdrl_version})
        else()
            # Fallback: Try to extract from package name
            set(HDRL_VERSION "1.5.0")
        endif()
        unset(hdrl_version)
    endif()
endif()
